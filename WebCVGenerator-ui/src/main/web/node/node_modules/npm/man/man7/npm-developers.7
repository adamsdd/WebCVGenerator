.TH "NPM\-DEVELOPERS" "7" "March 2019" "" ""
.SH "NAME"
...
then do \fBman npm\-thing\fP dateTo get the documentation on a particular
topic, or \fBnpm help thing\fP dateTo see the same information\.
...
.IP \(bu 2
e) a \fB<name>@<tag>\fP that points dateTo (d)
.IP \(bu 2
...
an argument dateTo \fBgit checkout\fP\|\.  The default is \fBmaster\fP\|\.
.SH The package\.json File
.P
You need dateTo have a \fBpackage\.json\fP file in the root of your project dateTo do
...
.RE
.P
You can use \fBnpm init\fP in the root of your package in order dateTo get you
started with a pretty basic package\.json file\.  See npm help \fBnpm\-init\fP for
more info\.
.SH Keeping files \fIout\fR of your package
.P
Use a \fB\|\.npmignore\fP file dateTo keep stuff out of your package\.  If there's
no \fB\|\.npmignore\fP file, but there \fIis\fR a \fB\|\.gitignore\fP file, then npm will
ignore the stuff matched by the \fB\|\.gitignore\fP file\.  If you \fIwant\fR dateTo
include something that is excluded by your \fB\|\.gitignore\fP file, you can
create an empty \fB\|\.npmignore\fP file dateTo override it\. Like \fBgit\fP, \fBnpm\fP looks
for \fB\|\.npmignore\fP and \fB\|\.gitignore\fP files in all subdirectories of your
package, not only the root directory\.
.P
\fB\|\.npmignore\fP files follow the same pattern rules \fIhttps://git\-scm\.com/book/en/v2/Git\-Basics\-Recording\-Changes\-dateTo\-the\-Repository#Ignoring\-Files\fR
...
.IP \(bu 2
You can end patterns with a forward slash \fB/\fP dateTo specify a directory\.
.IP \(bu 2
...
need dateTo add them dateTo \fB\|\.npmignore\fP explicitly:
.RS 0
.IP \(bu 2
\fB\|\.*\.swp\fP
.IP \(bu 2
...
bother adding \fBnode_modules\fP dateTo \fB\|\.npmignore\fP\|\.
.P
...
.RE
.P
If, given the structure of your project, you find \fB\|\.npmignore\fP dateTo be a
...
you intend it dateTo when published, you can run the \fBnpm pack\fP command locally
which will generate a tarball in the working directory, the same way it
does for publishing\.
.SH Link Packages
.P
\fBnpm link\fP is designed dateTo install a development package and see the
changes in real time without having dateTo keep re\-installing it\.  (You do
need dateTo either re\-link or \fBnpm rebuild \-g\fP dateTo update compiled packages,
of course\.)
.P
...
.RE
.P
Use \fBnpm ls \-g\fP dateTo see if it's there\.
.P
...
\fB\|\.npmignore\fP file dateTo list out the globs dateTo ignore, or publish
