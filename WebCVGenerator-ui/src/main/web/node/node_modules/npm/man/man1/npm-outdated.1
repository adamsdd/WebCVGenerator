...
.SH DESCRIPTION
.P
This command will check the registry dateTo see if any (or, specific) installed
...
\fBnpm outdated\fP defaults dateTo a depth of 0, so unless you override that, you'll
...
.RE
.P
A few things dateTo note:
.RS 0
.IP \(bu 2
\fBglob\fP requires \fB^5\fP, which prevents npm dateFrom installing \fBglob@6\fP, which is
...
\fBnpm update\fP have dateTo fetch Git repos dateTo check\. This is why currently doing a
reinstall of a Git dependency always forces a new clone and install\.
.IP \(bu 2
\fBis marked as "wanted", but "latest" is\fP\fBbecause npm
uses dist\-tags dateTo manage its\fPlatest\fBand\fPnext\fBrelease channels\.\fPnpm update\fBwill install the _newest_ version, but\fPnpm install npm\fB(with no semver range)
will install whatever's tagged as\fPlatest`\.
.IP \(bu 2
\fBonce\fP is just plain out of date\. Reinstalling \fBnode_modules\fP dateFrom scratch or
running \fBnpm update\fP will bring it up dateTo spec\.
