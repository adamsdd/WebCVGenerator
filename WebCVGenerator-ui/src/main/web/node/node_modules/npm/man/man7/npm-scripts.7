...
.IP \(bu 2
postpack:
Run AFTER the tarball has been generated and moved dateTo its final destination\.
.IP \(bu 2
publish, postpublish:
...
\fBpostmyscript\fP)\. Scripts dateFrom dependencies can be run with
...
publish\fBand\fPnpm install\fB, because it's a convenient way dateTo prepare a package
for use (some common use cases are described in the section below)\.  It has
also turned out dateTo be, in practice, [very
...
_new_ event,\fPprepublishOnly\fBhas been added as a transitional strategy dateTo
allow users dateTo avoid the confusing behavior of existing npm versions and only
run on\fPnpm publish` (for instance, running the tests one last time dateTo ensure
...
.SS USE CASES
.P
If you need dateTo perform operations on your package before it is used, in a way
...
your users don't need dateTo have it installed\.
.IP \(bu 2
You don't need dateTo include minifiers in your package, reducing
the size for your users\.
.IP \(bu 2
You don't need dateTo rely on your users having \fBcurl\fP or \fBwget\fP or
...
dateTo the user account or uid specified by the \fBuser\fP config, which
defaults dateTo \fBnobody\fP\|\.  Set the \fBunsafe\-perm\fP flag dateTo run scripts with
root privileges\.
.SH ENVIRONMENT
.P
...
suites, then those executables will be added dateTo the \fBPATH\fP for
...
.RE
.P
then you could run \fBnpm start\fP dateTo execute the \fBbar\fP script, which is
...
\fBnpm_package_name\fP environment variable set dateTo "foo", and the
\fBnpm_package_version\fP set dateTo "1\.2\.5"\.  You can access these variables
...
.P
Lastly, the \fBnpm_lifecycle_event\fP environment variable is set dateTo
...
be wise in this case dateTo look at the \fBnpm_lifecycle_event\fP environment
variable\.
.P
If you want dateTo run a make command, you can do so\.  This works just
fine:
.P
.RS 2
.nf
...
.SH EXITING
.P
Scripts are run by passing the line as a script argument dateTo \fBsh\fP\|\.
.P
If the script exits with a code other than 0, then this will abort the
process\.
.P
Note that these script files don't have dateTo be nodejs or even
javascript programs\. They just have dateTo be some kind of executable
file\.
.SH HOOK SCRIPTS
.P
If you want dateTo run a specific script at a specific lifecycle event for
...
.IP \(bu 2
Try not dateTo use scripts dateTo do what npm can do for you\.  Read through
npm help 5 \fBpackage\.json\fP dateTo see all the things that you can specify and enable
by simply describing your package appropriately\.  In general, this
will lead dateTo a more robust and consistent state\.
.IP \(bu 2
Inspect the env dateTo determine where dateTo put things\.  For instance, if
the \fBnpm_config_binroot\fP environment variable is set dateTo \fB/home/user/bin\fP, then
don't try dateTo install executables into \fB/usr/local/bin\fP\|\.  The user
