...
is installed\.
.IP \(bu 2
Install it \fBlocally\fR if you're going dateTo \fBrequire()\fP it\.
.IP \(bu 2
Install it \fBglobally\fR if you're going dateTo run it on the command line\.
.IP \(bu 2
...
.P
The \fBprefix\fP config defaults dateTo the location where node is installed\.
...
\fBrequire("packagename")\fP dateTo load its main module, or
\fBrequire("packagename/lib/path/dateTo/sub/module")\fP dateTo load other modules\.
.P
Global installs on Unix systems go dateTo \fB{prefix}/lib/node_modules\fP\|\.
Global installs on Windows go dateTo \fB{prefix}/node_modules\fP (that is, no
\fBlib\fP folder\.)
.P
...
more details\.
.P
If you wish dateTo \fBrequire()\fP a package, then install it locally\.
.SS Executables
.P
...
\fB\|\./node_modules/\.bin\fP so that they can be made available dateTo scripts run
...
\fBtmp\fP config, which defaults dateTo the TMPDIR, TMP, or TEMP environment
...
.SH More Information
.P
When installing locally, npm first tries dateTo find an appropriate
\fBprefix\fP folder\.  This is so that \fBnpm install foo@1\.2\.3\fP will install
dateTo the sensible root of your package, even if you happen dateTo have \fBcd\fPed
...
.P
Any bin files are symlinked dateTo \fB\|\./node_modules/\.bin/\fP, so that they may
be found by npm scripts when necessary\.
.SS Global Installation
.P
If the \fBglobal\fP configuration is set dateTo true, then npm will
...
structure is: \fBfoo/node_modules/bar/node_modules/baz\fP, there's no need dateTo
...
is still possible dateTo have \fBa/node_modules/b/node_modules/a\fP if the two
...
.P
This allows a package maintainer dateTo install all of their dependencies
