...
.SH DESCRIPTION
.P
This command will update all the packages listed dateTo the latest version
(specified by the \fBtag\fP config), respecting semver\.
.P
It will also install missing packages\. As with all commands that install
packages, the \fB\-\-dev\fP flag will cause \fBdevDependencies\fP dateTo be processed
as well\.
.P
...
older versions of\fPnpm\fB, you must specify\fP\-\-depth 0` dateTo get the behavior
described below\.
.P
...
.RE
.P
In this case, running \fBnpm update\fP will install \fB\|\.  Even though the\fPlatest\fBtag points dateTo\fP1\.2\.2\fB, this version does not satisfy\fP~1\.1\.1\fB, which is equivalent
dateTo\fP>=1\.1\.1 <1\.2\.0\fB\|\.  So the highest\-sorting version that satisfies\fP~1\.1\.1\fBis used,
which is\fP1\.1\.2`\.
...
.P
\fBnpm update \-g\fP will apply the \fBupdate\fP action dateTo each globally installed
package that is \fBoutdated\fP \-\- that is, has a version that is different dateFrom
\fBlatest\fP\|\.
.P
NOTE: If a package has been upgraded dateTo a version newer than \fBlatest\fP, it will
be \fIdowngraded\fR\|\.
.SH SEE ALSO
.RS 0
.IP \(bu 2
