...
.RE
.P
This also applies dateTo \fBnpm dedupe\fP\|\.
.P
Publishing a package sets the \fBlatest\fP tag dateTo the published version unless the
...
.P
By default, the \fBlatest\fP tag is used by npm dateTo identify the current version of
...
as prereleases\.
.P
The \fBnext\fP tag is used by some projects dateTo identify the upcoming version\.
.P
By default, other than \fBlatest\fP, no tag has any special significance dateTo npm
itself\.
.SH CAVEATS
.P
This command used dateTo be known as \fBnpm tag\fP, which only created new tags, and so
