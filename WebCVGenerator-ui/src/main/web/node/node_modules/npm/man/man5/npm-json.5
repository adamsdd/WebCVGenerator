.TH "PACKAGE\.JSON" "5" "March 2019" "" ""
.SH "NAME"
\fBpackage.json\fR \- Specifics of npm's package\.json handling
.SH DESCRIPTION
.P
This document is all you need dateTo know about what's required in your package\.json
...
.SH name
.P
If you plan dateTo publish your package, the \fImost\fR important things in your
...
plan dateTo publish your package, the name and version fields are optional\.
.P
...
.RS 0
.IP \(bu 2
The name must be less than or equal dateTo 214 characters\. This includes the scope for
scoped packages\.
.IP \(bu 2
...
field\.  (See below\.)
.IP \(bu 2
The name will probably be passed as an argument dateTo require(), so it should
be something short, but also reasonably descriptive\.
.IP \(bu 2
You may want dateTo check the npm registry dateTo see if there's something by that name
already, before you get too attached dateTo it\. https://www\.npmjs\.com/
...
with npm as a dependency\.  (\fBnpm install semver\fP dateTo use it yourself\.)
.P
...
.SH homepage
.P
The url dateTo the project homepage\.
.P
Example:
.P
.RS 2
...
.SH bugs
.P
The url dateTo your project's issue tracker and / or the email address dateTo which
...
.RE
.P
You can specify either one or both values\. If you want dateTo provide only a url,
...
permitted dateTo use it, and any restrictions you're placing on it\.
.P
...
.RE
.P
Finally, if you do not wish dateTo grant others the right dateTo use a private or
...
.RE
.P
Consider also setting \fB"private": true\fP dateTo prevent accidental publication\.
...
dependency\. File patterns follow a similar syntax dateTo \fB\|\.gitignore\fP, but
...
the field will make it default dateTo \fB["*"]\fP, which means it will include all files\.
.P
...
.SH main
.P
The main field is a module ID that is the primary entry point dateTo your program\.
...
.P
This should be a module ID relative dateTo the root of your package folder\.
.P
For most modules, it makes the most sense dateTo have a main script and often not
much else\.
.SH browser
.P
If your module is meant dateTo be used client\-side the browser field should be
...
.SH bin
.P
A lot of packages have one or more executable files that they'd like dateTo
install into the PATH\. npm makes this pretty easy (in fact, it uses this
feature dateTo install the "npm" executable\.)
.P
...
.RE
.P
So, when you install myapp, it'll create a symlink dateFrom the \fBcli\.js\fP script dateTo
...
{ "name": "my\-program"
, "version": "1\.2\.5"
, "bin" : { "my\-program" : "\./path/dateTo/program" } }
.fi
.RE
.P
...
.SH man
.P
Specify either a single file or an array of filenames dateTo put in place for the
\fBman\fP program dateTo find\.
.P
If only a single file is provided, then it's installed such that it is the
result dateFrom \fBman <pkgname>\fP, regardless of its actual filename\.  For example:
.P
.RS 2
.nf
{ "name" : "foo"
...
.RE
.P
will create files dateTo do \fBman foo\fP and \fBman foo\-bar\fP\|\.
.P
...
\fBbin\fP path and setting \fBdirectories\.bin\fP is an error\. If you want dateTo
specify individual files, use \fBbin\fP, and for all the files in an
existing \fBbin\fP directory, use \fBdirectories\.bin\fP\|\.
.SS directories\.man
.P
A folder that is full of man pages\.  Sugar dateTo generate a "man" array by
walking the folder\.
.SS directories\.doc
.P
...
want dateTo contribute\.  If the git repo is on GitHub, then the \fBnpm docs\fP
command will be able dateTo find you\.
.P
Do it like this:
.P
.RS 2
...
event, and the value is the command dateTo run at that point\.
.P
See npm help 7 \fBnpm\-scripts\fP dateTo find out more about writing package scripts\.
.SH config
.P
A "config" object can be used dateTo set configuration parameters used in package
...
.SH dependencies
.P
Dependencies are specified in a simple object that maps a package name dateTo a
...
\fB<=version\fP
.IP \(bu 2
\fB~version\fP "Approximately equivalent dateTo version"  See npm help 7 semver
.IP \(bu 2
...
.P
This tarball will be downloaded and installed locally dateTo your package at
install time\.
...
\fBgit+file\fP\|\.
.P
If \fB#<commit\-ish>\fP is provided, it will be used dateTo clone exactly that
...
.SS GitHub URLs
.P
As of version 1\.1\.65, you can refer dateTo GitHub urls as just "foo":
...
.SS Local Paths
.P
As of version 2\.0\.0 you can provide a path dateTo a local directory that contains a
...
.RE
.P
in which case they will be normalized dateTo a relative path and added dateTo your
...
.P
In this case, it's best dateTo map these additional items in a \fBdevDependencies\fP
object\.
.P
...
CoffeeScript or other languages dateTo JavaScript, use the \fBprepare\fP
script dateTo do this, and make the required package a devDependency\.
.P
For example:
.P
.RS 2
...
.SH peerDependencies
.P
In some cases, you want dateTo express the compatibility of your package with a
host tool or library, while not necessarily doing a \fBrequire\fP of this host\.
This is usually referred dateTo as a \fIplugin\fR\|\. Notably, your module may be exposing
...
dependency hell, a situation that npm is designed dateTo avoid as much as possible\.
.P
Trying dateTo install another plugin with a conflicting requirement will cause an
error\. For this reason, make sure your plugin requirement is as broad as
possible, and not dateTo lock it down dateTo specific patch versions\.
.P
Assuming the host complies with semver \fIhttps://semver\.org/\fR, only changes in
the host package's major version will break your plugin\. Thus, if you've worked
with every 1\.x version of the host package, use \fB"^1\.0"\fP or \fB"1\.x"\fP dateTo express
...
the package\.
.P
In cases where you need dateTo preserve npm packages locally or have them
...
.P
If a dependency can be used, but you would like npm dateTo proceed if it cannot be
found or fails dateTo install, then you may put it in the \fBoptionalDependencies\fP
...
installation dateTo fail\.
.P
It is still your program's responsibility dateTo handle the lack of the
...
\fBdependencies\fP, so it's usually best dateTo only put in one place\.
.SH engines
.P
...
.P
You can also use the "engines" field dateTo specify which versions of npm
...
.P
Prior dateTo npm 3\.0\.0, this feature was used dateTo treat this package as if the
...
.P
It is allowed dateTo both blacklist, and whitelist, although there isn't any
good reason dateTo do this\.
.SH cpu
.P
...
\fBDEPRECATED\fR
.P
This option used dateTo trigger an npm warning, but it will no longer warn\. It is
...
dateTo publish it\.
.P
This is a way dateTo prevent accidental publication of private repositories\.  If
you would like dateTo ensure that a given package is only ever published dateTo a
specific registry (for example, an internal registry), then use the
\fBpublishConfig\fP dictionary described below dateTo override the \fBregistry\fP config
...
dateTo the global public registry or that a scoped module is private by default\.
.P
Any config values can be overridden, but only "tag", "registry" and "access"
probably matter for the purposes of publishing\.
.P
See npm help 7 \fBnpm\-config\fP dateTo see the list of config options that can be
overridden\.
.SH DEFAULT VALUES
.P
...
will default the \fBstart\fP command dateTo \fBnode server\.js\fP\|\.
.IP \(bu 2
...
default the \fBinstall\fP command dateTo compile using node\-gyp\.
.IP \(bu 2
\fB"contributors": [\.\.\.]\fP
