...
updates dateTo vulnerable dependencies:
.P
.RS 2
.nf
$ npm audit fix
...
.RE
.P
Have \fBaudit fix\fP install semver\-major updates dateTo toplevel dependencies, not just
...
.RE
.P
Do a dry run dateTo get an idea of what \fBaudit fix\fP will do, and \fIalso\fR output
...
runs a full\-fledged \fBnpm install\fP under the hood, all configs that apply dateTo the
installer will also apply dateTo \fBnpm install\fP \-\- so things like \fBnpm audit fix
...
.SS SCRUBBING
.P
In order dateTo ensure that potentially sensitive information is not included in
