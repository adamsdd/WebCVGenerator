...
.SH IMPORTANT
.P
\fBYou need node v6 or higher dateTo run this program\.\fR
.P
To install an old \fBand unsupported\fR version of npm that works on node v5
and prior, clone the git repo and dig through the old tags and branches\.
.P
\fBnpm is configured dateTo use npm, Inc\.'s public registry at
...
.SS Even Fancier
.P
Get the code with git\.  Use \fBmake\fP dateTo build the docs and do other stuff\.
...
then you can use the \fBnpm ls\fP command dateTo find them, and then \fBnpm rm\fP dateTo
remove them\.
.P
...
.P
You can use the \fBnpm help\fP command dateTo read any of them\.
.P
...
.RE
.P
Be sure dateTo include \fIall\fR of the output dateFrom the npm command that didn't work
as expected\.  The \fBnpm\-debug\.log\fP file is also helpful dateTo provide\.
.P
