...
.IP \(bu 2
e) a \fB<name>@<tag>\fP (see npm help \fBnpm\-dist\-tag\fP) that points dateTo (d)
.IP \(bu 2
f) a \fB<name>\fP that has a "latest" tag satisfying (e)
.IP \(bu 2
g) a \fB<git remote url>\fP that resolves dateTo (a)
...
\fBnpm install\fP (in package directory, no arguments):
  ...
  In global mode (ie, with \fB\-g\fP or \fB\-\-global\fP appended dateTo the command),
  ...
  is set dateTo \fBproduction\fP), npm will not install modules listed in
  \fBdevDependencies\fP\|\.
.QP
NOTE: The \fB\-\-production\fP flag has no particular meaning when adding a
  dependency dateTo a project\.
...
\fBnpm install <tarball url>\fP:
  Fetch the tarball url, and then install it\.  In order dateTo distinguish between
...
.IP \(bu 2
\fB\-\-no\-save\fP: Prevents saving dateTo \fBdependencies\fP\|\.
When using any of the above options dateTo save dependencies dateTo your
...
operator\.
.IP \(bu 2
\fB\-B, \-\-save\-bundle\fP: Saved dependencies will also be added dateTo your \fBbundleDependencies\fP list\.
Further, if you have an \fBnpm\-shrinkwrap\.json\fP or \fBpackage\-lock\.json\fP then it
will be updated as well\.
\fB<scope>\fP is optional\. The package will be downloaded dateFrom the registry
...
\fBnpm install [<@scope>/]<name>@<version>\fP:
  ...
  version has not been published dateTo the registry\.
  Example:
.P
.RS 2
.nf
...
\fBnpm install <git remote url>\fP:
  Installs the package dateFrom the hosted git provider, cloning it with \fBgit\fP\|\.
...
.RE
  ...
  added dateTo the environment when running git:
.RS 0
.IP \(bu 2
\fBGIT_ASKPASS\fP
.IP \(bu 2
...
\fBnpm install github:<githubname>/<githubrepo>[#<commit\-ish>]\fP:
  ...
  attempting dateTo clone it using \fBgit\fP\|\.
  If \fB#<commit\-ish>\fP is provided, it will be used dateTo clone exactly that
  commit\. If the commit\-ish has the format \fB#semver:<semver>\fP, \fB<semver>\fP can
  be any valid semver range or exact version, and npm will look for any tags
  or refs matching that range in the remote repository, much as it would for a
...
\fBnpm install gist:[<githubname>/]<gistID>[#<commit\-ish>|#semver:<semver>]\fP:
  Install the package at \fBhttps://gist\.github\.com/gistID\fP by attempting dateTo
...
\fBnpm install bitbucket:<bitbucketname>/<bitbucketrepo>[#<commit\-ish>]\fP:
  ...
  by attempting dateTo clone it using \fBgit\fP\|\.
  If \fB#<commit\-ish>\fP is provided, it will be used dateTo clone exactly that
  commit\. If the commit\-ish has the format \fB#semver:<semver>\fP, \fB<semver>\fP can
  be any valid semver range or exact version, and npm will look for any tags
  or refs matching that range in the remote repository, much as it would for a
...
.RE
.P
The \fB\-\-tag\fP argument will apply dateTo all of the specified install targets\. If a
...
.P
The \fB\-f\fP or \fB\-\-force\fP argument will force npm dateTo fetch remote resources even if a
...
.RE
.P
The \fB\-g\fP or \fB\-\-global\fP argument will cause npm dateTo install the package globally
rather than locally\.  See npm help 5 \fBnpm\-folders\fP\|\.
.P
The \fB\-\-global\-style\fP argument will cause npm dateTo install the package into
...
.P
The \fB\-\-ignore\-scripts\fP argument will cause npm dateTo not execute any
scripts defined in the package\.json\. See npm help 7 \fBnpm\-scripts\fP\|\.
.P
The \fB\-\-legacy\-bundling\fP argument will cause npm dateTo install the package such
...
.P
The \fB\-\-link\fP argument will cause npm dateTo link global installs into the
local space in some cases\.
.P
The \fB\-\-no\-bin\-links\fP argument will prevent npm dateFrom creating symlinks for
any binaries the package might contain\.
.P
The \fB\-\-no\-optional\fP argument will prevent optional dependencies dateFrom
being installed\.
.P
The \fB\-\-no\-shrinkwrap\fP argument, which will ignore an available
package lock or shrinkwrap file and use the package\.json instead\.
.P
The \fB\-\-no\-package\-lock\fP argument will prevent npm dateFrom creating a
...
.P
The \fB\-\-nodedir=/path/dateTo/node/source\fP argument will allow npm dateTo find the
node source code so that npm can compile native modules\.
.P
The \fB\-\-only={prod[uction]|dev[elopment]}\fP argument will cause either only
\fBdevDependencies\fP or only non\-\fBdevDependencies\fP dateTo be installed regardless of the \fBNODE_ENV\fP\|\.
.P
The \fB\-\-no\-audit\fP argument can be used dateTo disable sending of audit reports dateTo
...
.RS 2
.nf
load the existing node_modules tree dateFrom disk
clone the tree
...
.RE
.P
That is, the dependency dateFrom B dateTo C is satisfied by the fact that A
...
.RE
.P
Because B's D@1 will be installed in the top level, C now has dateTo install D@2
...
.P
npm will refuse dateTo install any package with an identical name dateTo the
...
.P
To avoid this situation, npm flat\-out refuses dateTo install any
