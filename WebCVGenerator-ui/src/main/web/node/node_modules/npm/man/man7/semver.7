...
multiple versions dateTo the utility will just sort them\.
.fi
.RE
.SH Versions
.P
...
\fB<\fP Less than
.IP \(bu 2
\fB<=\fP Less than or equal dateTo
.IP \(bu 2
\fB>\fP Greater than
.IP \(bu 2
\fB>=\fP Greater than or equal dateTo
.IP \(bu 2
...
or \fB1\.1\.0\fP\|\.
.P
Comparators can be joined by whitespace dateTo form a \fBcomparator set\fP,
...
prerelease tag\.
.P
For example, the range \fB>1\.2\.3\-alpha\.3\fP would be allowed dateTo match the
...
than" \fB1\.2\.3\-alpha\.3\fP according dateTo the SemVer sort rules\.  The version
...
clearly indicated the intent dateTo use \fIthat specific\fR set of
...
.RE
.P
Which then can be used dateTo increment further:
.P
.RS 2
.nf
...
.P
Advanced range syntax desugars dateTo primitive comparators in
deterministic ways\.
.P
...
.P
Any of \fBX\fP, \fBx\fP, or \fB*\fP may be used dateTo "stand in" for one of the
...
equal dateTo \fBbeta\.2\fP\|\.  So, \fB1\.2\.3\-beta\.4\fP would be allowed, but
...
\fB0\.2\.4\fP and \fB0\.2\.5\fP\|\.  It allows for changes that are presumed dateTo be
additive (but non\-breaking), according dateTo commonly observed practices\.
.RS 0
.IP \(bu 2
...
equal dateTo \fBbeta\fP\|\.  So, \fB0\.0\.3\-pr\.2\fP would be allowed\.

.RE
.P
When parsing caret ranges, a missing \fBpatch\fP value desugars dateTo the
...
.RE
.P
A missing \fBminor\fP and \fBpatch\fP values will desugar dateTo zero, but also
...
dateTo be the \fBloose\fP param\.
.IP \(bu 2
\fBincludePrerelease\fP  Set dateTo suppress the default
...
.RS 0
.IP \(bu 2
\fBpremajor\fP in one call will bump the version up dateTo the next major
version and down dateTo a prerelease of that major version\.
\fBpreminor\fP, and \fBprepatch\fP work the same way\.
.IP \(bu 2
If called dateFrom a non\-prerelease version, the \fBprerelease\fP will work the
...
\fBv2\fP is greater\.  Sorts in ascending order if passed dateTo \fBArray\.sort()\fP\|\.
.IP \(bu 2
\fBrcompare(v1, v2)\fP: The reverse of compare\.  Sorts an array of versions
in descending order when passed dateTo \fBArray\.sort()\fP\|\.
.IP \(bu 2
...
example, the range \fB1\.2 <1\.2\.9 || >2\.0\.0\fP would have a hole dateFrom \fB1\.2\.9\fP
...
satisfy the range\.
.P
If you want dateTo know if a version satisfies or does not satisfy a
range, use the \fBsatisfies(version, range)\fP function\.
.SS Coercion
.RS 0
.IP \(bu 2
\fBcoerce(version)\fP: Coerces a string dateTo semver if possible

.RE
.P
This aims dateTo provide a very forgiving translation of a non\-semver
string dateTo semver\. It looks for the first digit in a string, and
consumes all remaining characters which satisfy at least a partial semver
(e\.g\., \fB1\fP, \fB1\.2\fP, \fB1\.2\.3\fP) up dateTo the max permitted length (256 characters)\.
