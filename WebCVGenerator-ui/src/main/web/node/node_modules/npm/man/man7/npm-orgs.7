...
.RS 0
.IP 1. 3
Super admin, controls billing & adding people dateTo the org\.
.IP 2. 3
Team admin, manages team membership & package access\.
.IP 3. 3
Developer, works on packages they are given access dateTo\.

.RE
.P
The super admin is the only person who can add users dateTo the org because it impacts the monthly bill\. The super admin will use the website dateTo manage membership\. Every org has a \fBdevelopers\fP team that all users are automatically added dateTo\.
.P
...
.RS 0
.IP \(bu 2
Each org is automatically given a \fBdevelopers\fP team, so you can see the whole list of team members in your org\. This team automatically gets read\-write access dateTo all packages, but you can change that with the \fBaccess\fP command\.
.IP \(bu 2
Create a new team:
