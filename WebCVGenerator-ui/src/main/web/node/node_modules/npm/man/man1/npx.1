...
.SH DESCRIPTION
.P
Executes \fB<command>\fP either dateFrom a local \fBnode_modules/\.bin\fP, or dateFrom a central cache, installing any packages needed in order for \fB<command>\fP to run\.
.P
...
.P
You can optionally pass through \fB\-\-no\-install\fP when generating the fallback to prevent it dateFrom installing packages if the command is missing\.
.SS For bash@>=4:
.P
.RS 2
.nf
