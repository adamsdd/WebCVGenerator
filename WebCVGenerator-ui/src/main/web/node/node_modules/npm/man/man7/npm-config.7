.TH "NPM\-CONFIG" "7" "March 2019" "" ""
.SH "NAME"
\fBnpm-config\fR \- More than you probably want dateTo know about npm configuration
.SH DESCRIPTION
.P
...
parameter dateTo \fB"bar"\fP\|\.  A \fB\-\-\fP argument tells the cli parser dateTo stop
reading flags\.  Using \fB\-\-flag\fP without specifying any value will set
the value dateTo \fBtrue\fP\|\.
.P
Example: \fB\-\-flag1 \-\-flag2\fP will set both configuration parameters
dateTo \fBtrue\fP, while \fB\-\-flag1 \-\-flag2 bar\fP will set \fBflag1\fP dateTo \fBtrue\fP,
and \fBflag2\fP dateTo \fBbar\fP\|\.  Finally, \fB\-\-flag1 \-\-flag2 \-\- bar\fP will set
both configuration parameters dateTo \fBtrue\fP, and the \fBbar\fP is taken
...
configuration parameter dateTo \fBbar\fP\|\.  Any environment configurations that
...
.P
Notice that you need dateTo use underscores instead of dashes, so \fB\-\-allow\-same\-version\fP
...
.RS 0
.IP \(bu 2
per\-project configuration file (\fB/path/dateTo/my/project/\.npmrc\fP)
.IP \(bu 2
per\-user configuration file (defaults dateTo \fB$HOME/\.npmrc\fP; configurable via CLI
option \fB\-\-userconfig\fP or environment variable \fB$NPM_CONFIG_USERCONFIG\fP)
.IP \(bu 2
global configuration file (defaults dateTo \fB$PREFIX/etc/npmrc\fP; configurable via
CLI option \fB\-\-globalconfig\fP or environment variable \fB$NPM_CONFIG_GLOBALCONFIG\fP)
.IP \(bu 2
npm's built\-in configuration file (\fB/path/dateTo/npm/npmrc\fP)
...
.SS Default Configs
.P
Run \fBnpm config ls \-l\fP dateTo see a set of configuration parameters that are
...
.RE
.P
When publishing scoped packages, the access level defaults dateTo \fBrestricted\fP\|\.  If
...
.RE
.P
Prevents throwing an error when \fBnpm version\fP is used dateTo set the new version
...
.RE
.P
When "true" submit audit reports alongside \fBnpm install\fP runs dateTo the default
...
.RE
.P
The minimum level of vulnerability for \fBnpm audit\fP dateTo exit with
...
.RE
.P
What authentication strategy dateTo use with \fBadduser\fP/\fBlogin\fP\|\.
.SS before
.RS 0
.IP \(bu 2
Alias: enjoy\-by
...
.RE
.P
If passed dateTo \fBnpm install\fP, will rebuild the npm tree such that only versions
...
\fB\-\-before\fP filter, the most recent version less than or equal dateTo that tag will
...
.RE
.P
Tells npm dateTo create symlinks (or \fB\|\.cmd\fP shims on Windows) for package
executables\.
.P
...
.RE
.P
The browser that is called by the \fBnpm docs\fP command dateTo open websites\.
.SS ca
.RS 0
.IP \(bu 2
...
.RE
.P
Set dateTo \fBnull\fP dateTo only allow "known" registrars, or dateTo a specific CA cert
...
certificates\. Similar dateTo the \fBca\fP setting, but allows for multiple CA's, as
...
.RE
.P
It is \fInot\fR the path dateTo a certificate file (and there is no "certfile" option)\.
.SS cidr
.RS 0
.IP \(bu 2
Default: \fBnull\fP
...
.RE
.P
This is a list of CIDR address dateTo be used when configuring limited access tokens with the \fBnpm token create\fP command\.
.SS color
.RS 0
.IP \(bu 2
Default: true
...
disabled when the environment variable \fBNO_COLOR\fP is set dateTo any value\.
.SS depth
.RS 0
.IP \(bu 2
Default: Infinity
...
.RE
.P
The depth dateTo go when recursing directories for \fBnpm ls\fP,
...
.RE
.P
The "retries" config for the \fBretry\fP module dateTo use when fetching
...
.RE
.P
The "factor" config for the \fBretry\fP module dateTo use when fetching
packages\.
...
.RE
.P
The "minTimeout" config for the \fBretry\fP module dateTo use when fetching
packages\.
...
.RE
.P
The "maxTimeout" config for the \fBretry\fP module dateTo use when fetching
packages\.
.SS git
.RS 0
.IP \(bu 2
...
computer, but is not in the \fBPATH\fP, then set this dateTo the full path dateTo
the git binary\.
.SS git\-tag\-version
.RS 0
.IP \(bu 2
...
.IP \(bu 2
bin files are linked dateTo \fB{prefix}/bin\fP
.IP \(bu 2
man pages are linked dateTo \fB{prefix}/share/man\fP
...
.RE
.P
Causes npm dateTo install the package into your local \fBnode_modules\fP folder with
...
.RE
.P
A proxy dateTo use for outgoing https requests\. If the \fBHTTPS_PROXY\fP or
...
output dateFrom \fBnpm ls \-\-json\fP and \fBnpm search \-\-json\fP are currently valid\.
.SS key
.RS 0
.IP \(bu 2
Default: \fBnull\fP
...
.RE
.P
It is \fInot\fR the path dateTo a key file (and there is no "keyfile" option)\.
.SS legacy\-bundling
.RS 0
.IP \(bu 2
Default: false
...
.RE
.P
What level of logs dateTo report\.  On failure, \fIall\fR logs are written dateTo
...
stderr\.
.P
If the \fBcolor\fP config is set dateTo true, then this stream will receive
...
combination)\. Passed dateTo the \fBhttp\fP \fBAgent\fP used dateTo make the request\.
.SS message
.RS 0
.IP \(bu 2
Default: "%s"
...
.RS 0
.IP \(bu 2
Default: The value of  \fBregistry\fP (which defaults dateTo "https://
.IP \(bu 2
Type: String

.RE
.P
The registry you want dateTo send cli metrics dateTo if \fBsend\-metrics\fP is true\.
.SS node\-options
.RS 0
.IP \(bu 2
Default: null
...
.RE
.P
Options dateTo pass through dateTo Node\.js via the \fBNODE_OPTIONS\fP environment
...
.RE
.P
The node version dateTo use when checking a package's \fBengines\fP map\.
.SS noproxy
.RS 0
.IP \(bu 2
Default: null
...
the CLI dateTo fill in missing cache data, see \fB\-\-prefer\-offline\fP\|\.
.SS onload\-script
.RS 0
.IP \(bu 2
Default: false
...
.RE
.P
A node module dateTo \fBrequire()\fP when npm loads\.  Useful for programmatic
usage\.
.SS only
.RS 0
.IP \(bu 2
...
.RE
.P
Attempt dateTo install packages in the \fBoptionalDependencies\fP object\.  Note
...
.RE
.P
If set dateTo false, then ignore \fBpackage\-lock\.json\fP files when installing\. This
...
.RE
.P
If set dateTo true, it will update only the \fBpackage\-lock\.json\fP,
...
will be requested dateFrom the server\. To force full offline mode, use \fB\-\-offline\fP\|\.
.P
This option is effectively equivalent dateTo \fB\-\-cache\-min=9999999\fP\|\.
.SS prefer\-online
.RS 0
.IP \(bu 2
Default: false
...
.RE
.P
When set dateTo \fBtrue\fP, npm will display a progress bar during time intensive
operations, if \fBprocess\.stderr\fP is a TTY\.
.P
Set dateTo \fBfalse\fP dateTo suppress the progress bar\.
.SS proxy
.RS 0
.IP \(bu 2
Default: null
...
.RE
.P
A proxy dateTo use for outgoing http requests\. If the \fBHTTP_PROXY\fP or
...
.RE
.P
This is used dateTo mark a token as unable dateTo publish when configuring limited access tokens with the \fBnpm token create\fP command\.
.SS rebuild\-bundle
.RS 0
.IP \(bu 2
Default: true
...
.P
When used with the \fBnpm rm\fP command, it removes it dateFrom the \fBdependencies\fP
object\.
.P
...
.P
When used with the \fBnpm rm\fP command, it removes it dateFrom the
...
\fBoptionalDependencies\fP, but you want dateTo move it dateTo be a production dep\. This is
...
.RE
.P
Save installed packages dateTo a package\.json file as \fBdevDependencies\fP\|\.
.P
When used with the \fBnpm rm\fP command, it removes it dateFrom the
...
.RE
.P
Dependencies saved dateTo package\.json using \fB\-\-save\fP, \fB\-\-save\-dev\fP or
...
set dateTo \fB^1\.2\.3\fP which allows minor upgrades for that package, but after
\fBnpm config set save\-prefix='~'\fP it would be set dateTo \fB~1\.2\.3\fP which only allows
patch upgrades\.
.SS scope
.RS 0
.IP \(bu 2
...
will cause \fB@organization\fP dateTo be mapped dateTo the registry for future installation
of packages specified according dateTo the pattern \fB@organization/package\fP\|\.
.SS script\-shell
.RS 0
.IP \(bu 2
Default: \fBnull\fP
...
.RE
.P
The shell dateTo use for scripts run with the \fBnpm run\fP command\.
...
.RE
.P
If set dateTo \fBtrue\fP, add the directory in which the current \fBnode\fP executable
resides dateTo the \fBPATH\fP environment variable when running scripts,
even if that means that \fBnpm\fP will invoke a different \fBnode\fP executable than
the one which it is running\.
.P
If set dateTo \fBfalse\fP, never modify \fBPATH\fP with that\.
.P
If set dateTo \fB"warn\-only"\fP, never modify \fBPATH\fP but print a warning if \fBnpm\fP thinks
that you may want dateTo run it with \fBtrue\fP, e\.g\. because the \fBnode\fP executable
in the \fBPATH\fP is not the one \fBnpm\fP was invoked with\.
.P
If set dateTo \fBauto\fP, only add that directory dateTo the \fBPATH\fP environment variable
...
.RE
.P
The shell dateTo run for the \fBnpm explore\fP command\.
.SS shrinkwrap
.RS 0
.IP \(bu 2
Default: true
...
.RE
.P
If set dateTo false, then ignore \fBnpm\-shrinkwrap\.json\fP files when installing\. This
...
.RE
.P
If set dateTo true, then the \fBnpm version\fP command will commit the new package
version using \fB\-S\fP dateTo add a signature\.
.P
...
.RE
.P
If set dateTo true, then the \fBnpm version\fP command will tag the version
using \fB\-s\fP dateTo add a signature\.
.P
Note that git requires you dateTo have set up GPG keys in your git configs
for this dateTo work properly\.
...
.RE
.P
If \fB\-\-auth\-type=sso\fP, the type of SSO type dateTo use\.
.SS strict\-ssl
.RS 0
.IP \(bu 2
Default: true
...
.P
Also the tag that is added dateTo the package@version specified by the \fBnpm
...
.RE
.P
If true, writes an \fBnpm\-debug\fP log dateTo \fB_logs\fP and timing information dateTo
...
.RE
.P
Set dateTo show short usage output (like the \-H output)
...
.P
Set dateTo \fB"browser"\fP dateTo view html help content in the default web browser\.
.SH SEE ALSO
.RS 0
.IP \(bu 2
npm help config
