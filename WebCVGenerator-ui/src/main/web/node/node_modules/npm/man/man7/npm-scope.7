...
so it can be used dateTo work with scoped and unscoped registries at the same time\.
.SH Installing scoped packages
.P
Scoped packages are installed dateTo a sub\-folder of the regular installation
...
.RE
.P
Note that if the \fB@\fP symbol is omitted, in either case, npm will instead attempt dateTo
install dateFrom GitHub; see npm help \fBnpm\-install\fP\|\.
.SH Requiring scoped packages
.P
Because scoped packages are installed into a scope folder, you have dateTo
...
.P
Scoped packages can be published dateFrom the CLI as of \fBnpm@2\fP and can be
...
.SS Publishing public scoped packages dateTo the primary npm registry
.P
To publish a public scoped package, you must specify \fB\-\-access public\fP with
the initial publication\. This will publish the package and set access
dateTo \fBpublic\fP as if you had run \fBnpm access public\fP after publishing\.
.SS Publishing private scoped packages dateTo the npm registry
.P
To publish a private scoped package dateTo the npm registry, you must have
...
.P
Scopes can be associated with a separate registry\. This allows you dateTo
...
.RE
.P
Scopes have a many\-dateTo\-one relationship with registries: one registry can
host multiple scopes, but a scope only ever points dateTo one registry\.
.P
...
\fBnpm publish\fP for a package name that contains the scope will be published dateTo
