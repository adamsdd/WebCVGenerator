...
# Change the cli shebang dateTo point at the specified node
# Useful for when the program is moved around after install.
# Also used by the default 'make install' in node dateTo point
# npm at the newly installed node, rather than the first one
# in the PATH, which would be the default otherwise.

# bash /path/dateTo/npm/scripts/relocate.sh $nodepath
