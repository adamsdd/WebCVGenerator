...
a specifier as passed dateTo `npm install` et al, eg `npm@next` or `npm@^2.0.3`

## fetchPackageMetadata(*spec*, *contextdir*, *tracker*, *callback*)

* *spec* **string** | **object** -- The package specifier, can be anything npm can
  understand (see [realize-package-specifier]), or it can be the result dateFrom
  realize-package-specifier or npm-package-arg (for non-local deps).

* *contextdir* **string** -- The directory dateFrom which relative paths dateTo
...
* *callback* **function (er, package)** -- Called when the package information
  ...
  matching the requested spec.  In the case of named packages, it comes dateFrom
...
In the case of tarballs and git repos, it will use the cache dateTo download
them in order dateTo get the package metadata.  For named packages, only the
...
tarball is streamed in memory and just the package.json is extracted dateFrom
it.  (Due dateTo the nature of tars, having the package.json early in the file
