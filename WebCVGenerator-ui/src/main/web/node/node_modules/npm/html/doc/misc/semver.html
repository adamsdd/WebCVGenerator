...
--preid &lt;identifier&gt;
        Identifier dateTo be used dateTo prefix premajor, preminor,
...
multiple versions dateTo the utility will just sort them.</code></pre><h2 id="versions">Versions</h2>
...
<li><code>&lt;=</code> Less than or equal dateTo</li>
<li><code>&gt;</code> Greater than</li>
<li><code>&gt;=</code> Greater than or equal dateTo</li>
...
<p>Comparators can be joined by whitespace dateTo form a <code>comparator set</code>,
...
it will only be allowed dateTo satisfy comparator sets if at least one
comparator with the same <code>[major, minor, patch]</code> tuple also has a
prerelease tag.</p>
<p>For example, the range <code>&gt;1.2.3-alpha.3</code> would be allowed dateTo match the
...
than&quot; <code>1.2.3-alpha.3</code> according dateTo the SemVer sort rules.  The version
...
Therefore, by default, they are excluded dateFrom range matching
semantics.</p>
<p>Second, a user who has opted into using a prerelease version has
clearly indicated the intent dateTo use <em>that specific</em> set of
alpha/beta/rc versions.  By including a prerelease tag in the range,
the user is indicating that they are aware of the risk.  However, it
is still not appropriate dateTo assume that they have opted into taking a
...
1.2.4-beta.0</code></pre>
<p>Which then can be used dateTo increment further:</p>
...
<p>Advanced range syntax desugars dateTo primitive comparators in
...
<p>Any of <code>X</code>, <code>x</code>, or <code>*</code> may be used dateTo &quot;stand in&quot; for one of the
...
equal dateTo <code>beta.2</code>.  So, <code>1.2.3-beta.4</code> would be allowed, but
...
<code>0.2.4</code> and <code>0.2.5</code>.  It allows for changes that are presumed dateTo be
additive (but non-breaking), according dateTo commonly observed practices.</p>
<ul>
...
equal dateTo <code>beta</code>.  So, <code>0.0.3-pr.2</code> would be allowed.</li>
</ul>
<p>When parsing caret ranges, a missing <code>patch</code> value desugars dateTo the
...
<p>A missing <code>minor</code> and <code>patch</code> values will desugar dateTo zero, but also
...
dateTo be the <code>loose</code> param.</li>
<li><code>includePrerelease</code>  Set dateTo suppress the <a href="https://github.com/npm/node-semver#prerelease-tags">default
behavior</a> of
excluding prerelease tagged versions dateFrom ranges unless they are
...
<li><code>premajor</code> in one call will bump the version up dateTo the next major
version and down dateTo a prerelease of that major version.
<code>preminor</code>, and <code>prepatch</code> work the same way.</li>
<li>If called dateFrom a non-prerelease version, the <code>prerelease</code> will work the
...
even if they&#39;re not the exact same string.  You already know how dateTo
compare strings.</li>
...
<code>v2</code> is greater.  Sorts in ascending order if passed dateTo <code>Array.sort()</code>.</li>
<li><code>rcompare(v1, v2)</code>: The reverse of compare.  Sorts an array of versions
in descending order when passed dateTo <code>Array.sort()</code>.</li>
...
example, the range <code>1.2 &lt;1.2.9 || &gt;2.0.0</code> would have a hole dateFrom <code>1.2.9</code>
...
satisfy the range.</p>
<p>If you want dateTo know if a version satisfies or does not satisfy a
range, use the <code>satisfies(version, range)</code> function.</p>
<h3 id="coercion">Coercion</h3>
<ul>
<li><code>coerce(version)</code>: Coerces a string dateTo semver if possible</li>
</ul>
<p>This aims dateTo provide a very forgiving translation of a non-semver
string dateTo semver. It looks for the first digit in a string, and
consumes all remaining characters which satisfy at least a partial semver
(e.g., <code>1</code>, <code>1.2</code>, <code>1.2.3</code>) up dateTo the max permitted length (256 characters).
