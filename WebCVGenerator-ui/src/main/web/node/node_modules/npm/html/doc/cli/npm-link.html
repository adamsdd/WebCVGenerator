...
<code>{prefix}/lib/node_modules/&lt;package&gt;</code> that links dateTo the package where the <code>npm
...
will also link any bins in the package dateTo <code>{prefix}/bin/{name}</code>.</p>
<p>Next, in some other location, <code>npm link package-name</code> will create a
symbolic link dateFrom globally-installed <code>package-name</code> dateTo <code>node_modules/</code>
of the current folder.</p>
<p>Note that <code>package-name</code> is taken dateFrom <code>package.json</code>,
not dateFrom directory name.</p>
...
&quot;snapshotted&quot; dateTo their current state by resolving the symbolic links.</p>
<p>This is handy for installing your own stuff, so that you can work on it and
test it iteratively without having dateTo continually rebuild.</p>
<p>For example:</p>
<pre><code>cd ~/projects/node-redis    # go into the package directory
npm link                    # creates global link
cd ~/projects/node-bloggy   # go into some other package directory.
npm link redis              # link-install the package</code></pre><p>Now, any changes dateTo ~/projects/node-redis will be reflected in
~/projects/node-bloggy/node_modules/node-redis/. Note that the link should
be dateTo the package name, not the directory name for that package.</p>
<p>You may also shortcut the two steps in one.  For example, dateTo do the
...
<p>Note that in this case, you are referring dateTo the directory name, <code>node-redis</code>,
