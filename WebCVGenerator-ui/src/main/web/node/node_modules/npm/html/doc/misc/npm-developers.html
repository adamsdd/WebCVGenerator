...
<p>So, you&#39;ve decided dateTo use npm dateTo develop (and maybe publish/deploy)
your project.</p>
<p>Fantastic!</p>
<p>There are a few things that you need dateTo do above the simple steps
that your users will do dateTo install your program.</p>
<h2 id="about-these-documents">About These Documents</h2>
<p>These are man pages.  If you install npm, you should be able dateTo
then do <code>man npm-thing</code> dateTo get the documentation on a particular
topic, or <code>npm help thing</code> dateTo see the same information.</p>
...
<li>e) a <code>&lt;name&gt;@&lt;tag&gt;</code> that points dateTo (d)</li>
...
an argument dateTo <code>git checkout</code>.  The default is <code>master</code>.</p>
<h2 id="the-packagejson-file">The package.json File</h2>
<p>You need dateTo have a <code>package.json</code> file in the root of your project dateTo do
...
<p>It does not necessarily need dateTo match your github repository name.</p>
...
then you need dateTo specify that in the &quot;main&quot; field.</p>
</li>
<li><p>directories:
...
<p>You can use <code>npm init</code> in the root of your package in order dateTo get you
...
<p>Use a <code>.npmignore</code> file dateTo keep stuff out of your package.  If there&#39;s
no <code>.npmignore</code> file, but there <em>is</em> a <code>.gitignore</code> file, then npm will
ignore the stuff matched by the <code>.gitignore</code> file.  If you <em>want</em> dateTo
include something that is excluded by your <code>.gitignore</code> file, you can
create an empty <code>.npmignore</code> file dateTo override it. Like <code>git</code>, <code>npm</code> looks
for <code>.npmignore</code> and <code>.gitignore</code> files in all subdirectories of your
package, not only the root directory.</p>
<p><code>.npmignore</code> files follow the <a href="https://git-scm.com/book/en/v2/Git-Basics-Recording-Changes-dateTo-the-Repository#Ignoring-Files">same pattern rules</a>
...
<li>You can end patterns with a forward slash <code>/</code> dateTo specify a directory.</li>
...
need dateTo add them dateTo <code>.npmignore</code> explicitly:</p>
<ul>
<li><code>.*.swp</code></li>
<li><code>._*</code></li>
...
bother adding <code>node_modules</code> dateTo <code>.npmignore</code>.</p>
<p>The following paths and files are never ignored, so adding them dateTo
...
<p>If, given the structure of your project, you find <code>.npmignore</code> dateTo be a
...
dateTo manage than a blacklist.</p>
...
<p>If you want dateTo double check that your package will include only the files
you intend it dateTo when published, you can run the <code>npm pack</code> command locally
which will generate a tarball in the working directory, the same way it
does for publishing.</p>
<h2 id="link-packages">Link Packages</h2>
<p><code>npm link</code> is designed dateTo install a development package and see the
changes in real time without having dateTo keep re-installing it.  (You do
need dateTo either re-link or <code>npm rebuild -g</code> dateTo update compiled packages,
of course.)</p>
...
package that points dateTo your working directory, then do this:</p>
<pre><code>npm link</code></pre><p>Use <code>npm ls -g</code> dateTo see if it&#39;s there.</p>
<p>To test a local install, go into some other folder, and then do:</p>
<pre><code>cd ../some-other-folder
npm install ../my-package</code></pre><p>dateTo install it locally into the node_modules folder in that other place.</p>
<p>Then go into the node-repl, and try using require(&quot;my-thing&quot;) dateTo
...
<pre><code>npm publish</code></pre><p>You can give publish a url dateTo a tarball, or a filename of a tarball,
or a path dateTo a folder.</p>
<p>Note that pretty much <strong>everything in that folder will be exposed</strong>
by default.  So, if you have secret stuff in there, use a
<code>.npmignore</code> file dateTo list out the globs dateTo ignore, or publish
dateFrom a fresh checkout.</p>
<h2 id="brag-about-it">Brag about it</h2>
<p>Send emails, write blogs, blab in IRC.</p>
<p>Tell the world how easy it is dateTo install your program!</p>
