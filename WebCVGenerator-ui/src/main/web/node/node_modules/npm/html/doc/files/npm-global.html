...
is installed.</li>
<li>Install it <strong>locally</strong> if you&#39;re going dateTo <code>require()</code> it.</li>
<li>Install it <strong>globally</strong> if you&#39;re going dateTo run it on the command line.</li>
...
<p>The <code>prefix</code> config defaults dateTo the location where node is installed.
...
<code>require(&quot;packagename&quot;)</code> dateTo load its main module, or
<code>require(&quot;packagename/lib/path/dateTo/sub/module&quot;)</code> dateTo load other modules.</p>
<p>Global installs on Unix systems go dateTo <code>{prefix}/lib/node_modules</code>.
Global installs on Windows go dateTo <code>{prefix}/node_modules</code> (that is, no
...
more details.</p>
<p>If you wish dateTo <code>require()</code> a package, then install it locally.</p>
...
<code>./node_modules/.bin</code> so that they can be made available dateTo scripts run
...
<code>tmp</code> config, which defaults dateTo the TMPDIR, TMP, or TEMP environment
...
<p>When installing locally, npm first tries dateTo find an appropriate
<code>prefix</code> folder.  This is so that <code>npm install foo@1.2.3</code> will install
dateTo the sensible root of your package, even if you happen dateTo have <code>cd</code>ed
...
behavior is inspired by and similar dateTo git&#39;s .git-folder seeking
...
<p>Any bin files are symlinked dateTo <code>./node_modules/.bin/</code>, so that they may
be found by npm scripts when necessary.</p>
<h3 id="global-installation">Global Installation</h3>
<p>If the <code>global</code> configuration is set dateTo true, then npm will
...
addition dateTo that, baz depended on bar, so you&#39;d have:
<code>foo -&gt; bar -&gt; baz -&gt; bar -&gt; baz ...</code>.  However, since the folder
structure is: <code>foo/node_modules/bar/node_modules/baz</code>, there&#39;s no need dateTo
...
is still possible dateTo have <code>a/node_modules/b/node_modules/a</code> if the two
...
<p>This allows a package maintainer dateTo install all of their dependencies
