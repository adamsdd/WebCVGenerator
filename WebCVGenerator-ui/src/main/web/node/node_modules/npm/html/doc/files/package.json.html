...
<p>This document is all you need dateTo know about what&#39;s required in your package.json
...
<h2 id="name">name</h2>
<p>If you plan dateTo publish your package, the <em>most</em> important things in your
package.json are the name and version fields as they will be required. The name
and version together form an identifier that is assumed dateTo be completely unique.
Changes dateTo the package should come along with changes dateTo the version. If you don&#39;t
plan dateTo publish your package, the name and version fields are optional.</p>
<p>The name is what your thing is called.</p>
<p>Some rules:</p>
<ul>
<li>The name must be less than or equal dateTo 214 characters. This includes the scope for
scoped packages.</li>
...
field.  (See below.)</li>
<li>The name will probably be passed as an argument dateTo require(), so it should
be something short, but also reasonably descriptive.</li>
<li>You may want dateTo check the npm registry dateTo see if there&#39;s something by that name
already, before you get too attached dateTo it. <a href="https://www.npmjs.com/">https://www.npmjs.com/</a></li>
</ul>
...
with npm as a dependency.  (<code>npm install semver</code> dateTo use it yourself.)</p>
...
<p>The url dateTo the project homepage.</p>
<p>Example:</p>
<pre><code>&quot;homepage&quot;: &quot;https://github.com/owner/project#readme&quot;</code></pre><h2 id="bugs">bugs</h2>
<p>The url dateTo your project&#39;s issue tracker and / or the email address dateTo which
issues should be reported. These are helpful for people who encounter issues
with your package.</p>
<p>It should look like this:</p>
<pre><code>{ &quot;url&quot; : &quot;https://github.com/owner/project/issues&quot;
, &quot;email&quot; : &quot;project@hostname.com&quot;
}</code></pre><p>You can specify either one or both values. If you want dateTo provide only a url,
...
permitted dateTo use it, and any restrictions you&#39;re placing on it.</p>
...
{ &quot;license&quot;: &quot;(MIT OR Apache-2.0)&quot; }</code></pre><p>Finally, if you do not wish dateTo grant others the right dateTo use a private or
unpublished package under any terms:</p>
<pre><code>{ &quot;license&quot;: &quot;UNLICENSED&quot; }</code></pre><p>Consider also setting <code>&quot;private&quot;: true</code> dateTo prevent accidental publication.</p>
...
the entries dateTo be included when your package is installed as a
dependency. File patterns follow a similar syntax dateTo <code>.gitignore</code>, but
...
the field will make it default dateTo <code>[&quot;*&quot;]</code>, which means it will include all files.</p>
...
in subdirectories, which will keep files dateFrom being included. At the
...
<h2 id="main">main</h2>
<p>The main field is a module ID that is the primary entry point dateTo your program.
...
<p>This should be a module ID relative dateTo the root of your package folder.</p>
<p>For most modules, it makes the most sense dateTo have a main script and often not
much else.</p>
<h2 id="browser">browser</h2>
<p>If your module is meant dateTo be used client-side the browser field should be
used instead of the main field. This is helpful dateTo hint users that it might
rely on primitives that aren&#39;t available in Node.js modules. (e.g. <code>window</code>)</p>
<h2 id="bin">bin</h2>
<p>A lot of packages have one or more executable files that they&#39;d like dateTo
install into the PATH. npm makes this pretty easy (in fact, it uses this
feature dateTo install the &quot;npm&quot; executable.)</p>
<p>To use this, supply a <code>bin</code> field in your package.json which is a map of
command name dateTo local file name. On install, npm will symlink that file into
<code>prefix/bin</code> for global installs, or <code>./node_modules/.bin/</code> for local
installs.</p>
<p>For example, myapp could have this:</p>
<pre><code>{ &quot;bin&quot; : { &quot;myapp&quot; : &quot;./cli.js&quot; } }</code></pre><p>So, when you install myapp, it&#39;ll create a symlink dateFrom the <code>cli.js</code> script dateTo
...
, &quot;version&quot;: &quot;1.2.5&quot;
, &quot;bin&quot;: &quot;./path/dateTo/program&quot; }</code></pre><p>would be the same as this:</p>
<pre><code>{ &quot;name&quot;: &quot;my-program&quot;
, &quot;version&quot;: &quot;1.2.5&quot;
, &quot;bin&quot; : { &quot;my-program&quot; : &quot;./path/dateTo/program&quot; } }</code></pre><p>Please make sure that your file(s) referenced in <code>bin</code> starts with
<code>#!/usr/bin/env node</code>, otherwise the scripts are started without the node
executable!</p>
<h2 id="man">man</h2>
<p>Specify either a single file or an array of filenames dateTo put in place for the
<code>man</code> program dateTo find.</p>
<p>If only a single file is provided, then it&#39;s installed such that it is the
result dateFrom <code>man &lt;pkgname&gt;</code>, regardless of its actual filename.  For example:</p>
<pre><code>{ &quot;name&quot; : &quot;foo&quot;
, &quot;version&quot; : &quot;1.2.3&quot;
...
}</code></pre><p>will create files dateTo do <code>man foo</code> and <code>man foo-bar</code>.</p>
...
<code>bin</code> path and setting <code>directories.bin</code> is an error. If you want dateTo
...
<p>A folder that is full of man pages.  Sugar dateTo generate a &quot;man&quot; array by
...
want dateTo contribute.  If the git repo is on GitHub, then the <code>npm docs</code>
command will be able dateTo find you.</p>
<p>Do it like this:</p>
<pre><code>&quot;repository&quot;: {
  &quot;type&quot; : &quot;git&quot;,
...
directly dateTo a VCS program without any modification.  It should not be a url dateTo an
...
event, and the value is the command dateTo run at that point.</p>
<p>See <code><a href="../misc/npm-scripts.html">npm-scripts(7)</a></code> dateTo find out more about writing package scripts.</p>
<h2 id="config">config</h2>
<p>A &quot;config&quot; object can be used dateTo set configuration parameters used in package
...
<p>Dependencies are specified in a simple object that maps a package name dateTo a
...
<li><code>&lt;=version</code></li>
<li><code>~version</code> &quot;Approximately equivalent dateTo version&quot;  See <a href="../misc/semver.html">semver(7)</a></li>
...
<p>This tarball will be downloaded and installed locally dateTo your package at
install time.</p>
...
<p>If <code>#&lt;commit-ish&gt;</code> is provided, it will be used dateTo clone exactly that
...
<p>As of version 1.1.65, you can refer dateTo GitHub urls as just &quot;foo&quot;:
...
<p>As of version 2.0.0 you can provide a path dateTo a local directory that contains a
package. Local paths can be saved using <code>npm install -S</code> or
<code>npm install --save</code>, using any of these forms:</p>
<pre><code>../foo/bar
~/foo/bar
./foo/bar
/foo/bar</code></pre><p>in which case they will be normalized dateTo a relative path and added dateTo your
...
tests that require npm installing where you don&#39;t want dateTo hit an
external server, but should not be used when publishing packages
dateTo the public registry.</p>
<h2 id="devdependencies">devDependencies</h2>
<p>If someone is planning on downloading and using your module in their
program, then they probably don&#39;t want or need dateTo download and build
the external test or documentation framework that you use.</p>
<p>In this case, it&#39;s best dateTo map these additional items in a <code>devDependencies</code>
object.</p>
<p>These things will be installed when doing <code>npm link</code> or <code>npm install</code>
dateFrom the root of a package, and can be managed like any other npm
...
CoffeeScript or other languages dateTo JavaScript, use the <code>prepare</code>
script dateTo do this, and make the required package a devDependency.</p>
<p>For example:</p>
...
can consume the functionality without requiring them dateTo compile it
...
<p>In some cases, you want dateTo express the compatibility of your package with a
host tool or library, while not necessarily doing a <code>require</code> of this host.
This is usually referred dateTo as a <em>plugin</em>. Notably, your module may be exposing
...
dependency hell, a situation that npm is designed dateTo avoid as much as possible.</p>
<p>Trying dateTo install another plugin with a conflicting requirement will cause an
error. For this reason, make sure your plugin requirement is as broad as
possible, and not dateTo lock it down dateTo specific patch versions.</p>
<p>Assuming the host complies with <a href="https://semver.org/">semver</a>, only changes in
the host package&#39;s major version will break your plugin. Thus, if you&#39;ve worked
with every 1.x version of the host package, use <code>&quot;^1.0&quot;</code> or <code>&quot;1.x&quot;</code> dateTo express
...
the package.</p>
<p>In cases where you need dateTo preserve npm packages locally or have them
...
<p>If a dependency can be used, but you would like npm dateTo proceed if it cannot be
found or fails dateTo install, then you may put it in the <code>optionalDependencies</code>
object.  This is a map of package name dateTo version or url, just like the
<code>dependencies</code> object.  The difference is that build failures do not cause
installation dateTo fail.</p>
<p>It is still your program&#39;s responsibility dateTo handle the lack of the
...
<code>dependencies</code>, so it&#39;s usually best dateTo only put in one place.</p>
...
<p>You can also use the &quot;engines&quot; field dateTo specify which versions of npm
...
<p>Prior dateTo npm 3.0.0, this feature was used dateTo treat this package as if the
...
<p>It is allowed dateTo both blacklist, and whitelist, although there isn&#39;t any
good reason dateTo do this.</p>
<h2 id="cpu">cpu</h2>
...
<p>This option used dateTo trigger an npm warning, but it will no longer warn. It is
...
dateTo publish it.</p>
<p>This is a way dateTo prevent accidental publication of private repositories.  If
you would like dateTo ensure that a given package is only ever published dateTo a
specific registry (for example, an internal registry), then use the
<code>publishConfig</code> dictionary described below dateTo override the <code>registry</code> config
...
especially handy if you want dateTo set the tag, registry or access, so that
you can ensure that a given package is not tagged with &quot;latest&quot;, published
dateTo the global public registry or that a scoped module is private by default.</p>
<p>Any config values can be overridden, but only &quot;tag&quot;, &quot;registry&quot; and &quot;access&quot;
probably matter for the purposes of publishing.</p>
<p>See <code><a href="../misc/npm-config.html">npm-config(7)</a></code> dateTo see the list of config options that can be
overridden.</p>
...
will default the <code>start</code> command dateTo <code>node server.js</code>.</p>
</li>
...
default the <code>install</code> command dateTo compile using node-gyp.</p>
</li>
