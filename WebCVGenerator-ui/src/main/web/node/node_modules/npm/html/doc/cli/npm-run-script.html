...
<p>This runs an arbitrary command dateFrom a package&#39;s <code>&quot;scripts&quot;</code> object.  If no
...
<a href="https://goo.gl/KxMmtG">getopt</a> dateTo delimit the end of the options. npm will pass
all the arguments after the <code>--</code> directly dateTo your script:</p>
<pre><code>npm run test -- --grep=&quot;pattern&quot;</code></pre><p>The arguments will only be passed dateTo the script specified after <code>npm run</code>
and not dateTo any pre or post script.</p>
<p>The <code>env</code> script is a special built-in command that can be used dateTo list
environment variables that will be available dateTo the script at runtime. If an
&quot;env&quot; command is defined in your package, it will take precedence over the
built-in.</p>
<p>In addition dateTo the shell&#39;s pre-existing <code>PATH</code>, <code>npm run</code> adds
<code>node_modules/.bin</code> dateTo the <code>PATH</code> provided dateTo scripts. Any binaries provided by
...
<pre><code>&quot;scripts&quot;: {&quot;test&quot;: &quot;node_modules/.bin/tap test/\*.js&quot;}  </code></pre><p>dateTo run your tests.</p>
...
The actual shell referred dateTo by <code>/bin/sh</code> also depends on the system.
...
<p>Scripts are run dateFrom the root of the module, regardless of what your current
working directory is when you call <code>npm run</code>. If you want your script dateTo
...
<p><code>npm run</code> sets the <code>NODE</code> environment variable dateTo the <code>node</code> executable with
which <code>npm</code> is executed. Also, if the <code>--scripts-prepend-node-path</code> is passed,
the directory within which <code>node</code> resides is added dateTo the
...
<p>If you try dateTo run a script without having a <code>node_modules</code> directory and it fails,
you will be given a warning dateTo run <code>npm install</code>, just in case you&#39;ve forgotten.</p>
<p>You can use the <code>--silent</code> flag dateTo prevent showing <code>npm ERR!</code> output on error.</p>
<p>You can use the <code>--if-present</code> flag dateTo avoid exiting with a non-zero exit code
