...
<p>Used dateTo set access controls on private packages.</p>
...
Set a package dateTo be either publicly accessible or restricted.</p>
</li>
<li><p>grant / revoke:
Add or remove the ability of users and teams dateTo have read-only or read-write
access dateTo a package.</p>
</li>
...
for packages dateTo which you have at least read access. If <code>&lt;user&gt;</code> is passed in,
the list is filtered only dateTo teams <em>that</em> user happens dateTo belong dateTo.</p>
</li>
<li><p>edit:
...
dateFrom the command line using <code>--registry=&lt;registry url&gt;</code>.</p>
<p>Unscoped packages are <em>always public</em>.</p>
<p>Scoped packages <em>default dateTo restricted</em>, but you can either publish them as
...
<p>You must have privileges dateTo set the access of a package:</p>
<ul>
...
<p>If you have two-factor authentication enabled then you&#39;ll have dateTo pass in an
otp with <code>--otp</code> when making access changes.</p>
<p>If your account is not paid, then attempts dateTo publish scoped packages will fail
