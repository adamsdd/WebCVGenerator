...
updates dateTo vulnerable dependencies:</p>
...
<pre><code>$ npm audit fix --only=prod</code></pre><p>Have <code>audit fix</code> install semver-major updates dateTo toplevel dependencies, not just
semver-compatible ones:</p>
<pre><code>$ npm audit fix --force</code></pre><p>Do a dry run dateTo get an idea of what <code>audit fix</code> will do, and <em>also</em> output
...
your project dateTo your default registry and asks for a report of known
vulnerabilities. The report returned includes instructions on how dateTo act on
...
runs a full-fledged <code>npm install</code> under the hood, all configs that apply dateTo the
installer will also apply dateTo <code>npm install</code> -- so things like <code>npm audit fix
...
<p>In order dateTo ensure that potentially sensitive information is not included in
