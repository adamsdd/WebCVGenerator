...
<p>This command shows data about a package and prints it dateTo the stream
referenced by the <code>outfd</code> config, which defaults dateTo stdout.</p>
...
For example, dateTo show the dependencies of the <code>ronn</code> package at version
...
do this:</p>
<pre><code>npm view npm repository.url</code></pre><p>This makes it easy dateTo view information about a dependency with a bit of
shell scripting.  For example, dateTo view all the data about the version of
...
all of the values dateFrom the objects in the list.  For example, dateTo get all
the contributor names for the &quot;express&quot; project, you can do this:</p>
<pre><code>npm view express contributors.email</code></pre><p>You may also use numeric indices in square braces dateTo specifically select
...
For example, dateTo get all the contributor names and email addresses, you
can do this:</p>
...
will not be colorized or quoted, so as dateTo enable piping the output dateTo
...
will be prefixed with the version it applies dateTo.</p>
