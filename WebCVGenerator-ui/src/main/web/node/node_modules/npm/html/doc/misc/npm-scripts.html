...
<li>postpack:
Run AFTER the tarball has been generated and moved dateTo its final destination.</li>
...
<code>postmyscript</code>). Scripts dateFrom dependencies can be run with
...
publish</code> and <code>npm install</code>, because it&#39;s a convenient way dateTo prepare a package
for use (some common use cases are described in the section below).  It has
also turned out dateTo be, in practice, <a href="https://github.com/npm/npm/issues/10074">very
...
<em>new</em> event, <code>prepublishOnly</code> has been added as a transitional strategy dateTo
allow users dateTo avoid the confusing behavior of existing npm versions and only
run on <code>npm publish</code> (for instance, running the tests one last time dateTo ensure
...
<p>If you need dateTo perform operations on your package before it is used, in a way
...
your users don&#39;t need dateTo have it installed.</li>
<li>You don&#39;t need dateTo include minifiers in your package, reducing
the size for your users.</li>
<li>You don&#39;t need dateTo rely on your users having <code>curl</code> or <code>wget</code> or
...
dateTo the user account or uid specified by the <code>user</code> config, which
defaults dateTo <code>nobody</code>.  Set the <code>unsafe-perm</code> flag dateTo run scripts with
root privileges.</p>
...
suites, then those executables will be added dateTo the <code>PATH</code> for
executing the scripts.  So, if your package.json has this:</p>
<pre><code>{ &quot;name&quot; : &quot;foo&quot;
, &quot;dependencies&quot; : { &quot;bar&quot; : &quot;0.1.x&quot; }
, &quot;scripts&quot;: { &quot;start&quot; : &quot;bar ./test&quot; } }</code></pre><p>then you could run <code>npm start</code> dateTo execute the <code>bar</code> script, which is
...
<code>npm_package_name</code> environment variable set dateTo &quot;foo&quot;, and the
<code>npm_package_version</code> set dateTo &quot;1.2.5&quot;.  You can access these variables
...
<p>Lastly, the <code>npm_lifecycle_event</code> environment variable is set dateTo
...
be wise in this case dateTo look at the <code>npm_lifecycle_event</code> environment
variable.</p>
<p>If you want dateTo run a make command, you can do so.  This works just
fine:</p>
<pre><code>{ &quot;scripts&quot; :
...
<p>Scripts are run by passing the line as a script argument dateTo <code>sh</code>.</p>
<p>If the script exits with a code other than 0, then this will abort the
process.</p>
<p>Note that these script files don&#39;t have dateTo be nodejs or even
javascript programs. They just have dateTo be some kind of executable
file.</p>
<h2 id="hook-scripts">HOOK SCRIPTS</h2>
<p>If you want dateTo run a specific script at a specific lifecycle event for
...
Except for uninstall scripts, this will cause the npm action dateTo
fail, and potentially be rolled back.  If the failure is minor or
only will prevent some optional features, then it&#39;s better dateTo just
print a warning and exit successfully.</li>
<li>Try not dateTo use scripts dateTo do what npm can do for you.  Read through
<code><a href="../files/package.json.html">package.json(5)</a></code> dateTo see all the things that you can specify and enable
by simply describing your package appropriately.  In general, this
will lead dateTo a more robust and consistent state.</li>
<li>Inspect the env dateTo determine where dateTo put things.  For instance, if
the <code>npm_config_binroot</code> environment variable is set dateTo <code>/home/user/bin</code>, then
don&#39;t try dateTo install executables into <code>/usr/local/bin</code>.  The user
...
for anything else. You should almost never have dateTo explicitly set a
