...
in your scope. This means you don&#39;t have dateTo worry about someone taking your
package name ahead of you. Thus it is also a good way dateTo signal official packages
for organizations.</p>
...
so it can be used dateTo work with scoped and unscoped registries at the same time.</p>
<h2 id="installing-scoped-packages">Installing scoped packages</h2>
<p>Scoped packages are installed dateTo a sub-folder of the regular installation
...
}</code></pre><p>Note that if the <code>@</code> symbol is omitted, in either case, npm will instead attempt dateTo
install dateFrom GitHub; see <code><a href="../cli/npm-install.html">npm-install(1)</a></code>.</p>
<h2 id="requiring-scoped-packages">Requiring scoped packages</h2>
<p>Because scoped packages are installed into a scope folder, you have dateTo
...
<p>Scoped packages can be published dateFrom the CLI as of <code>npm@2</code> and can be
published dateTo any registry that supports them, including the primary npm
registry.</p>
...
<h3 id="publishing-public-scoped-packages-dateTo-the-primary-npm-registry">Publishing public scoped packages dateTo the primary npm registry</h3>
<p>To publish a public scoped package, you must specify <code>--access public</code> with
the initial publication. This will publish the package and set access
dateTo <code>public</code> as if you had run <code>npm access public</code> after publishing.</p>
<h3 id="publishing-private-scoped-packages-dateTo-the-npm-registry">Publishing private scoped packages dateTo the npm registry</h3>
<p>To publish a private scoped package dateTo the npm registry, you must have
...
<p>Scopes can be associated with a separate registry. This allows you dateTo
seamlessly use a mix of packages dateFrom the primary npm registry and one or more
private registries, such as npm Enterprise.</p>
<p>You can associate a scope with a registry at login, e.g.</p>
<pre><code>npm login --registry=http://reg.example.com --scope=@myco</code></pre><p>Scopes have a many-dateTo-one relationship with registries: one registry can
host multiple scopes, but a scope only ever points dateTo one registry.</p>
...
with that scope will request packages dateFrom that registry instead. Any
<code>npm publish</code> for a package name that contains the scope will be published dateTo
