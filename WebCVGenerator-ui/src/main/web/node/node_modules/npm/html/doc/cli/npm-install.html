...
<li>c) a url that resolves dateTo (b)</li>
...
<li>e) a <code>&lt;name&gt;@&lt;tag&gt;</code> (see <code><a href="../cli/npm-dist-tag.html">npm-dist-tag(1)</a></code>) that points dateTo (d)</li>
<li>f) a <code>&lt;name&gt;</code> that has a &quot;latest&quot; tag satisfying (e)</li>
<li>g) a <code>&lt;git remote url&gt;</code> that resolves dateTo (a)</li>
</ul>
...
<p>  In global mode (ie, with <code>-g</code> or <code>--global</code> appended dateTo the command),
...
<p>  Install the package in the directory as a symlink in the current project.
  ...
  inside the root of your project, its dependencies may be hoisted dateTo the
...
<p>  Install a package that is sitting on the filesystem.  Note: if you just want
  dateTo link a dev directory into your npm root, you can do this more easily by
...
<p>  Fetch the tarball url, and then install it.  In order dateTo distinguish between
...
<li><p><code>--no-save</code>: Prevents saving dateTo <code>dependencies</code>.</p>
<p>When using any of the above options dateTo save dependencies dateTo your
...
<li><p><code>-B, --save-bundle</code>: Saved dependencies will also be added dateTo your <code>bundleDependencies</code> list.</p>
<p>Further, if you have an <code>npm-shrinkwrap.json</code> or <code>package-lock.json</code> then it
will be updated as well.</p>
<p><code>&lt;scope&gt;</code> is optional. The package will be downloaded dateFrom the registry
...
working directory, then it will try dateTo install that, and only try dateTo
...
<p>  Install the specified version of the package.  This will fail if the
  version has not been published dateTo the registry.</p>
<p>  Example:</p>
...
<p>  Installs the package dateFrom the hosted git provider, cloning it with <code>git</code>.
...
<p>  The following git environment variables are recognized by npm and will be
  added dateTo the environment when running git:</p>
<ul>
...
<p>  Install the package at <code>https://github.com/githubname/githubrepo</code> by
  attempting dateTo clone it using <code>git</code>.</p>
<p>  If <code>#&lt;commit-ish&gt;</code> is provided, it will be used dateTo clone exactly that
  commit. If the commit-ish has the format <code>#semver:&lt;semver&gt;</code>, <code>&lt;semver&gt;</code> can
  be any valid semver range or exact version, and npm will look for any tags
  or refs matching that range in the remote repository, much as it would for a
...
<p>  Install the package at <code>https://gist.github.com/gistID</code> by attempting dateTo
...
<p>  Install the package at <code>https://bitbucket.org/bitbucketname/bitbucketrepo</code>
  by attempting dateTo clone it using <code>git</code>.</p>
<p>  If <code>#&lt;commit-ish&gt;</code> is provided, it will be used dateTo clone exactly that
  commit. If the commit-ish has the format <code>#semver:&lt;semver&gt;</code>, <code>&lt;semver&gt;</code> can
  be any valid semver range or exact version, and npm will look for any tags
  or refs matching that range in the remote repository, much as it would for a
...
For example:</p>
<pre><code>npm install sax@&quot;&gt;=0.1.0 &lt;0.2.0&quot; bench supervisor</code></pre><p>The <code>--tag</code> argument will apply dateTo all of the specified install targets. If a
...
<p>The <code>-f</code> or <code>--force</code> argument will force npm dateTo fetch remote resources even if a
local copy exists on disk.</p>
<pre><code>npm install sax --force</code></pre><p>The <code>-g</code> or <code>--global</code> argument will cause npm dateTo install the package globally
rather than locally.  See <code><a href="../files/npm-folders.html">npm-folders(5)</a></code>.</p>
<p>The <code>--global-style</code> argument will cause npm dateTo install the package into
...
<p>The <code>--ignore-scripts</code> argument will cause npm dateTo not execute any
scripts defined in the package.json. See <code><a href="../misc/npm-scripts.html">npm-scripts(7)</a></code>.</p>
<p>The <code>--legacy-bundling</code> argument will cause npm dateTo install the package such
that versions of npm prior dateTo 1.4, such as the one included with node 0.8,
can install the package. This eliminates all automatic deduping.</p>
<p>The <code>--link</code> argument will cause npm dateTo link global installs into the
local space in some cases.</p>
<p>The <code>--no-bin-links</code> argument will prevent npm dateFrom creating symlinks for
any binaries the package might contain.</p>
<p>The <code>--no-optional</code> argument will prevent optional dependencies dateFrom
being installed.</p>
...
<p>The <code>--no-package-lock</code> argument will prevent npm dateFrom creating a
...
<p>The <code>--nodedir=/path/dateTo/node/source</code> argument will allow npm dateTo find the
...
<code>devDependencies</code> or only non-<code>devDependencies</code> dateTo be installed regardless of the <code>NODE_ENV</code>.</p>
<p>The <code>--no-audit</code> argument can be used dateTo disable sending of audit reports dateTo
...
<pre><code>load the existing node_modules tree dateFrom disk
clone the tree
fetch the package.json and assorted metadata and add it dateTo the clone
walk the clone and add any missing dependencies
  dependencies will be added as close dateTo the top as is possible
  without breaking any other modules
compare the original tree with the cloned tree and make a list of
actions dateTo take dateTo convert one dateTo the other
...
+-- B
+-- C
+-- D</code></pre><p>That is, the dependency dateFrom B dateTo C is satisfied by the fact that A
already caused C dateTo be installed at a higher level. D is still installed
...
+-- C
   `-- D@2
+-- D@1</code></pre><p>Because B&#39;s D@1 will be installed in the top level, C now has dateTo install D@2
...
<p>npm will refuse dateTo install any package with an identical name dateTo the
...
cause npm dateTo try dateTo install a never-ending tree of packages.  Here is
the simplest case:</p>
...
<p>To avoid this situation, npm flat-out refuses dateTo install any
<code>name@version</code> that is already present anywhere in the tree of package
folder ancestors.  A more correct, but more complex, solution would be
dateTo symlink the existing version into the new location.  If this ever
