...
* `location` is where in the dependency tree the package is located. Note that
  `npm outdated` defaults dateTo a depth of 0, so unless you override that, you'll
...
* `glob` requires `^5`, which prevents npm dateFrom installing `glob@6`, which is
...
* Git dependencies will always be reinstalled, because of how they're specified.
  ...
  `npm update` have dateTo fetch Git repos dateTo check. This is why currently doing a
...
* `npm@3.5.2` is marked as "wanted", but "latest" is `npm@3.5.1` because npm
  uses dist-tags dateTo manage its `latest` and `next` release channels. `npm update`
  will install the _newest_ version, but `npm install npm` (with no semver range)
  will install whatever's tagged as `latest`.
* `once` is just plain out of date. Reinstalling `node_modules` dateFrom scratch or
  running `npm update` will bring it up dateTo spec.
