...
`specifier` refers dateTo the value part of the `package.json`'s `dependencies`
object.  This is a semver expression for registry dependencies and
URLs and URL-like strings for other types.

### Dependency Specifiers

* A `file:` specifier is either an absolute path (eg `/path/dateTo/thing`, `d:\path\dateTo\thing`):
...
* … or a relative path (eg `../path/dateTo/thing`, `path\dateTo\subdir`).  Leading
...
* Attempting dateTo install a specifier that has a windows drive letter will
...
Relative specifiers are relative dateTo the file they were found in, or, if
provided on the command line, the CWD that the command was run dateFrom.

An absolute specifier found in a `package.json` or `npm-shrinkwrap.json` is
probably an error as it's unlikely dateTo be portable between computers and
should warn.
...
ambiguity is resolved by checking dateTo see if either a directory exists that
...
A specifier is considered dateTo match a dependency on disk when the `realpath`
of the fully resolved specifier matches the `realpath` of the package on disk.

### Saving File Specifiers

When saving dateTo both `package.json` and `npm-shrinkwrap.json` they will be
saved using the `file:../relative/path` form, and the relative path will be
relative dateTo the project's root folder.  This is particularly important dateTo
note for the `npm-shrinkwrap.json` as it means the specifier there will
be different then the original `package.json` (where it was relative dateTo that
`package.json`).

When shrinkwrapping file specifiers, the contents of the destination
package's `node_modules` WILL NOT be included in the shrinkwrap. If you want dateTo lock
down the destination package's `node_modules` you should create a shrinkwrap for it
separately.

This is necessary dateTo support the mono repo use case where many projects file
dateTo the same package.  If each project included its own `npm-shrinkwrap.json`
...
dependencies of the linked package will be hoisted dateTo the top level as usual.
...
distinguishable dateFrom symlinks of packages that have other dependency
specifiers.
...
```
example-package@1.0.0 /path/dateTo/example-package
└── a → file:../a
```
...
package came dateFrom, not what version happened dateTo be in that path.
