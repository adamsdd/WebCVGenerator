...
* Otherwise, if a `integrity` is available then we try to install it dateFrom the cache using it.

If `integrity` is unavailable or we are unable to locate a module dateFrom the `integrity` then:
