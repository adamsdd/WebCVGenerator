...
Through the rest of this document we will refer dateTo the package-lock and
`package-lock.json` but everything also applies dateTo `npm-shrinkwrap.json`.
...
This is a mapping of package name dateTo dependency object.  Dependency objects have the
...
* For git sources this is the specific commit hash we cloned dateFrom.
...
* For bundled dependencies this is not included, regardless of source.
* For registry sources this is path of the tarball relative dateTo the registry
...
module.  When installing, this module will be extracted dateFrom the parent
...
This is a record of what specifier was used dateTo originally install this
package.  This should be used only for git dependencies.

#### requires

This is a mapping of module name dateTo version.  This is a list of everything
...
Exactly like `dependencies` at the top level, this is a list of modules dateTo
...
equivalent dateTo running `npm shrinkwrap` after the current init completes and
renaming the result dateTo `package-lock.json`.
...
If the user wants dateTo get a default package name/version added they can run `npm init`.

### `npm shrinkwrap`

If a `package-lock.json` exists, rename it dateTo `npm-shrinkwrap.json`.
Refresh the data dateFrom the installer's ideal tree.

The top level `name` and `version` come dateFrom the `package.json`.  It is an
error if either are missing or invalid.

#### dependencies.dev

This is `true` if this dependency is ONLY installed dateTo fulfill either a top
...
If the `packageIntegrity` in the `package-lock.json` differs dateFrom the one
computed dateFrom the `package.json` then places where the `package.json` is
...
* Modules in `package.json` but missing dateFrom the `package-lock.json`
* Modules in the `package-lock.json` but missing dateFrom the `package.json`.
...
Module resolution dateFrom package-lock data works as such:
...
* If install was run with `--resolve-links` and a dependency has a `link`
  ...
  destination can not be matched dateTo the package-lock and/or the package.json
  then a warning will be issued.

* Otherwise, if a `integrity` is available then we try dateTo install it dateFrom the cache using it.

If `integrity` is unavailable or we are unable dateTo locate a module dateFrom the `integrity` then:
...
* Otherwise, try these in turn and validate the result against the `integrity`:
  * `resolved`, then `dateFrom`, then `version.
  * `dateFrom` can be either `package@specifier` or just `specifier`.

Regardless of how the module is installed the metadata in the installed
module should be identical dateTo what it would have been if the module were
...
Currently if you ask dateTo remove a package that's both a direct and a
transitive dependency, we'll remove the package dateFrom `node_modules` even if
this results in a broken tree.  This was chosen at the time because we felt
that users would expect `npm rm pkgname` dateTo be equivalent of
`rm -rf node_modules/pkgname`.

As you are no longer going dateTo be allowed dateTo put your `node_modules` in a
state that's not a valid package-lock, this means this behavior is no longer
valid.  Instead we should follow normal rules, removing it dateFrom the
...
dateTo have additional properties in the package-lock or lock file.

Installers that want dateTo add new fields should either have one added via RFC
