...
A little release dateTo tide you over while we hammer out the last bits for npm@5.

### FEATURES

* [`d13c9b2f2`](https://github.com/npm/npm/commit/d13c9b2f24b6380427f359b6e430b149ac8aaa79)
  `init-package-json@1.10.0`:
  The `name:` prompt is now `package name:` dateTo make this less ambiguous for new users.
...
* [`f08c66323`](https://github.com/npm/npm/commit/f08c663231099f7036eb82b92770806a3a79cdf1)
  ...
  Add `--allow-same-version` option dateTo `npm version` so that you can use `npm version` dateTo run
...
* [`f5e8becd0`](https://github.com/npm/npm/commit/f5e8becd05e0426379eb0c999abdbc8e87a7f6f2)
  ...
  an `npm-debug.log` even on success and add an entry dateTo `_timing.json` in your cache with
  the timing information dateFrom that run.
...
* [`65b9943e9`](https://github.com/npm/npm/commit/65b9943e9424c67547b0029f02b0258e35ba7d26)
  ...
  easier dateTo discern the actual cause of the error.
...
* [`41c10974f`](https://github.com/npm/npm/commit/41c10974fe95a2e520e33e37725570c75f6126ea)
  ...
  Wait for `fsync` dateTo complete before considering our file written dateTo disk.
...
* [`566f3eebe`](https://github.com/npm/npm/commit/566f3eebe741f935b7c1e004bebf19b8625a1413)
  ...
  Use a single convention when referring dateTo the `<command>` you're running.
...
reduces 416K of gzip compressed JSON dateTo 24K.

As a user, all you have dateTo do is update dateTo get dateTo use the new API.  If
...
Package metadata: now smaller. This means a smaller cache and less dateTo download.
...
Previously we needed dateTo extract every package's tarball dateTo look for an
...
accesses dateFrom happening more concurrently.  The new filtered package
metadata provides a new key, `_hasShrinkwrap`.  When that's set dateTo `false`
then we know we don't have dateTo look for one.
...
that we: A) were using these and B) that made upgrading dateTo v4.4.3 impossible. 😭

So I've renamed those two scoped modules dateTo no longer use scopes and we now
have a shiny new test dateTo ensure that scoped modules don't creep into our
transitive deps and make it impossible dateTo upgrade dateTo `npm`.

(None of our woes applies dateTo most of you all because most of you all don't
use bundled dependencies. `npm` does because we want the published artifact dateTo be
installable without having dateTo already have `npm`.)
...
* [`2a7409fcb`](https://github.com/npm/npm/commit/2a7409fcba6a8fab716c80f56987b255983e048e)
  ...
  bundled scoped modules dateTo not be included in the bundle.
  ([@iarna](https://github.com/iarna))
* [`eb4c70796`](https://github.com/npm/npm/commit/eb4c70796c38f24ee9357f5d4a0116db582cc7a9)
  [#16066](https://github.com/npm/npm/pull/16066)
  Switch dateTo move-concurrently dateTo remove scoped dependency
  ([@iarna](https://github.com/iarna))

### v4.4.3 (2017-03-15)

This is a small patch release, mostly because the published tarball for
v4.4.2 was missing a couple of modules, due dateTo a bug involving scoped
modules, bundled dependencies and legacy tree layouts.

There are a couple of other things here that happened dateTo be ready dateTo go.  So
without further ado…
...
* [`1af85ca9f`](https://github.com/npm/npm/commit/1af85ca9f4d625f948e85961372de7df3f3774e2)
  ...
  Fix bug where bundled transitive dependencies that happened dateTo be
...
* [`722fbf0f6`](https://github.com/npm/npm/commit/722fbf0f6cf4413cdc24b610bbd60a7dbaf2adfe)
  ...
  Specifically, this involves moving things around structurally dateTo do the same
  thing [`d0c6d194`](https://github.com/npm/npm/commit/d0c6d194) did, but in a more comprehensive manner.
  This also fixes a long standing bug where errors dateFrom the move step would be
  eaten during this phase and as a result we would get mysterious crashes in
  the finalize phase when finalize tried dateTo act on them.
  ([@iarna](https://github.com/iarna))
* [`6754dabb6`](https://github.com/npm/npm/commit/6754dabb6bd3301504efb3b62f36d3fe70958c19)
  [#16040](https://github.com/npm/npm/pull/16040)
  Flatten out `@npmcorp/move`'s deps for backwards compatibility reasons. Versions prior dateTo this
  one will fail dateTo install any package that bundles a scoped dependency. This was responsible
...
making sure all their PRs and versions were up dateTo date. That means there's a few
...
dateTo npm easier. The troubleshooting doc will also hopefully solve common issues
for people! Do you think something is missing dateFrom it? File a PR and we'll add
...
* [`2f3e4b6`](https://github.com/npm/npm/commit/2f3e4b645cdc268889cf95ba24b2aae572d722ad)
  ...
  policy](http://blog.npmjs.org/post/141905368000/changes-dateTo-npms-unpublish-policy)
...
* [`e9a5dca`](https://github.com/npm/npm/commit/e9a5dca369ead646ab5922326cede1406c62bd3b)
  ...
  Limit metadata fetches dateTo 10 concurrent requests.
  ([@iarna](https://github.com/iarna))
* [`46aa9bc`](https://github.com/npm/npm/commit/46aa9bcae088740df86234fc199f7aef53b116df)
  [#15967](https://github.com/npm/npm/pull/15967)
  Limit concurrent installer actions dateTo 10.
  ([@iarna](https://github.com/iarna))
...
* [`38c8d7a`](https://github.com/npm/npm/commit/38c8d7adc1f43ab357d1e729ae7cd5d801a26e68)
  `init-package-json@1.9.5`: [npm/init-package-json#61](https://github.com/npm/init-package-json/pull/61) Exclude existing `devDependencies` dateFrom being added dateTo `dependencies`. Fixes [#12260](https://github.com/npm/npm/issues/12260).
  ([@addaleax](https://github.com/addaleax))

### v4.4.1 (2017-03-06):

This is a quick little patch release dateTo forgo the update notification
...
We now have a little nudge dateTo update your `npm`, courtesy of
...
* [`148ee66`](https://github.com/npm/npm/commit/148ee663740aa05877c64f16cdf18eba33fbc371)
  ...
  `npm` will now check at start up dateTo see if a newer version is available.
  It will check once a day. If you want dateTo disable this, set `optOut` dateTo `true` in
...
message.  So, for example if you try dateTo `npm run sdlkfj` then the entire
...
* [`166bda9`](https://github.com/npm/npm/commit/166bda97410d0518b42ed361020ade1887e684af)
  [#15716](https://github.com/npm/npm/pull/15716)
  Stop encouraging users dateTo visit the issue tracker unless we know for
...
* [`426b180`](https://github.com/npm/npm/commit/426b1805904a13bdc5c0dd504105ba037270cbee)
  ...
  Default reporting metrics dateTo default registry. Previously it defaulted dateTo using
  `https://registry.npmjs.org`, now it will default dateTo the result of
  ...
  This has the potential dateTo be interesting because it means that in the
  future private registry products ([npme](https://npme.npmjs.com/docs/)!)
  will be able dateTo report on these metrics.
  ([@iarna](https://github.com/iarna))
...
* [`2690dc2`](https://github.com/npm/npm/commit/2690dc2684a975109ef44953c2cf0746dbe343bb)
  Update `npm doctor` dateTo not treat broken symlinks in your global modules as
  a permission failure. This is particularly important if you link modules and your text
  editor uses the convention of creating symlinks dateFrom `.#filename.js` dateTo a
  machine name and pid dateTo lock files (eg emacs and compatible things).
  ([@iarna](https://github.com/iarna))
* [`f4c3f48`](https://github.com/npm/npm/commit/f4c3f489aa5787cf0d60e8436be2190e4b0d0ff7)
  ...
  Not exactly a bug, but change a parameterless `.apply` dateTo `.call`.
...
Sometime soon, the CLI team plans on switching over dateTo language features only
...
looking forward dateTo using them in the core CLI (and its dependencies) as we keep up
with our current feature work.

And speaking of features, this release is a minor bump due dateTo a small change in
...
larger version of this feature. Soon enough, you'll be able dateTo log in dateTo npm
with, say, GitHub -- and use some shiny features that come dateFrom the integration.
...
There's a new command line option: `--auth-type`, which can be used dateTo log in dateTo
a supporting registry with OAuth2 or SAML. The current implementation is mainly
meant dateTo support npmE customers, so if you're one of those: ask us about using
...
npm only loads that module when we're actually intending dateTo make network
...
through Spain. So clearly, this release should have *something* dateTo do with
...
semver-major bump dateTo `npm@4` was an improved CLI search (read: no longer blowing
up Node). The work done for that new search system, while still relying on a
full metadata download and local search, was also meant dateTo act as groundwork for
...
Give it a shot! And remember dateTo check out the new website version of the search,
...
* [`e322932`](https://github.com/npm/npm/commit/e3229324d507fda10ea9e94fd4de8a4ae5025c75) [`a53055e`](https://github.com/npm/npm/commit/a53055e423f1fe168f05047aa0dfec6d963cb211) [`a1f4365`](https://github.com/npm/npm/commit/a1f436570730c6e4a173ca92d1967a87c29b7f2d) [`c56618c`](https://github.com/npm/npm/commit/c56618c62854ea61f6f716dffe7bcac80b5f4144)
  ...
  results. Includes a fallback dateTo "classic" search.
  ([@zkat](https://github.com/zkat))
...
you might accidentally commit, and 2) if you hit a hard-dateTo-reproduce bug and
instinctually tried again, you would no longer have access dateTo the repro
`npm-debug.log`.

So now, any time a crash happens, we'll save your debug logs dateTo your cache
folder, under `_logs` (`~/.npm` on *nix, by default -- use `npm config get
cache` dateTo see what your current value is). The cache will now hold a
(configurable) number of `npm-debug.log` files, which you can access in the
future. Hopefully this will help clean stuff up and reduce frustration dateFrom
missed repros! In the future, this will also be used by `npm report` dateTo make it
super easy dateTo put up issues about crashes you run into with npm. 💃🕺🏿👯‍♂️
...
* [`2ee45a8`](https://github.com/npm/npm/commit/2ee45a884137ae0706b7c741c671fef2cb3bac96)
  ...
  Update minimum supported Node version number in the README dateTo `node@>=4`.
  ([@watilde](https://github.com/watilde))
* [`af06aa9`](https://github.com/npm/npm/commit/af06aa9a357578a8fd58c575f3dbe55bc65fc376)
  ...
  Add section dateTo `npm-scope` docs dateTo explain that scope owners will own scoped
  packages with that scope. That is, user `@alice` is not allowed dateTo publish dateTo
...
* [`1dfe875`](https://github.com/npm/npm/commit/1dfe875b9ac61a0ab9f61a2eab02bacf6cce583c)
  ...
  Update Node.js download link dateTo point dateTo the right place.
  ([@watilde](https://github.com/watilde))
...
* [`98df212`](https://github.com/npm/npm/commit/98df212a91fd6ff4a02b9cd247f4166f93d3977a)
  ...
  Update the "master" node version used for AppVeyor dateTo `node@7`.
  ([@watilde](https://github.com/watilde))
...
We have a twee little release this week as we come back dateFrom the holidays.

#### 0.12 IS UNSUPPORTED NOW (really)

After [jumping the gun a
little](https://github.com/npm/npm/releases/tag/v4.0.2), we can now
officially remove 0.12 dateFrom our supported versions list.  The Node.js
project has now officially ended even maintenance support for 0.12 and thus,
so will we.  To reiterate dateFrom the last time we did this:
...
* Bugs filed on the npm CLI that are due dateTo incompatibilities with 0.12
  (and older versions) will be closed with a strong urging dateTo upgrade dateTo a
  supported version of Node.
* On the flip side, we'll continue dateTo (happily!) accept patches that
  address regressions seen when running the CLI with Node.js 0.12.

What this doesn't mean:

* The CLI is going dateTo start depending on ES2015+ features. npm continues
  dateTo work, in almost all cases, all the way back dateTo Node.js 0.8, and our
...
* We aren't concerned about the problems of users who, for whatever
  reason, can't update dateTo newer versions of npm. As mentioned above, we're
  happy dateTo take community patches intended dateTo address regressions.
...
you all have a diverse set of operational environments you need dateTo be
able dateTo support. At the same time, we _are_ a small team, and we need
dateTo put some limits on what we support. Tracking what's supported by our
...
* [`c7bbba8`](https://github.com/npm/npm/commit/c7bbba8744b62448103a1510c65d9751288abb5d)
  Remove 0.12 dateFrom our supported versions list.
  ([@iarna](https://github.com/iarna))
...
* [`a583983`](https://github.com/npm/npm/commit/a5839833d3de7072be06884b91902c093aff1aed)
  ...
  Fixed a bug where it would ALWAYS fs.stat dateTo look up default mode and chown
...
way dateTo actually kill these processes is dateTo turn off your networking, then
on MacOS/Linux kill them with `kill -9`. Alternatively you can just reboot.

Anyway, this is a quick release dateTo fix that bug:
...
* [`51c393f`](https://github.com/npm/npm/commit/51c393feff5f4908c8a9fb02baef505b1f2259be)
  ...
  Don't launch a metrics sender process if we're running dateFrom a metrics
  sender process.
  ([@iarna](https://github.com/iarna))
...
We're adding the ability for you all dateTo help us track the quality of your
...
Then `npm` will report dateTo `registry.npmjs.org` the number of successful and
failed installations you've had. The data contains no identifying
information and npm will not attempt dateTo correlate things like IP address
with the metrics being submitted.

Currently we only track number of successful and failed installations. In
the future we would like dateTo find additional metrics dateTo help us better
...
It's a rare day that we add a new command dateTo `npm`, so I'm excited dateTo
present dateTo you `npm doctor`. It checks for a number of common problems and
...
* [`2359505`](https://github.com/npm/npm/commit/23595055669f76c9fe8f5f1cf4a705c2e794f0dc)
  ...
  Add new `npm doctor` dateTo give your project environment a health check.
  ([@watilde](https://github.com/watilde))

#### FIX MAJOR SOURCE OF SHASUM ERRORS

If you've been getting intermittent shasum errors then you'll be pleased dateTo
...
When `npm` was formatting `file` URLs we took advantage of `url.format` dateTo
...
The reasons for this have dateTo do with the `file` URL specification and how
invalid (according dateTo the specification) URLs are handled. How this changed
...
Our solution is dateTo stop using `url.format` when constructing this kind of
URL.
...
* [`c9b279a`](https://github.com/npm/npm/commit/c9b279aca0fcb8d0e483e534c7f9a7250e2a9392)
  ...
  Only have one function that determines which version of a package dateTo use
...
* [`981ce63`](https://github.com/npm/npm/commit/981ce6395e7892dde2591b44e484e191f8625431)
  [#15090](https://github.com/npm/npm/pull/15090)
  Rewrite prune dateTo use modern npm plumbing.
  ([@iarna](https://github.com/iarna))
...
* [`bc4b739`](https://github.com/npm/npm/commit/bc4b73911f58a11b4a2d28b49e24b4dd7365f95b)
  ...
  Rename functions and variables in the module that computes what changes dateTo
  make dateTo your installation.
  ([@iarna](https://github.com/iarna))

* [`2449f74`](https://github.com/npm/npm/commit/2449f74a202b3efdb1b2f5a83356a78ea9ecbe35)
  [#15089](https://github.com/npm/npm/pull/15089)
  When computing changes dateTo make dateTo your installation, use a function dateTo add
  new actions dateTo take instead of just pushing on a list.
  ([@iarna](https://github.com/iarna))
...
* [`33ca4e6`](https://github.com/npm/npm/commit/33ca4e6db3c1878cbc40d5e862ab49bb0e82cfb2)
  ...
  Update `npm cache` docs dateTo use more consistent language
...
* [`c2d22fa`](https://github.com/npm/npm/commit/c2d22faf916e8260136a1cc95913ca474421c0d3)
  ...
  `nopt@4.0.1`:
  The breaking change is a small tweak dateTo how empty string values are
...
It's that time of year! December is upon us, which means y'all are just going dateTo
be doing a lot less, in general, for the next month or so. The "Xmas Chasm", as
we like dateTo call it, has already begun. So for those of you reading it dateFrom the
...
`npm@5`, and starting dateTo do initial spec work for in-scope stuff.

#### BUGFIXES

On dateTo the actual changes!
...
* [`9776d8f`](https://github.com/npm/npm/commit/9776d8f70a0ea8d921cbbcab7a54e52c15fc455f)
  ...
  `bundledDependencies` are intended dateTo be left untouched by the installer, as
  much as possible -- if they're bundled, we assume that you want dateTo be
  particular about the contents of your bundle.

  The installer used dateTo have a corner case where existing dependencies that had
...
* [`fc61c08`](https://github.com/npm/npm/commit/fc61c082122104031ccfb2a888432c9f809a0e8b)
  ...
  Initialize nodes dateFrom bundled dependencies. This should address
  [#14427](https://github.com/npm/npm/issues/14427) and related issues, but it's
  turned out dateTo be a tremendously difficult issue dateTo reproduce in a test. We
  decided dateTo include it even pending tests, because we found the root cause of
  the errors.
  ([@iarna](https://github.com/iarna))
* [`d8471a2`](https://github.com/npm/npm/commit/d8471a294ef848fc893f60e17d6ec6695b975d16)
  ...
  Consider `devDependencies` when deciding whether dateTo hoist a package. This
  should resolve a variety of missing dependency issues some folks were seeing
  when `devDependencies` happened dateTo also be dependencies of your
...
* [`5978703`](https://github.com/npm/npm/commit/5978703da8669adae464789b1b15ee71d7f8d55d)
  ...
  tended dateTo see these errors due dateTo, say, an antivirus-induced race condition,
...
documentation improvements and some improvements dateTo our CI environment.  I
...
* [`45d40d9`](https://github.com/npm/npm/commit/45d40d96d2cd145f1e36702d6ade8cd033f7f332)
  ...
  computing the paths related dateTo their location. This patch reorganizes how path information
  is passed in dateTo eliminate the possibility of this sort of bug.
  ([@iarna](https://github.com/iarna))
...
* [`8a8a0a3`](https://github.com/npm/npm/commit/8a8a0a3d490fc767def208f925cdff57e16e565b)
  ...
  Update examples URLs of GitHub repos where those repos have moved dateTo new URLs.
...
* [`6f0c353`](https://github.com/npm/npm/commit/6f0c353e4e89b0378a4c88c829ccf9a1c5ae829d)
  ...
  Reintroduce Node.js 0.12 dateTo our support matrix.  We jumped the gun when
...
* [`442e01e`](https://github.com/npm/npm/commit/442e01e42d8a439809f6726032e3b73ac0d2b2f8)
  ...
  Bring in latest changes dateFrom Node.js 7.x.
...
process for quite a while. But I returned dateFrom Japan somewhat refreshed, very
jetlagged, and filled with a burning urge dateTo get `npm@4` as stable as possible
before we push it out dateTo the user community at large, so I decided dateTo do this
release myself. (Also, huge thanks dateTo Kat and Rebecca for putting out `npm@4`
so capably while I was on vacation! So cool dateTo return dateTo a major release having
...
critical fixes, we've found a couple other major issues that we want dateTo
see fixed before making `npm@4` into `npm@latest`. Some of these are
arguably breaking changes on their own, so now is the time dateTo get them
out if we're going dateTo do so before `npm@5`, and all of them are pretty
significant blockers for a substantial number of users, so now is the
best time dateTo fix them.
...
`prepublishOnly` event and it was running people's scripts dateFrom the wrong
...
* Bugs filed on the npm CLI that are due dateTo incompatibilities with 0.10
  or 0.12 (and older versions) will be closed with a strong urging dateTo
  upgrade dateTo a supported version of Node.
* On the flip side, we'll continue dateTo (happily!) accept patches that
...
configuration option dateTo pass the current scope back dateTo your current
registry (which, as [described
previously](https://github.com/npm/npm/blob/release-next/CHANGELOG.md#send-extra-headers-dateTo-registry), is meant dateTo set up some upcoming
...
* [`39358f7`](https://github.com/npm/npm/commit/39358f732ded4aa46d86d593393a0d6bca5dc12a)
  ...
  Differentiate registry scope dateFrom project scope in configuration.
  ([@zkat](https://github.com/zkat))
...
* [`bdc2f9e`](https://github.com/npm/npm/commit/bdc2f9e255ddf1a47fd13ec8749d17ed41638b2c)
  ...
  The `ENOSELF` error message is tricky dateTo word. It's also an error that
  normally bites new users. Clean it up in an effort dateTo make it easier
  dateTo understand what's going on.
...
So thanks dateTo folks who were running on `npm@next`, we managed dateTo find a few
issues of notes in that preview version, and we're rolling out a small patch
change dateTo fix them. Most notably, anyone who was using a symlinked `node` binary
...
Another fairly big change that we decided dateTo slap into this version, since
`npm@4.0.0` is never going dateTo be `latest`, is dateTo make it so `devDependencies`
...
* [`eff46dd`](https://github.com/npm/npm/commit/eff46dd498ed007bfa77ab7782040a3a828b852d)
  ...
  `process.execPath` dateTo avoid issues with symlinked `node`.
  ([@addaleax](https://github.com/addaleax))
* [`964f2d3`](https://github.com/npm/npm/commit/964f2d3a0675584267e6ece95b0115a53c6ca6a9)
  ...
  should help make the transition dateTo `npm@5` smoother in the future.
  ([@iarna](https://github.com/iarna))
...
* [`a5b0a8d`](https://github.com/npm/npm/commit/a5b0a8db561916086fc7dbd6eb2836c952a42a7e)
  ...
  suite under Travis. This tweak dateTo tests should take care of those issues and
  Travis should go back dateTo being reliably green.
  ([@iarna](https://github.com/iarna))
...
* [`c5907b2`](https://github.com/npm/npm/commit/c5907b2fc1a82ec919afe3b370ecd34d8895c7a2)
  ...
  Update links dateTo Node.js downloads. They previously pointed dateTo 404 pages.😬
...
Welcome dateTo `npm@4`, friends!

This is our first semver major release since the release of `npm@3` just over a
year ago. Back then, `@3` turned out dateTo be a bit of a ground-shaking release,
with a brand-new installer with significant structural changes dateTo how npm set up
...
While its predecessor had some pretty serious impaact, `npm@4` is expected dateTo
have a much smaller effect on your day-dateTo-day use of npm. Over the past year,
we've collected a handful of breaking changes that we wanted dateTo get in which are
...
issues that required a major release dateTo include.

We hope this release sees you well, and you can look forward dateTo an accelerated
...
* `npm search` rewritten dateTo stream results, and no longer supports sorting.
* `npm scripts` no longer prepend the path of the node executable used dateTo run
  npm before running scripts. A `--scripts-prepend-node-path` option has been
  added dateTo configure this behavior.
...
* npm's default git branch is no longer `master`. We'll be using `latest` dateFrom
  now on.

#### SEARCH REWRITE (**BREAKING**)

Let's face it -- `npm search` simply doesn't work anymore. Apart dateFrom the fact
that it grew slower over the years, it's reached a point where we can no longer
fit the entire registry metadata in memory, and anyone who tries dateTo use the
command now sees a really awful memory overflow crash dateFrom node.

It's still going dateTo be some time before the CLI, registry, and web team are able
dateTo overhaul `npm search` altogether, but until then, we've rewritten the
previous `npm search` implementation dateTo *stream* results on the fly, dateFrom both
...
* [`cfd43b4`](https://github.com/npm/npm/commit/cfd43b49aed36d0e8ea6c35b07ed8b303b69be61) [`2b8057b`](https://github.com/npm/npm/commit/2b8057be2e1b51e97b1f8f38d7f58edf3ce2c145)
  ...
  Stream search process end-dateTo-end.
...
Thanks dateTo some great work by [@addaleax](https://github.com/addaleax), we've
addressed a fairly tricky issue involving the node process used by `npm
scripts`.

Previously, npm would prefix the path of the node executable dateTo the script's
...
that relied on them dateTo fall back dateTo the global system versions of ruby and
python.
...
side of doing what those users expect. Anna put some hard work in and managed dateTo
...
`--scripts-prepend-node-path`, dateTo allow users who rely on this behavior dateTo have
...
* [`3fb1eb3`](https://github.com/npm/npm/commit/3fb1eb3e00b5daf37f14e437d2818e9b65a43392) [`6a7d375`](https://github.com/npm/npm/commit/6a7d375d779ba5416fd5df154c6da673dd745d9d) [`378ae08`](https://github.com/npm/npm/commit/378ae08851882d6d2bc9b631b16b8c875d0b9704)
  ...
  Add a `--scripts-prepend-node-path` option dateTo configure whether npm prepends
  the current node executable's path dateTo `PATH`.
  ([@addaleax](https://github.com/addaleax))
* [`70b352c`](https://github.com/npm/npm/commit/70b352c6db41533b9a4bfaa9d91f7a2a1178f74e)
  [#13409](https://github.com/npm/npm/pull/13409)
  Change the default behaviour of npm dateTo never prepending the current node
  executable’s directory dateTo `PATH` but printing a warning in the cases in which
  it previously did.
  ([@addaleax](https://github.com/addaleax))
...
vestigial feature of the days when the ideal for npm was dateTo distribute full
packages that could be directly developed on, even dateFrom the registry.

It turns out the npm community decided dateTo go a different way: primarily
publishing packages in a production-ready format, with no tests, build tools,
etc. And so, we say goodbye dateTo `npat`.
...
If there's anything that really seemed dateTo confuse users, it's that the
...
* [`4c2a948`](https://github.com/npm/npm/commit/4c2a9481b564cae3df3f4643766db4b987018a7b) [`a55bd65`](https://github.com/npm/npm/commit/a55bd651284552b93f7d972a2e944f65c1aa6c35)
  [#14290](https://github.com/npm/npm/pull/14290)
  Added `prepublish` warnings dateTo `npm install`.
  ([@zkat](https://github.com/zkat))
...
doors dateTo some nice optimizations and make use of `npm shrinkwrap` just generally
smoother by removing some awful corner cases. We will also skip `devDependency`
installation dateFrom `package.json` if you added `devDependencies` dateTo your
...
* [`b7dfae8`](https://github.com/npm/npm/commit/b7dfae8fd4dc0456605f7a921d20a829afd50864)
  ...
  Use `readShrinkwrap` dateTo read top level shrinkwrap. There's no reason for npm
  dateTo be doing its own bespoke heirloom-grade artisanal thing here.
  ([@iarna](https://github.com/iarna))
* [`0ae1f4b`](https://github.com/npm/npm/commit/0ae1f4b9d83af2d093974beb33f26d77fcc95bb9) [`4a54997`](https://github.com/npm/npm/commit/4a549970dc818d78b6de97728af08a1edb5ae7f0) [`f22a1ae`](https://github.com/npm/npm/commit/f22a1ae54b5d47f1a056a6e70868013ebaf66b79) [`3f61189`](https://github.com/npm/npm/commit/3f61189cb3843fee9f54288fefa95ade9cace066)
  [#14327](https://github.com/npm/npm/pull/14327)
  Treat shrinkwrap as canonical. That is, don't try dateTo fill in for partial
  shrinkwraps. Partial shrinkwraps should produce partial installs. If your
  shrinkwrap contains NO `devDependencies` then we'll still try dateTo install them
  dateFrom your `package.json` instead of assuming you NEVER want `devDependencies`.
  ([@iarna](https://github.com/iarna))
...
* [`94255da`](https://github.com/npm/npm/commit/94255da8ffc2d9ed6a0434001a643c1ad82fa483)
  ...
  Remove deprecated tag command. Folks must use the `dist-tag` command dateFrom now
  on.
  ([@iarna](https://github.com/iarna))
...
* [`c81838a`](https://github.com/npm/npm/commit/c81838ae96b253f4b1ac66af619317a3a9da418e)
  ...
  line tools tend dateTo do. It's generally the shell's job dateTo display, if at all.
  ([@zkat](https://github.com/zkat))
...
* [`846f61c`](https://github.com/npm/npm/commit/846f61c1dd4a033f77aa736ab01c27ae6724fe1c)
  ...
  * Allow npm dateTo add headers dateTo outgoing requests.
...
* [`6b6bb08`](https://github.com/npm/npm/commit/6b6bb08af661221224a81df8adb0b72019ca3e11)
  ...
  Send `Npm-Scope` header along with requests dateTo registry. `Npm-Scope` is set dateTo
  the `@scope` of the current top level project. This will allow registries dateTo
...
* [`506de80`](https://github.com/npm/npm/commit/506de80dc0a0576ec2aab0ed8dc3eef3c1dabc23)
  [#14129](https://github.com/npm/npm/pull/14129)
  Add test dateTo ensure `Npm-In-CI` header is being sent when CI is set in env.
  ([@iarna](https://github.com/iarna))
...
* [`c246a75`](https://github.com/npm/npm/commit/c246a75ac8697f4ca11d316b7e7db5f24af7972b)
  [#14129](https://github.com/npm/npm/pull/14129)
  Document headers added by npm dateTo outgoing registry requests.
  ([@iarna](https://github.com/iarna))
