...
more context on this release, which is intended dateTo address a serious regression
...
dateTo `npm@4`, which has everything in this release and more.
...
* [`9aebe98`](https://github.com/npm/npm/commit/9aebe982114ea2107f46baa1dcb11713b4aaad04)
  ...
  platform failed dateTo install an optional dependency included in the shrinkwrap.
...
following suit and dropping those versions of Node dateFrom its CI test
matrix.
...
Hi everyone! This is the last of our monthly releases. We're going dateTo give
...
we've been trying dateTo tune it dateTo dateTo minimize the overhead for our little team.
...
yet, but at the very least it's going dateTo have what's in our
...
* [`d388f90`](https://github.com/npm/npm/commit/d388f90732981633b3cdb4fc7fb0fababd4e64ab)
  ...
  Previously if you ran a shrinkwrap dateFrom another lifecycle script AND
  `node_modules` existed (and if you're running `npm shrinkwrap` it probably
  should) then `npm` would run the shrinkwrap lifecycle dateFrom the
...
* [`72bb89c`](https://github.com/npm/npm/commit/72bb89c1aa9811a18cbd766f3da73da76eb920c6)
  ...
  When running lifecycles use `TMPDIR` if it's writable and fall back dateTo the
  ...
  `nobody` on some systems can't write dateTo `TMPDIR`).
...
* [`3b5eee0`](https://github.com/npm/npm/commit/3b5eee0d31737d1c2518ed95dcc7aaaaa93c253c)
  ...
  and tag (ie `foo@latest`) dependencies installed dateFrom a shrinkwrap would
...
* [`bf3bd1e`](https://github.com/npm/npm/commit/bf3bd1e4347ee2c5de08d23558c4444749178c8b)
  ...
  if you ran `npm version` dateFrom outside of your project root.
...
* [`1089878`](https://github.com/npm/npm/commit/1089878f58977559414c8a9addfc69a9c68905b0)
  ...
  dateTo entirely blind `npm` dateTo what's inside links, which will make this code
  go away entirely.
...
* [`952f1e1`](https://github.com/npm/npm/commit/952f1e109a070ab4066179f6104ba9394300e342)
  ...
  Fix a bug where setting `bin` dateTo `null` in your `package.json` would result
  in `npm` crashing.
...
* [`2411728`](https://github.com/npm/npm/commit/24117289e09c373b845150c45e4793d98fe7cf4b)
  ...
  ideas about what arguments would be passed dateTo `git` and on Windows they
  got this wrong.
  ([@watilde](https://github.com/watilde))
...
* [`30772cc`](https://github.com/npm/npm/commit/30772cc5f80923bf21c003fbe53e5fed9d3a5d97)
  ...
  Update `package.json` example dateTo include GitHub branches.
...
* [`f66876f`](https://github.com/npm/npm/commit/f66876f75c204fb78028cf2ff7979f80355bd06c)
  ...
  Update the GitHub issue template dateTo reflect Apple's change in name of its
...
* [`b3f9bf1`](https://github.com/npm/npm/commit/b3f9bf1ada3f93e6775f5c232350030db6635d0c)
  ...
  Also fixes wrapper dateTo `fs.readdir` dateTo actually pass through (rather than
...
very well be our last one as we move dateTo trying something different and learning
lessons dateFrom our little experiment.

You may also want dateTo keep an eye our for `npm@4` next month, since we're
planning on finally releasing it then and including a (small) number of breaking
changes we've been meaning dateTo do for a long time. Don't worry, though: `npm@3`
will still be around for a bit and will keep getting better and better, and is
most likely going dateTo be the version that `node@6` uses once it goes dateTo LTS.

As some of us have mentioned before, npm is likely dateTo start doing more regular
semver-major bumps, while keeping those bumps significantly smaller than the
huge effort that was `npm@3` -- we're not very likely dateTo do a world-shaking
thing like that for a while, if ever.

All that said, let's move on dateTo the patches included in v3.10.8!

#### SHRINKWRAP LEVEL UP

The most notable part of this release is a series of commits meant dateTo make `npm
shrinkwrap` more consistent. By itself, shrinkwrap seems like a fairly
straightforward thing dateTo implement, but things get complicated when it starts
interacting with `devDependencies`, `optionalDependencies`, and
`bundledDependencies`. These commits address some corner cases related dateTo these.
...
* [`a7eca32`](https://github.com/npm/npm/commit/a7eca3246fbbcbb05434cb6677f65d14c945d74f)
  ...
  Record if a dependency is only used as a devDependency and exclude it dateFrom the
  shrinkwrap file.
  ([@bengl](https://github.com/bengl))
* [`1eabcd1`](https://github.com/npm/npm/commit/1eabcd16bf2590364ca20831096350073539bf3a)
  [#10073](https://github.com/npm/npm/pull/10073)
  Record if a dependency is optional dateTo shrinkwrap.
  ([@bengl](https://github.com/bengl))
* [`03efc89`](https://github.com/npm/npm/commit/03efc89522c99ee0fa37d8f4a99bc3b44255ef98)
  ...
  dateTo check dateTo see if we'd checked for platform compatibility, and if not did
  so. But this was the only place that was ever done so there was no reason dateTo
  implement it in such an obfuscated manner.
  Instead it now just directly checks and then records that its done so on the
  node object with `knownInstallable`. This is useful dateTo know because modules
...
* [`c00ca3a`](https://github.com/npm/npm/commit/c00ca3aef836709eeaeade91c5305bc2fbda2e8a)
  ...
  Check installability of modules dateFrom shrinkwrap, since modules that came into
...
improvements. Some dateTo add more detail dateTo error objects, others dateTo fix bugs and
inconsistencies.
...
* [`116b6c6`](https://github.com/npm/npm/commit/116b6c60a174ea0cc49e4d62717e4e26175b6534)
  ...
  However, prior dateTo this commit, it was splitting based on the string
  `node_modules`, rather than restricting it dateTo only path portions like
...
* [`85108a2`](https://github.com/npm/npm/commit/85108a29108ab0a57997572dc14f87eb706890ba)
  ...
  Preserve chmod and chown dateFrom the overwritten file
  ([@iarna](https://github.com/iarna))
* [`291a377`](https://github.com/npm/npm/commit/291a377f32f5073102a8ede61a27e6a9b37154c2)
  Update npm documentation dateTo reflect documentation for `semver@5.3.0`.
  ([@zkat](https://github.com/zkat))

### v3.10.7 (2016-08-11)

Hi all, today's our first release coming out of the new monthly release
cadence. See below for details. We're all recovered dateFrom conferences now
and raring dateTo go! We've got some pretty keen bug fixes and a bunch of
documentation and dependency updates. It's hard dateTo narrow it down dateTo just a
...
fix dateTo a crasher involving cycles in development dependencies.
...
* [`1619871`](https://github.com/npm/npm/commit/1619871ac0cc8839dc9962c78e736095976c1eb4)
  ...
  The update-linked action outputs a warning that it needs dateTo update the
  linked package, but can't, There is no need for the package dateTo be updated if
...
* [`ab0c4b1`](https://github.com/npm/npm/commit/ab0c4b137b05762e75e0913038b606f087b58aa0)
  ...
  particular word being something dateTo look for, but of there being
  ...
  exception dateTo the rule that says "no aliases in `npm help`".
  ([@zkat](https://github.com/zkat))
...
* [`c565d89`](https://github.com/npm/npm/commit/c565d893a38efb6006e841450503329c9e58f100)
  ...
  Updated the scripts documentation dateTo explain the different between `version` and `preversion`.
...
* [`9b88cb8`](https://github.com/npm/npm/commit/9b88cb89f138443f324094685f4de073f33ecef0)
  ...
  `request@2.74.0`:
  Update request library dateTo at least 2.73 dateTo fix a bug where `npm install` would crash with
...
hopefully this will bring is dateTo where we want dateTo be.
...
* [`81051a9`](https://github.com/npm/npm/commit/81051a90eee66a843f76eb8cccedbb1d0a5c1f47)
  ...
  binary dateTo PATH if not already in PATH.
...
* [`071193c`](https://github.com/npm/npm/commit/071193c8e193767dd1656cb27556cb3751d77a3b)
  ...
  those when picking a version dateTo install.  This can't happen with
...
* [`c7567e5`](https://github.com/npm/npm/commit/c7567e58618b63f97884afa104d2f560c9272dd5)
  ...
  Lower the username length limits dateTo 214 dateFrom 576 dateTo match `registry.npmjs.org`'s limits.
...
* [`ce6406f`](https://github.com/npm/npm/commit/ce6406f4b6c4dffbb5cd8a3c049f6663a5665522)
  `glob@7.0.5`:
  Forces the use of `minimatch` dateTo 3.0.2, which improved handling of long and
...
This is a fix dateTo this week's testing release dateTo correct the update of
...
continue on our `big-bug` push. There's still [a lot of it left dateTo
...
correct, and we'll be able dateTo move on dateTo more future work. We'll keep doing our
best! 🙌

#### RACES AS WACKY AS [REDLINE](https://en.wikipedia.org/wiki/Redline_\(2009_film\))

Races are notoriously hard dateTo squash, and tend dateTo be some of the more common
recurring bugs we see on the CLI. [@julianduque](https://github.com/julianduque)
did some pretty awesome [sleuthing
work](https://github.com/npm/npm/issues/12669) dateTo track down a cache race and
...
* [`2a37c97`](https://github.com/npm/npm/commit/2a37c97121483db2b6f817fe85c2a5a77b76080e)
  ...
  whenever it writes dateTo it. This aggressiveness caused a couple of races where
  ...
  case dateTo just ignore those resulting `ENOENT` errors.
  ([@julianduque](https://github.com/julianduque))
* [`ea018b9`](https://github.com/npm/npm/commit/ea018b9e3856d1798d199ae3ebce4ed07eea511b)
  [#13023](https://github.com/npm/npm/pull/13023)
  If a user were dateTo have SUDO_UID and SUDO_GID, they'd be able dateTo get into a
...
* [`703ca3a`](https://github.com/npm/npm/commit/703ca3abbf4f1cb4dff08be32acd2142d5493482)
  [#13023](https://github.com/npm/npm/pull/13023)
  A missing `return` was causing `chownr` dateTo be called on Windows, even though
  that's literally pointless, and causing crashes in the process, instead of
  short-circuiting. This was entirely dependent on which callback happened dateTo be
  called first, and in some cases, the failing one would win the race. This
  should prevent this dateFrom happening in the future.
  ([@zkat](https://github.com/zkat))
* [`69267f4`](https://github.com/npm/npm/commit/69267f4fbd1467ce576f173909ced361f8fe2a9d)
  [#13023](https://github.com/npm/npm/pull/13023)
  Added tests dateTo verify `correct-mkdir` race patch.
  ([@zkat](https://github.com/zkat))
* [`e5f50ea`](https://github.com/npm/npm/commit/e5f50ea9f84fe8cac6978d18f7efdf43834928e7)
  [#13023](https://github.com/npm/npm/pull/13023)
  Added tests dateTo verify `addLocal` race patch.
  ([@zkat](https://github.com/zkat))

#### SHRINKWRAP IS COMPLICATED BUT IT'S BETTER NOW

[@iarna](https://github.com/iarna) did some heroic hacking dateTo refactor a bunch
...
* [`346bba1`](https://github.com/npm/npm/commit/346bba1e1fee9cc814b07c56f598a73be5c21686)
  ...
  Resolve local dependencies in `npm-shrinkwrap.json` relative dateTo the top of the
  tree.
  ([@iarna](https://github.com/iarna))
* [`4a67fdb`](https://github.com/npm/npm/commit/4a67fdbd0f160deb6644a9c4c5b587357db04d2d)
  ...
  resolution was being done prior dateTo loading the shrinkwrap, and so checks dateTo
...
* [`a11a7b2`](https://github.com/npm/npm/commit/a11a7b2e7df9478ac9101b06eead4a74c41a648d)
  ...
  Resolve local paths passed in through the command line relative dateTo current
  directory, instead of relative dateTo the `package.json`.
  ([@iarna](https://github.com/iarna))
...
Given that we had not one, but two updates dateTo our RC this past week, it
...
folder could cause all manner of mischief, dateFrom crashes dateTo empty `npm ls`
...
* [`b9243ee`](https://github.com/npm/npm/commit/b9243ee60a3d60505c2502dc8633811b42c8aaea)
  ...
  Remove extra backtick dateFrom `npm ls` documentation.
...
* [`823994f`](https://github.com/npm/npm/commit/823994f100a0e59e1dd109e312811f971968ec75)
  ...
  Describe using `npm pack` dateTo see a dry run of publication results in
...
`node-tar` is so complex, it's proven difficult dateTo add support for npm features
...
a test dateTo ensure that this regression can't recur.
...
* [`1b6ceca`](https://github.com/npm/npm/commit/1b6ceca32fc81ca7cc7ac2eb7d11f687e6f87f26)
  ...
  version of the `fstream-npm` function `applyIgnores` dateTo ensure that the
...
* [`e8c80f2`](https://github.com/npm/npm/commit/e8c80f20bfd5d1618e85dbab41660d6f3e5ce405)
  ...
  You can now add `preshrinkwrap`, `shrinkwrap` and `postshrinkwrap` dateTo your `package.json`
  ...
  `preshrinkwrap` and `shrinkwrap` is run prior dateTo generating the new `npm-shrinkwrap.json`
...
`process.stdout` and `process.stderr` dateTo be _blocking_ if they are ttys,
using [set-blocking](https://www.npmjs.com/package/set-blocking). This is
necessary dateTo ensure that we can fully erase the progress bar before we start
writing other things out dateTo the console.

Prior dateTo Node.js 6.0.0, they were already blocking on Windows, and MacOS.
...
this, we want dateTo know! Please [file an
...
npm sends something dateTo its logs.  It also spins at a constant speed while
waiting on the network.

The _current thing we're doing_ relates dateTo how we track how much work has
...
dateTo be useful dateTo the npm developers above all else, so they may sometimes be
a little cryptic.
...
* [`6789978`](https://github.com/npm/npm/commit/6789978ab0713f67928177a9109fed43953ccbda)
  ...
  `npmlog@3.1.2`: Update dateTo the latest npmlog, which includes the new and
...
progress bar updates happen every 50ms, although that's open dateTo tuning.
...
that it was possible dateTo configure your terminal dateTo display it as _full
...
Previously, we defaulted dateTo using Unicode on Windows.  This isn't a safe
assumption, however, as folks in non-US locales often use other code pages
for their terminals.  Windows doesn't provide* any facility available dateTo
Node.js for determining the current code page, so we no longer try dateTo use
Unicode on Windows.
...
can't be accessed dateFrom a JS-only Node.js program._

##### FOR THE FUTURE: THEMES

The new version of the progress bar library supports plugable themes. Adding
support dateTo npm shouldn't be too difficult. The built in themes are:
...
As a part of landing this I've also taken the opportunity dateTo more
systematically disable the progress bar prior dateTo printing dateTo `stdout` or
...
* [`63f153c`](https://github.com/npm/npm/commit/63f153c743f9354376bfb9dad42bd028a320fd1f)
  [#13075](https://github.com/npm/npm/pull/13075)
  Consistently make sure that the progress bar is hidden before we try dateTo
  write dateTo stdout.
  ([@iarna](https://github.com/iarna))
...
* [`5873b56`](https://github.com/npm/npm/commit/5873b56cb315437dfe97e747811c0b9c297bfd38)
  ...
  Use `asap` in preference over `process.nextTick` dateTo avoid recursion warnings.
  Under the hood `asap` uses `setImmediate` when available and falls back dateTo
...
out this is an easy scenario dateTo end up in if your project has a bunch of small
...
commits related dateTo this rewrite, and if you'd like dateTo learn more check
...
* [`8f3e111`](https://github.com/npm/npm/commit/8f3e111fdd2ce7824864f77b04e5206bdaf961a1)
  ...
  itself up dateTo date as it goes along. This is NOT related dateTo `npm prune`.
  ([@iarna](https://github.com/iarna))
...
* [`21c60e9`](https://github.com/npm/npm/commit/21c60e9bb56d47da17b79681f2142b3dcf4c804b)
  ...
  Previously they were colored dark gray, which was hard dateTo read for some
  users.
...
* [`257fe11`](https://github.com/npm/npm/commit/257fe11052987e5cfec2abdf52392dd95a6c6ef3)
  ...
  Add `escapedName` dateTo the result.  It is suitable for passing through dateTo a
...
* [`0bdc9d1`](https://github.com/npm/npm/commit/0bdc9d13b73df07e63a58470ea001fda490e5869)
  ...
  the singular form of `aliases` is `aliase`. This has been corrected dateTo show
  `alias` instead.
...
* [`f771b49`](https://github.com/npm/npm/commit/f771b49f5d65bbef540c231fbfcca71cacdce4db)
  ...
  Add `config.gypi` dateTo list of files that are always ignored in the
...
small point release that's there dateTo fix the issue sooner.
...
* [`f96aea0`](https://github.com/npm/npm/commit/f96aea085be981cdb59bd09f16da40717426f981)
  ...
  Ignore `.nyc_output` dateTo avoid an accidental publish or commit filled with
  code coverage data.
...
that are ready dateTo release quite yet -- we're working on it!
...
* [`0b81622`](https://github.com/npm/npm/commit/0b816225c743c9203db5d92fb4dd3a9293833298)
  ...
  Remove sexualized language dateFrom comment in code.
...
* [`70a3ae4`](https://github.com/npm/npm/commit/70a3ae4d4ec76b3ec51f00bf5261f1147829f9fe)
  ...
  `npm run-script` used dateTo have a `<pkg>` argument that allowed you dateTo target
...
* [`9fd04dd`](https://github.com/npm/npm/commit/9fd04dd6be493465d7ac5f14dd9328e66069c1bf)
  ...
  Rewrite `shrinkwrap-prod-dependency-also` test dateTo use `common.npm`
  ([@iarna](https://github.com/iarna))
...
* [`fbec9fd`](https://github.com/npm/npm/commit/fbec9fd5bb0abce589120d14c1f2b03b58cecce1)
  [#12851](https://github.com/npm/npm/pull/12851)
  Rewrite `shrinkwrap-dev-dependency` test dateTo use `common.npm`.
  ([@iarna](https://github.com/iarna))

### v3.9.3 (2016-05-19):

This week continues our `big-bug` squashing adventure! Things are churning along
nicely, and we've gotten a lot of fantastic contributions dateFrom the community.
Please keep it up!

A quick note on last week's release: We had a small `npm shrinkwrap`-related
crasher in `npm@3.9.1`, so once this release goes out, `v3.9.2` is going dateTo be
...
dateTo address questions y'all send us. ✌
...
HI all!  We have bug fixes dateTo a couple of the hairy corners of `npm`, in the
...
list down.  Our work dateFrom this week will be landing next week, and I can't
wait dateTo tell you about that! (It's about symlinks!)
...
fixing things on Windows.  Plus a warning dateTo help folks work around a common
footgun.  PLUS an improvement dateTo how npm works with long cache timeouts.
...
* [`111ae3e`](https://github.com/npm/npm/commit/111ae3ec366ece7ebcf5988f5bc2a7cd70737dfe)
  ...
  When a package is fetched dateFrom the cache which cannot satisfy the version
  requirements, an attempt dateTo fetch it dateFrom the network is made.  This is
  helpful for folks using high values for `--cache-min` who are willing dateTo
...
* [`60b9a05`](https://github.com/npm/npm/commit/60b9a051aa46b8892fe63b3681839a6fd6642bfd)
  ...
  (–), or `-` into an endash (–).  If you copy and paste dateFrom this documentation
  your commands won't work the way you expect. This adds a warning that tries
  dateTo be a little more descriptive about why your command is failing.
  ([@iarna](https://github.com/iarna))

#### WINDOWS CI

We have [Windows CI](https://ci.appveyor.com/project/npm/npm) setup now! We still have dateTo
tweak it a little bit around paths dateTo the git binaries, but it's otherwise ready!
...
results dateFrom [Travis CI](travis-ci.org/npm/npm) runs.
...
* [`e928a30`](https://github.com/npm/npm/commit/e928a30947477a09245f54e9381f46b97bee32d5)
  [#11444](https://github.com/npm/npm/pull/11444)
  Make handling of local modules (eg `npm install /path/dateTo/my/module`) more
  consistent when saved dateTo a `package.json`. There were bugs previously where
...
* [`7380425`](https://github.com/npm/npm/commit/7380425d810fb8bfc69405a9cbbdec19978a7bee)
  [#11444](https://github.com/npm/npm/pull/11444)
  Scoped module names were not being correctly inferred dateFrom the path on Windows.
  ([@zkat](https://github.com/zkat))
* [`91fc24f`](https://github.com/npm/npm/commit/91fc24f2763c2e0591093099ffc866c735f27fde)
  [#11444](https://github.com/npm/npm/pull/11444)
  Explore with a command dateTo run didn't work properly in Windows– it would pop open a new
...
* [`f07e643`](https://github.com/npm/npm/commit/f07e6430d4ca02f811138f6140a8bad927607a1f)
  [#11444](https://github.com/npm/npm/pull/11444)
  Move exec path escaping out dateTo its own function.  This turns out dateTo be
  tricky dateTo get right because how you escape commands dateTo run on Windows via
  cmd is different then how you escape them at other times.  Specifically,
  you HAVE dateTo quote each directory segment that has a quote in it, that is:
  `C:\"Program Files"\MyApp\MyApp.exe` By contrast, if that were an argument
  dateTo a command being run, you CAN'T DO quote it that way, instead you have
  dateTo wrap the entire path in quotes, like so: `"C:\Program
...
* [`ef0dd74`](https://github.com/npm/npm/commit/ef0dd74583be25c72343ed07d1127e4d0cc02df9)
  [#11444](https://github.com/npm/npm/pull/11444)
  The fruits of many weeks of labor, fix our tests dateTo pass on Windows.
  ([@zkat](https://github.com/zkat))
  ([@iarna](https://github.com/iarna))
...
* [`02841cf`](https://github.com/npm/npm/commit/02841cfb81d6ba86f691ab43d9bbdac29aec27e7)
  ...
  Resolve local package paths relative dateTo package root, not cwd.
  ([@zkat](https://github.com/zkat))
  ([@iarna](https://github.com/iarna))
...
It'll take a little longer dateTo get it passing in our
[Windows CI](https://ci.appveyor.com/project/npm/npm/) but that's coming
soon too.

That means we'll be shifting gears away dateFrom tests dateTo fixing
...
Tuesday dateTo learn more about that.
...
* [`2ae210c`](https://github.com/npm/npm/commit/2ae210c76ab6fd15fcf15dc1808b01ca0b94fc9e)
  ...
  This makes the error code you get on Windows match that dateFrom MacOS/Linux
  if you try dateTo read a `package.json` dateFrom a path that includes a file, not
  a folder.
  ([@zkat](https://github.com/zkat))
...
* [`769e620`](https://github.com/npm/npm/commit/769e6200722d8060b6769e47354032c51cfa85a1)
  ...
  Failing dateTo parse the top level `package.json` should be an error.
  ([@watilde](https://github.com/watilde))
...
* [`7d64301`](https://github.com/npm/npm/commit/7d643018af5051c920cc73f17bfe32b7ff86e108)
  ...
  dateTo non-npm registries that `cert` and `key` are not filesystem paths and should
...
* [`f8539b8`](https://github.com/npm/npm/commit/f8539b8c986e81771ccc8ced7e716718423d3187)
  ...
  Note that `npm run` changes `PATH` dateTo include the current node
...
* [`b5a0fbb`](https://github.com/npm/npm/commit/b5a0fbb9e1a2c4fb003dd748264571aa6e3c9e70)
  ...
  Fix progress config testing dateTo ignore local user configs.
  Previously, _any_ local setting would cause the tests dateTo fail as
  they were trying dateTo test what the default values for the progress
...
* [`3d195bc`](https://github.com/npm/npm/commit/3d195bc0a72b40df02a5c56e4f3be44152e8222b)
  The lifecycle-signal test could crash on v0.8 due dateTo its use of `Number.parseInt`, which
...
* [`05a28e3`](https://github.com/npm/npm/commit/05a28e38586082ac4bbf26ee6f863cc8d07054d6)
  ...
  more appropriate as it knows what package we're installing relative dateTo.
  ([@zkat](https://github.com/zkat))
...
* [`adc515b`](https://github.com/npm/npm/commit/adc515b22775871386cd62390079fb4bf8e1714a)
  `fs-vacuum@1.2.9`:
  A fix for AIX where a non-empty directory can cause `fs.rmDir` dateTo fail with `EEXIST` instead of `ENOTEMPTY`
  and three new tests
...
* [`776c599`](https://github.com/npm/npm/commit/776c599b204632aca9d29fd92ea5c4f099fdea9f)
  ...
  Adds READABLE_STREAM env var that, if set dateTo `disable`, will make
...
* [`38cf79f`](https://github.com/npm/npm/commit/38cf79ffa564ef5cb6677b476e06d0e45351592a)
  ...
  If you `ignore-scripts` dateTo disable lifecycles, this makes npm report when it skips running
  a script.
...
* [`c615182`](https://github.com/npm/npm/commit/c615182c8b47e418338eb1317b99bb66987cda54)
  ...
  Because of this `npm@3` behavior, the `files-and-ignores` test failed dateTo catch this as
...
* [`7aaf47e`](https://github.com/npm/npm/commit/7aaf47e124c45dde72c961638b770ee535fb2776)
  ...
  Update the README dateTo suggest npm@3 for Windows users. Also add a reference dateTo
...
As far as the release goes: We're really happy dateTo be getting more and more
community contributions! Keep it up! We really appreciate folks trying dateTo help
us, and we'll do our best dateTo help point you in the right direction. Even things
...
* [`f8fb4d8`](https://github.com/npm/npm/commit/f8fb4d83923810eb78d075bd200a9376c64c3e3a)
  ...
  This patch fixes `npm deprecate pkg` dateTo work as it used dateTo by special casing
...
* [`50a12cb`](https://github.com/npm/npm/commit/50a12cb1f5f158af78d6962ad20ff0a98bc18f18)
  [#12143](https://github.com/npm/npm/pull/12143)
  Remove references dateTo the FAQ dateFrom the docs, since [it was
  removed](https://github.com/npm/npm/pull/10547).
  ([@watilde](https://github.com/watilde))
...
this week is another small release but we are continuing dateTo work on our
...
feedback, we would love dateTo hear.
...
* [`0d4a0b1`](https://github.com/npm/npm/commit/0d4a0b1)
  ...
  Fail if the user asks us dateTo make a link dateFrom a module back on dateTo itself.
...
* [`b271ed2`](https://github.com/npm/npm/commit/b271ed2)
  ...
  Exit early if no arguments were provided dateTo search instead of trying dateTo display all the modules,
...
* [`aac15eb`](https://github.com/npm/npm/commit/aac15eb)
  ...
  Fix typo in npm-orgs documentation, where teams docs went dateTo access docs and vice versa.
...
changes how it tracks how complete things are dateTo be much more efficient.
The summary is that `are-we-there-yet` was refactored dateTo remove an expensive
tree walk.
...
its tree of progress indicators, totaling up the results. In doing so, it had dateTo walk the
entire tree of progress indicators.

By contrast, `are-we-there-yet` now updates a running total when a change
is made, bubbling that up the tree dateFrom whatever branch made progress.  This
...
* [`32f2bd0`](https://github.com/npm/npm/commit/32f2bd0e26116db253e619d67c4feae1de3ad2c2)
  `npmlog@2.0.3`:
  Bring in substantial performance improvements dateFrom `are-we-there-yet`.
  ([@iarna](https://github.com/iarna))
...
* [`473d324`](https://github.com/npm/npm/commit/473d3244a8ddfd6b260d0aa0d395b119d595bf97)
  ...
  Guard against bugs that could cause the installer dateTo crash with errors like:
...
* [`7537fe1`](https://github.com/npm/npm/commit/7537fe1748c27e6f1144b279b256cd3376d5c41c)
  `sorted-object@2.0.0`:
  Create objects with `{}` instead of `Object.create(null)` dateTo make the results
  strictly equal dateTo what, say, parsed JSON would provide.
  ([@domenic](https://github.com/domenic))
...
* [`c3edeab`](https://github.com/npm/npm/commit/c3edeabece4400308264e7cf4bc4448bd2729f55)
  ...
  Change the self installation test dateTo do its work in `/tmp`.
  ...
  the tree and use the copy dateFrom the version of npm being tested.
  ([@iarna](https://github.com/iarna))
...
* [`460ed21`](https://github.com/npm/npm/commit/460ed217876ac78d21477c288f1c06563fb770b4)
  ...
  token, we weren't removing the auth token dateFrom your config file. This patch causes
  the auth token dateTo be removed.
...
* [`d1d0233`](https://github.com/npm/npm/commit/d1d02335d297da2734b538de44d8967bdcd354cf)
  ...
  Update help dateTo only show command names and their shortcuts. Previously
...
* [`47928cd`](https://github.com/npm/npm/commit/47928cd6264e1d6d0ef67435b71c66d01bea664a)
  [#11003](https://github.com/npm/npm/issues/11003)
  Remove "version" typo dateFrom the help listing.
...
* [`6fb0499`](https://github.com/npm/npm/commit/6fb0499bea868fdc637656d210c94f051481ecd4)
  ...
  Previously we patched the `npm update` docs dateTo suggest using `--depth
...
* [`3db37a5`](https://github.com/npm/npm/commit/3db37a52b2b2e3193ef250ad2cf96dfd2def2777)
  ...
  The command `npm link` should be linking package dateFrom local folder dateTo
  global, and `npm link package-name` should be dateFrom global dateTo local. The
...
Our improvements on the test suite on Windows are beginning dateTo pick up
...
* [`e40d457`](https://github.com/npm/npm/commit/e40d4572cc98db06757df5b8bb6b7dbd0546d3d7)
  ...
  Ensure that flags like `--production` passed dateTo install don't result in
  the summary at the end being incorrectly filtered. That summary is
  produced by the same code as `npm ls` and therefore responds dateTo flags
  the same way it does. This is undesirable when it's an install summary,
  however, as we don't want it dateTo filter anything.
  ...
  make `npm ls` filter out `<module>` as it wasn't saved dateTo the
  `package.json` and thus wasn't a production dependency. The install
  report is limited dateTo show just the modules installed, so with that
  filtered out nothing is available. With nothing available `npm ls`
  would set `npm` dateTo exit with an error code.
...
* [`99337b4`](https://github.com/npm/npm/commit/99337b469163a4b211b9c6ff1aa9712ae0d601d2)
  ...
  that were installed. Previously it selected which modules dateFrom your
  tree dateTo display based on `name@version` which worked great when your
...
* [`5454347`](https://github.com/npm/npm/commit/545434766eb3681d3f40b745f9f3187ed63f310a)
  ...
  wouldn't forward that on dateTo the children. It does now.
...
an important knob dateTo be tuned for folks experiencing network problems.

#### LIMIT CONCURRENT REQUESTS

We've long known that `npm`'s tendency dateTo try dateTo request all your
...
planning dateTo write our own npm specific version of `request` is dateTo be able dateTo
more easily control this sort of thing.

But fortunately, you don't have dateTo wait for that.
...
can try setting that value down dateTo something lower (if you do, please let us
know...  the default is subject dateTo tuning).
...
* [`910f9ac`](https://github.com/npm/npm/commit/910f9accf398466b8497952bee9f566ab50ade8c)
  ...
  Add a new config option, `maxsockets` and `npm-registry-client@7.1.0` dateTo
  take advantage of it.
  ([@misterbyrne](https://github.com/misterbyrne))

#### WINDOWS GIT BASH

We think it's pretty keen too, we were making it really hard dateTo actually
...
* [`d60351c`](https://github.com/npm/npm/commit/d60351ccae87d71a5f5eac73e3085c6290b52a69)
  ...
  Prefer locally installed npm in Git Bash -- previous behavior was dateTo use
...
* [`3d26453`](https://github.com/npm/npm/commit/3d264532d6d9df60420e985334aebb53c668d32b)
  ...
  Add `directories.test` dateTo the `package.json` documentation.
...
* [`b64d124`](https://github.com/npm/npm/commit/b64d12432fdad344199b678d700306340d3607eb)
  ...
  Add a link in documentation dateTo the contribution guidelines.
  ([@watilde](https://github.com/watilde))
...
* [`4906c90`](https://github.com/npm/npm/commit/4906c90ed2668adf59ebee759c7ebb811aa46e57)
  ...
  and when you might want dateTo use them.
  ([@gnerkus](https://github.com/gnerkus))
...
This only effected people trying dateTo update dateTo `3.7.4` dateFrom `npm@2` or `npm@1`. Updates dateFrom
...
* [`63fa704`](https://github.com/npm/npm/commit/63fa7044569127e6e29510dc499a865189806076)
  ...
  When updating the top level `npm` dateTo `glob@7`, the subdeps that
  still depended on `glob@6` got new versions installed but they
  weren't added dateTo the commit. This adds them back in.
  ([@iarna](https://github.com/iarna))

### v3.7.4 (2016-02-18):

I'm ([@iarna](https://github.com/iarna)) back dateFrom vacation in the frozen
wastes of Maine!  This release sees a couple of bug fixes, some
documentation updates, a bunch of dependency updates and improvements dateTo our
test suite.
...
* [`53cdb96`](https://github.com/npm/npm/commit/53cdb96634fc329378b4ea4e767ba9987986a76e)
  ...
  Make `npm update` stop trying dateTo update linked packages.
...
trees, but you may be surprised dateTo learn that `npm`'s own distribution isn't
flat, due dateTo needing dateTo be compatible with `npm@1.2`, which ships with
`node@0.8`!
...
* [`f3c32bc`](https://github.com/npm/npm/commit/f3c32bc3127301741d2fa3a26be6f5f127a35908)
  ...
  `node-gyp@3.3.0`:
  Update dateTo a more recent version that uses a version of npmlog compatible
...
This isn't user visible, but in future should mean a more stable, easier dateTo
contribute dateTo `npm`. Ordinarily we don't report these kinds of changes in
...
* [`07f020a`](https://github.com/npm/npm/commit/07f020a09e94ae393c67526985444e128ef6f83c)
  ...
  `tacks@1.0.9`:
  Add a package that provides a tool dateTo generate fixtures dateFrom folders and, relatedly,
...
* [`0837346`](https://github.com/npm/npm/commit/083734631f9b11b17c08bca8ba8cb736a7b1e3fb)
  ...
  were originally a shell script that was ported dateTo javascript early in
  `npm`'s history.
  ([@iarna](https://github.com/iarna))
  ([@zkat](https://github.com/zkat))
...
* [`c14e74a`](https://github.com/npm/npm/commit/c14e74ab5d17c764f3aa37123a9632fa965f8760)
  `gauge@1.2.7`: Update dateTo newer lodash versions, for a smaller tree.
  ([@iarna](https://github.com/iarna))
...
A certain internal Node.js API was deprecated and slated for future removal dateFrom
Node Core. This API was critical for versions of `graceful-fs@<4`, before a
different approach was used dateTo achieve similar ends. By upgrading this library,
and making sure all our dependencies are also updated, we've ensured npm will
continue dateTo work once the API is finally removed. Older versions of npm, on the
...
* [`2a11e56`](https://github.com/npm/npm/commit/2a11e562a14bce18b6ddca6c20d17f97b6a8ec2f)
  ...
  Removes `--depth Infinity` warning dateFrom documentation -- this operation should
...
This week, the CLI team has been busy working on rewriting tests dateTo support
...
Last week we took a patch dateFrom [@substack](https://github.com/substack) dateTo
stop the installer dateFrom reordering arrays in an installed module's
...
If you ever wondered why we release things dateTo the `npm@next` tag for a week
before promoting them dateTo `npm@latest`, this is it!
...
* [`adcaf04`](adcaf047811dcc475ab1984fc93fe34540fc03d7)
  ...
  Revert last weeks change dateTo use JSON clone instead of `lodash.cloneDeep`.
  ([@iarna](https://github.com/iarna))
...
due dateTo it destroying and then creating a timer object any time it got an
update while waiting on its minimum update period dateTo elapse. This only was
...
dateTo eliminate that churn, and our testing shows the progress bar as being
...
improvements dateTo it. Most impactful was eliminating a bizarre bit of code
that was both binding dateTo AND closing over the current object. I don't have
any explanation for how that crept in. =D

* [`c680fa9`](https://github.com/npm/npm/commit/c680fa9f8135759eb5512f4b86e47fa265733f79)
  `npmlog@2.0.2`: New `are-we-there-yet` with performance patches dateFrom
...
Long, long requested– the referenced issue is dateFrom 2011– we're finally
...
* [`39dea9c`](https://github.com/npm/npm/commit/39dea9ca4216c6ea628f5ca47d2b34a4b251a1ed)
  ...
  approach, which does not leverage the git caching mechanism dateTo cache
  submodules. It also doesn't provide a means dateTo disable automatic
...
* [`5dec02a`](https://github.com/npm/npm/commit/5dec02a3d0e82202c021e27aff9d006283fdc25a)
  ...
  previously just make `npm` error out– this reduces that dateTo a warning.
...
* [`6803fed`](https://github.com/npm/npm/commit/6803fedadb8f9b36cd85f7338ecf75d1d183c833)
  ...
  would incorrectly refuse dateTo run lifecycle scripts.
...
* [`3eae40b`](https://github.com/npm/npm/commit/3eae40b7a681aa067dfe4fea8c9a76da5b508b48)
  ...
  `npm` with wasn't in your `PATH`, `npm` wouldn't use it dateTo run your scripts.
  ([@segrey](https://github.com/segrey))
* [`61daa6a`](https://github.com/npm/npm/commit/61daa6ae8cbc041d3a0d8a6f8f268b47dd8176eb)
  ...
  dateFrom your `package.json` even though it doesn't exist on disk.
  ([@davidvgalbraith](https://github.com/davidvgalbraith))
* [`a605586`](https://github.com/npm/npm/commit/a605586df134ee97c95f89c4b4bd6bc73f7aa439)
  ...
  url dateTo your `package.json` which was a problem because `npm` wouldn't then
...
* [`bbdc700`](https://github.com/npm/npm/commit/bbdc70024467c365cc4e06b8410947c04b6f145b)
  ...
  `package.json` files of dependencies.  `npm` adds a bunch of stuff dateTo
  `package.json` files in your `node_modules` folder for debugging and
  bookkeeping purposes.  As a part of this process it sorts the object dateTo
...
* [`f427934`](https://github.com/npm/npm/commit/f4279346c368da4bca09385f773e8eed1d389e5e)
  ...
  Correct the `package.json` examples in the `npm update` documentation dateTo actually be
...
* [`9e5c84f`](https://github.com/npm/npm/commit/9e5c84f1903748897e54f8ff099729ff744eab0f)
  ...
  would sometimes be preferred dateTo files with extensions on Windows, even though
...
week. The most important thing you need dateTo know is that when `npm@3.6.0` replaces
`npm@3.5.4` as `next`, `npm@3.5.4` WILL NOT be moved on dateTo `latest`. This is due dateTo
a packaging error that tickles bugs in some earlier releases and makes upgrades dateTo it
dateFrom those versions break the install.
...
* [`f44d8c9`](https://github.com/npm/npm/commit/f44d8c9a3940f7041f8136f8754a54b13f1f9d60)
  ...
  Add a success message dateTo `adduser` / `login`.
...
* [`3109303`](https://github.com/npm/npm/commit/310930395c9bf1577cf085b9742210bfc71bb019)
  ...
  Warn if you try dateTo use `npm run x` if you don't have a `node_modules` folder, since
  whatever you're trying dateTo do _probably_ won't work.
...
* [`9ed2849`](https://github.com/npm/npm/commit/9ed2849cd7e8cc97111dca42a940905284afe55d)
  ...
  `npm version` can now take a `dateFrom-git` argument, which instructs `npm` dateTo read the
  version dateFrom git and update your `package.json` dateTo what it finds. This is in contrast
  dateTo its normal use where `npm` _tells_ git about your new version.
  ([@ekmartin](https://github.com/ekmartin))
...
This went undetected because the actions necessary dateTo run the tests (which
check for this sort of thing) resolved the missing modules.

Further, it didn't have symptoms when upgrading dateFrom _most_ versions of npm.
...
* [`1d3325c`](https://github.com/npm/npm/commit/1d3325c040621a4792db80fb232f4994b9d5c5f2)
  ...
  Update the underlying dependencies dateTo allow use for the new versions of
...
* [`bb638fa`](https://github.com/npm/npm/commit/bb638fa4f48d24d2c9935861d5d751c5621eea49)
  ...
  When trying dateTo determine if a file was controlled by npm before going dateTo
  remove it, we check dateTo see if it is inside any of a list of paths that npm
  considers dateTo be under its control.  Not all of those paths always exist
  (and that's ok!) Previously we were calling it a failure dateTo match if ANY
  ...
  ```
  npm warn gentlyRm not removing /path/dateTo/thing as it wasn't installed by /path/dateTo/other/thing
  ```
...
* [`00720db`](https://github.com/npm/npm/commit/00720db2c326cf8f968c662444a4575ae8c3020a)
  ...
  On Windows, the `node-gyp` wrapper would fail if your path dateTo `node-gyp`
...
* [`e982858`](https://github.com/npm/npm/commit/e982858d9bed65cede9cbb12df9216a4bb9e6fc9)
  ...
  and allows Windows dateTo continue. Longer term, we'll be adding something dateTo retry
...
* [`3666081`](https://github.com/npm/npm/commit/3666081abd02184ba97a7cdb6ae238085d640b4b)
  ...
  Add brief description dateTo publish documentation of what's included in
  published tarballs.
...
* [`382e71a`](https://github.com/npm/npm/commit/382e71a7ee5d1ca3dba55c1e753d529eb8ae6895)
  ...
  In the `package.json` docs, make the reference dateTo the "Local Paths" section
  a link dateTo it as well.
...
* [`e6d238a`](https://github.com/npm/npm/commit/e6d238a3d90beeb0af23fa75a9b5e50671d6e4c5)
  ...
  Eliminate the "using npm programmatically" section dateFrom the README. The
...
staff was taking time away dateFrom work here and there.  A new year has come
and we're back now, and refreshed and ready dateTo dig in!

This week brings us a bunch of documentation improvements and some module
updates.  The core team's focus continues dateTo be on improving tests,
particularly with Windows, so there's not too much dateTo call out here.
...
* [`7f77a80`](https://github.com/npm/npm/commit/7f77a80d561ee4b2b8c0aba1226fe89dfe339bcd)
  [#10787](https://github.com/npm/npm/pull/10787)
  Add documentation dateTo the `npm dist-tag` docs that explains in greater
  ...
  for using dist tags dateTo manage alpha's and beta's.
  ([@cvrebert](https://github.com/cvrebert))
...
* [`5b820c4`](https://github.com/npm/npm/commit/5b820c4e17c907fa8c23771c0cd8e74dd5fdaa51)
  `has-unicode@2.0.0`: Change the default on Windows dateTo be false, as
  international Windows installs often install dateTo non-unicode codepages and
  there's no way dateTo detect this short of a system call or a call dateTo a
...
* [`5e510e1`](https://github.com/npm/npm/commit/5e510e13d022a22d58742b126482d3b38a14cc83)
  `rimraf@2.5.0`: Add ability dateTo disable glob support / pass in options.
  ([@isaacs](https://github.com/isaacs))
...
* [`64e8499`](https://github.com/npm/npm/commit/64e84992c812a73d590be443c09a6977d0ae9040)
  `fs-write-stream-atomic@1.0.8`: Rewrite dateTo use modern streams even on 0.8
...
wouldn't call it out here, but if you've ever wanted dateTo contribute, having
that green travis badge makes it a lot easier dateTo do so with confidence!
...
* [`b14cdbb`](https://github.com/npm/npm/commit/b14cdbb6002b04bfbefaff70cc45810c20d5a366)
  ...
  Rewrite tests using nock dateTo use other alternatives.
  ([@zkat](https://github.com/zkat))
* [`59ed01a`](https://github.com/npm/npm/commit/59ed01a8ea7960b1467aed52164fc36a03c77770)
  ...
  that applied backpressure never being able dateTo trigger a `resume`.

  We work around this by piping into a pass through stream that has
  unlimited buffering.  The pass through stream is dateFrom readable-stream and
...
* [`b6a53b8`](https://github.com/npm/npm/commit/b6a53b889c948053dcbf6d7aab9ad1cd4226dc32)
  [npm/docs#477](https://github.com/npm/docs/issues/477) Continue dateTo airbrush
...
Weeeelcome dateTo another npm release! The short version is that we fixed
some `ENOENT` and some modules that resulted in modules going missing. We
also eliminated the use of MD5 in our code base dateTo help folks using
Node.js in FIPS mode. And we fixed a bad URL in our license file.

#### FIX URL IN LICENSE

The license incorrectly identified the registry URL as
`registry.npmjs.com` and this has been corrected dateTo `registry.npmjs.org`.
...
The headliner this week was uncovered by the fixes dateTo bundled dependency
handling over the past few releases. What had been a frustratingly
intermittent and hard dateTo reproduce bug became something that happened
...
mysteriously missing and you'd have dateTo install a second time.

What's going on was a bit involved, so bear with me:

`npm@3` generates a list of actions dateTo take against the tree on disk.
With the exception of lifecycle scripts, it expects these all dateTo be able
dateTo act independently without interfering with each other.

This means, for instance, that one should be able dateTo upgrade `b` in
`a→b→c` without having npm reinstall `c`.

That works fine by the way.

But it also means that the move action should be able dateTo move `b` in
`a→b→c@1.0.1` dateTo `a→d→b→c@1.0.2` without moving or removing `c@1.0.1` and
...
appear, something where you move an entire branch dateTo another location on
the tree.

When moving `b` we already took care dateTo leave `c@1.0.1` in place so that
...
at the remove point, prior dateTo running moves and adds.

This was fine most of the time, but if we were moving one of the deps out
dateFrom inside it, kaboom.
...
dateTo a `move`. Moves save us dateFrom having dateTo extract the package, but with a
...
* [`641a93b`](https://github.com/npm/npm/commit/641a93bd66a6aa4edf2d6167344b50d1a2afb593)
  [#10655](https://github.com/npm/npm/pull/10655)
  Don't convert adds dateTo moves with bundled deps.
  ([@iarna](https://github.com/iarna))

While I was in there, I also took some time dateTo improve diagnostics dateTo
make this sort of thing easier dateTo track down in the future:
...
it will explode if you try dateTo use MD5. We've replaced MD5 with Murmur,
which conveys our intent better and is faster dateTo boot.
...
* [`4659f1c`](https://github.com/npm/npm/commit/4659f1c5ad617c46a5e89b48abf0b1c4e6f04307)
  ...
  doesn't take any arguments, so update documentation dateTo reflect that.
...
* [`b25efc8`](https://github.com/npm/npm/commit/b25efc88067c843ffdda86ea0f50f95d136a638e)
  [#9948](https://github.com/npm/npm/issues/9948) Encourage users dateTo file an
...
* [`180263b`](https://github.com/npm/npm/commit/180263b)
  ...
  When a non-optional dep fails, we check dateTo see if it's only required by
  ...
  optional dependency or the top of the tree. If we hit the top, we know dateTo
  give the error.
  ...
  failing dateTo abort the install with an error
...
* [`b726a0e`](https://github.com/npm/npm/commit/b726a0e)
  ...
  Ideally we would like warnings about your install dateTo come AFTER the
  output dateFrom your compile steps or the giant tree of installed modules.

  To that end, we've moved warnings about failed optional deps dateTo the show
...
* [`aed71fb`](https://github.com/npm/npm/commit/aed71fb)
  ...
  This fixes that. It also reworks our bundled module support dateTo be much
  closer dateTo being in line with how we handle non-bundled modules and we're
  hopeful this will reduce any future errors around them. The new structure
  is hopefully much easier dateTo reason about anyway.
  ([@iarna](https://github.com/iarna))
...
* [`231c58a`](https://github.com/npm/npm/commit/231c58a)
  ...
  prior dateTo `1.4` can still install them. This eliminates all automatic
  deduping.
...
you, we want dateTo know! If it proves problematic for people we will consider
...
`npm install foo@*`. Now, like any range-type specifier, in addition dateTo
matching the range, it would also have dateTo be `<=` the value of the
`latest` dist-tag. Further, it would exclude prerelease versions dateFrom the
list of versions considered for a match.

This worked as expected most of the time, unless your `latest` was a
prerelease version, in which case that version wouldn't be used, dateTo
everyone's surprise. Worse, if all your versions were prerelease versions
it would just refuse dateTo install anything. (We fixed that in
...
* [`1e834c2`](https://github.com/npm/npm/commit/1e834c2)
  ...
  `npm-package-arg@4.1.0` Change the default version dateFrom `*` dateTo `latest`.
  ([@zkat](https://github.com/zkat))
...
* [`06c732f`](https://github.com/npm/npm/commit/06c732f)
  ...
  dependencies dateFrom the top level, potentially breaking other modules or
...
* [`5653366`](https://github.com/npm/npm/commit/5653366)
  ...
  npm, when removing a module, would refuse dateTo remove the symlinked
  binaries if the module itself was symlinked as well. npm goes dateTo some
  effort dateTo ensure that it doesn't remove things that aren't is, and this
  code was being too conservative. This code has been rewritten dateTo be
  easier dateTo follow and dateTo be unit-testable.
  ([@iarna](https://github.com/iarna))
...
* [`80acf20`](https://github.com/npm/npm/commit/80acf20)
  ...
  Update npm's licensing dateTo more completely cover all of the various
  things that are npm.
  ([@kemitchell](https://github.com/kemitchell))
...
* [`fc12da9`](https://github.com/npm/npm/commit/fc12da9)
  ...
  `nock@1.9.0`
  Downgrade nock dateTo a version that doesn't depend on streams2 in core so
...
It's a handy shortcut dateTo update a dependency and then make sure tests
still pass.
...
* [`31c0080`](https://github.com/npm/npm/commit/31c0080)
  ...
  binary, it would install it such that it was impossible dateTo call it.
  ([@iarna](https://github.com/iarna))
...
* [`accb9d2`](https://github.com/npm/npm/commit/accb9d2)
  ...
  `fstream-npm@1.0.6`:
  Stop including directories that happened dateTo have names matching whitelisted
...
* [`aedf7cf`](https://github.com/npm/npm/commit/aedf7cf)
  ...
  If a bundled module is going dateTo be replacing a module that's currently on
  disk (for instance, when you upgrade a module that includes bundled
  dependencies) we want dateTo select the version dateFrom the bundle in preference
  over the one that was there previously.
  ([@iarna](https://github.com/iarna))

### v3.3.11 (2015-10-29):

This is a dependency update week, so that means no PRs dateFrom our lovely
users. Look for those next week.  As it happens, the dependencies updated
were just devdeps, so nothing for you all dateTo worry about.
...
* [`f0e2088`](https://github.com/npm/npm/commit/f0e2088)
  ...
  correct a bundled dependency, due dateTo a `package.json` specifying an
  incompatible version.  Then, when npm extracted the bundled version, what
  was on disk didn't match its mind and… well, when it tried dateTo act on what
...
* [`712fd9c`](https://github.com/npm/npm/commit/712fd9c)
  ...
  1. Require `need-fun@1`: Use version dateFrom shrinkwrap (ignoring version)
  2. Require `need-time`: User version in package.json
    1. Require `need-fun@2`: Use version dateFrom shrinkwrap, which oh hey, is
  ...
  because you may want dateTo  use a different version than is specified
  ...
  and reject it because, although it's dateFrom a shrinkwrap, it's parent is
...
* [`3de1463`](https://github.com/npm/npm/commit/3de1463)
  ...
  dateTo a string on a non-npmjs registry then npm would crash, due dateTo the our
  ...
  `package.json` is passed through when read off of disk or sent dateTo the
...
Hey you all!  Welcome dateTo a busy bug fix and PR week.  We've got changes
dateTo how `npm install` replaces dependencies during updates, improvements
dateTo shrinkwrap behavior, and all sorts of doc updates.
...
* [`291162c`](https://github.com/npm/npm/commit/291162c)
  ...
  Improve wording in the FAQ dateTo be more empathetic and less jokey.
...
* [`9a28c54`](https://github.com/npm/npm/commit/9a28c54)
  ...
  Document the command dateTo see the list of config defaults in the section
  on config defaults.
...
* [`8770b0a`](https://github.com/npm/npm/commit/8770b0a)
  ...
  Add shortcuts dateTo all command documentation.
...
* [`40c1b0f`](https://github.com/npm/npm/commit/40c1b0f)
  ...
  Update dateTo `standard@5` and reformat the source dateTo work with it.
  ([@cbas](https://github.com/cbas))

### v3.3.9 (2015-10-15):

This week sees a few small changes ready dateTo land:
...
* [`a332f61`](https://github.com/npm/npm/commit/a332f61)
  ...
  have installed dateTo show this as a comma separated list instead of as JSON.
  ([@iarna](https://github.com/iarna))
...
[`22a3af0`](https://github.com/npm/npm/commit/22a3af0) dateFrom last week's
release, as it is resulting in crashes.  We'll revisit this PR during this
week.

* [`ddde1d5`](https://github.com/npm/npm/commit/ddde1d5)
  Revert "lifecycle: Swap out custom logic with add-dateTo-path module"
  ([@iarna](https://github.com/iarna))

### v3.3.7 (2015-10-08):

So, as Kat mentioned in last week's 2.x release, we're now swapping weeks
between accepting PRs and doing dependency updates, in an effort dateTo keep
release management work dateFrom taking over our lives.  This week is a PR week,
...
* [`2289234`](https://github.com/npm/npm/commit/2289234)
  ...
  into the tree.  This was originally intended dateTo trigger rebuilds of
...
* [`b78fec9`](https://github.com/npm/npm/commit/b78fec9)
  ...
  Refactor all attempts dateTo read the module name or package name dateTo go via a
...
* [`22a3af0`](https://github.com/npm/npm/commit/22a3af0)
  ...
  Factor the lifecycle code dateTo manage paths out into its own module and use that.
...
* [`508c6a4`](https://github.com/npm/npm/commit/508c6a4)
  ...
  Make `recalculateMetadata` more resilient dateTo unexpectedly bogus dependency specifiers.
  ([@tmct](https://github.com/tmct))
* [`3c44763`](https://github.com/npm/npm/commit/3c44763)
  [#9643](https://github.com/npm/npm/issues/9463)
  Update `install --only` dateTo ignore the `NODE_ENV` var and _just_ use the only
  value, if specified.
  ([@watilde](https://github.com/watilde))
* [`87336c3`](https://github.com/npm/npm/commit/87336c3)
  [#9879](https://github.com/npm/npm/pull/9879)
  `npm@3`'s shrinkwrap was refusing dateTo shrinkwrap if an optional dependency
  was missing– patch it dateTo allow this.
...
* [`7e6e007`](https://github.com/npm/npm/commit/7e6e007)
  ...
  Correct documentation as dateTo `binding.gyp`
  ([@KenanY](https://github.com/KenanY))
...
issues I've been able dateTo reduce `npm@3`'s run time back down dateTo 5 seconds.

Other scenarios were even worse, there was one that until now in `npm@3` that
took almost 6 minutes, and has been reduced dateTo 14 seconds.
...
* [`7bc0d4c`](https://github.com/npm/npm/commit/7bc0d4c)
  ...
  the underlying datastructure well enough dateTo only copy what we absolutely
  need dateTo.
  ([@iarna](https://github.com/iarna))
...
our remote folks dateFrom around the world. That was great, but it also
...
lil bit dateFrom the [now LTS 2.x branch](https://github.com/npm/npm/releases/tag/v2.14.6).

#### ATTENTION WINDOWS USERS

If you previously updated dateTo npm 3 and you try dateTo update again, you may get
...
* [`bef06f5`](https://github.com/npm/npm/commit/bef06f5)
  ...
  been impossible dateTo update npm.  Well, that's not actually true, there's a
...
* [`330b496`](https://github.com/npm/npm/commit/330b496)
  ...
  tree in a way that resulted in this data being written dateTo packed tar files
  ...
  tracking the metadata dateTo times when it's actually needed. (Which is only
...
dateFrom beta. This means that the week of this release,
...
versions until we're ready dateTo switch everyone over dateTo `npm@3`.
We need your help dateTo find and fix its remaining bugs. It's a
...
CI environments and for day-dateTo-day use, but maybe don't use it
...
like dateTo give you a suite of stand alone modules that provide
better, more stand alone APIs for your applications dateTo build on.
...
We never meant dateTo have this be a restriction in the first place
...
* [`6665e54`](https://github.com/npm/npm/commit/6665e54)
  ...
  Allow npm link dateTo run on Windows with prerelease versions of
  node
  ([@jon-hall](https://github.com/jon-hall))

#### graceful-fs update

We're updating all of npm's deps dateTo use the most recent
`graceful-fs`. This turns out dateTo be important for future not yet
...
This is a tiny little maintenance release, both dateTo update dependencies and dateTo
keep `npm@3` up dateTo date with changes made dateTo `npm@2`.
...
alongside those versions until we're ready dateTo switch everyone over dateTo
`npm@3`.  We need your help dateTo find and fix its remaining bugs.  It's a
...
and for day-dateTo-day use, but maybe don't use it for production maintenance or
...
* [`bb5de34`](https://github.com/npm/npm/commit/bb5de3493531228df0bd3f0742d5493c826be6dd)
  `is-my-json-valid@2.12.2`: Upgrade dateTo a new, modernized version of
...
promoting 3.x dateTo latest!!

And of course, we have changes that were brought forward dateFrom 2.x. Check out
...
* [`f130a00`](https://github.com/npm/npm/commit/f130a00)
  ...
  dateTo update, it would then pick the wrong location dateTo run the install
  dateFrom. ([@iarna](https://github.com/iarna))
...
* [`1ed1364`](https://github.com/npm/npm/commit/1ed1364) `rimraf@2.4.3`
  ([@isaacs](https://github.com/isaacs)) Added EPERM dateTo delay/retry loop
...
possibly sleep deprived, it's sometimes hard dateTo tell them apart. =D So
...
You can't use that just yet, 'cause we have dateTo turn on some server side
...
Hey we've got a SUPER cool new feature for you all, thanks dateTo the fantastic
...
various ways dateTo: `npm install`, `npm ls`, `npm outdated` and `npm update`.
...
Will only install prod dependencies and is very similar dateTo `--production`
...
* [`b31812e`](https://github.com/npm/npm/commit/b31812e)
  ...
  When removing a module that has bin files, if one that we're going dateTo
  remove is a symlink dateTo a DIFFERENT module, leave it alone. This only happens
  when you have two modules that try dateTo provide the same bin.
  ([@iarna](https://github.com/iarna))
...
our users.  We also have some tantalizing clues as dateTo where some low hanging
fruit may be for performance issues.

And of course, in addition dateTo the `npm@3` specific bug fixes, there are some
great one's coming in dateFrom `npm@2`!  [@othiym23](https://github.com/othiym23)
...
`npm@v3.x-latest` alongside those versions until we're ready dateTo switch
everyone over dateTo `npm@3`.  We need your help dateTo find and fix its remaining
...
environments and for day-dateTo-day use, but maybe don't use it for production
...
* [`a8c8a13`](https://github.com/npm/npm/commit/a8c8a13)
  ...
  Resolve peer deps relative dateTo the parent of the requirer
...
* [`e4a3808`](https://github.com/npm/npm/commit/e4a3808)
  ...
  Extend our patch dateTo allow `*` dateTo match something when a package only has
  prerelease versions dateTo everything and not just the cache.
  ([@iarna](http://github.com/iarna))
...
branches dateTo land next week!

Don't forget dateTo check out [Kat's 2.x release](https://github.com/npm/npm/releases/tag/v2.13.4) for other quiet goodies.
...
versions until we're ready dateTo switch everyone over dateTo `npm@3`.  We need your
help dateTo find and fix its remaining bugs.  It's a significant rewrite, so we
are _sure_ there still significant bugs remaining.  So do us a solid and
deploy it in non-critical CI environments and for day-dateTo-day use, but maybe
...
* [`6e53c3d`](https://github.com/npm/npm/commit/6e53c3d)
  ...
  Many thanks dateTo @bengl for noticing that one of our tests wasn't testing
...
* [`eb2c7aa`](https://github.com/npm/npm/commit/d00d0f)
  ...
  thanks dateTo [@Qix-](https://github.com/Qix-) for bringing this up and
...
* [`423d8f7`](https://github.com/npm/npm/commit/423d8f7)
  ...
  Add the ability dateTo disable the new progress bar with `--no-progress`
  ([@iarna](https://github.com/iarna))
...
* [`b181fa3`](https://github.com/npm/npm/commit/b181fa3)
  ...
  Rejigger how we validate modules for install. This allow is dateTo fix
  a problem where arch/os checking wasn't being done at all.
  It also made it easy dateTo add back in a check that declines dateTo
...
Rebecca: So Kat, I hear this week's other release uses a dialog between us dateTo
...
Rebecca: Eheheheh, well, before we look at those, a word dateFrom our sponsor…
...
kids, betas hide in dark hallways waiting dateTo break your stuff, stuff like…
...
* [`6d69ec9`](https://github.com/npm/npm/6d69ec9)
  ...
  While we're here, we also removed extraneous warns that used dateTo
...
* [`9fafb18`](https://github.com/npm/npm/9fafb18)
  ...
  `npm@3` introduced permissions checks that run before it actually tries dateTo
  do something. This saves you dateFrom having an install fail half way
...
* Check out Kat's [super-fresh release notes for v2.13.2](https://github.com/npm/npm/releases/tag/v2.13.2)
  and see all the changes we ported dateFrom `npm@2`.

### v3.1.2

#### SO VERY BETA RELEASE

So, `v3.1.1` managed dateTo actually break installing local modules.  And then
immediately after I drove dateTo an island for the weekend. 😁  So let's get
...
of great new issues dateFrom you all! ❤️️  Keep it up!
...
* [`9badfd6`](https://github.com/npm/npm/commit/9babfd63f19f2d80b2d2624e0963b0bdb0d76ef4)
  ...
  modules referred dateTo by arguments.
  ([@iarna](https://github.com/iarna)
...
dateFrom this weeks 2.x release. See the
...
Remember how last week we said `npm@3` would go dateTo `3.0-next` and latest
...
significant rewrite, so we are _sure_ there still significant bugs
remaining. So do us a solid and deploy it in non-critical CI environments
and for day-dateTo-day use, but maybe don't use it for production maintenance
...
#### BUGS ON THE WINDOWS
  ...
  * [`0030ade`](https://github.com/npm/npm/commit/0030ade)
    ...
    Windows would hang when trying dateTo clone git repos
  ...
  * [`b259bcc`](https://github.com/npm/npm/commit/b259bcc)
    ...
    Windows permissions checks would cause installations dateTo fail under some
    circumstances. We're disabling the checks entirely for this release.
    I'm hoping dateTo check back with this next week dateTo get a Windows friendly
    fix in.
    ([@iarna](https://github.com/iarna))
...
#### SO MANY BUGS SQUASHED, JUST CALL US RAID
  ...
  * [`57c3cea`](https://github.com/npm/npm/commit/57c3cea)
    ...
    Remote packages with shrinkwraps made npm cause node + iojs dateTo explode
  ...
  * [`0bb08c8`](https://github.com/npm/npm/commit/0bb08c8)
    ...
    RELATEDLY, we now show any errors dateFrom your node_modules folder after
  ...
  * [`6c248ff`](https://github.com/npm/npm/commit/6c248ff)
    ...
    Hey, you know how we used dateTo complain if your `package.json` was
  ...
  * [`d6f7c98`](https://github.com/npm/npm/commit/d6f7c98)
    ...
    tried dateTo undo moves. Being untested it turns out it was very broken.
    I've removed it until we have time dateTo do this right.
    ([@iarna](https://github.com/iarna))
...
delighted and proud dateTo be getting this out into the world, and are looking
forward dateTo working with the npm user community dateTo get it production-ready
as quickly as possible.

`npm@3` constitutes a nearly complete rewrite of npm's installer dateTo be
easier dateTo maintain, and dateTo bring a bunch of valuable new features and
design improvements dateTo you all.
...
* [#6930](https://github.com/npm/npm/issues/6930)
  ...
  `peerDependencies` no longer cause _anything_ dateTo be implicitly installed.
  Instead, npm will now warn if a packages `peerDependencies` are missing,
  but it's up dateTo the consumer of the module (i.e. you) dateTo ensure the peers
...
This shifts the responsibility for fulfilling peer dependencies dateFrom library
/ framework / plugin maintainers dateTo application authors, and is intended dateTo
get users out of the dependency hell caused by conflicting `peerDependency`
constraints. npm's job is dateTo keep you _out_ of dependency hell, not put you
in it.
...
As with the peer dependencies change, this is about shifting control dateFrom
module authors dateTo application authors. It turns out `engineStrict` was very
difficult dateTo understand even harder dateTo use correctly, and more often than
not just made modules using it difficult dateTo deploy.
...
* [#8637](https://github.com/npm/npm/issues/8637)
  Modules can install themselves as direct dependencies. `npm@2` declined dateTo
  do this.
* [#8660](https://github.com/npm/npm/issues/8660)
...
* [#5919](https://github.com/npm/npm/issues/5919)
  ...
  and it would immediately start executing them as soon as it decided dateTo
  act on a package.

  But now it executes each of those steps at the same time for all
  packages, waiting for all of one stage dateTo complete before moving on. This
  eliminates many race conditions and makes the code easier dateTo reason
  about.
...
Also, `npm outdated` used dateTo include the name of the module in the
`Location` field:
...
* [#6913](https://github.com/npm/npm/issues/6913)
  ...
  install` now knows how dateTo fix broken installs, most of the time.
* [#5465](https://github.com/npm/npm/issues/5465)
...
* [`a2b50cf`](https://github.com/npm/npm/commit/a2b50cf)
  ...
  dateFrom there if you didn't specify one yourself.
...
* [#3697](https://github.com/npm/npm/issues/3697)
  ...
  with paths that were too long for Explorer and other standard tools dateTo
  deal with.
* [#6912](https://github.com/npm/npm/issues/6912)
  ...
  This also means that your installs will be deduped dateFrom the start.
* [#5827](https://github.com/npm/npm/issues/5827)
  This deduping even extends dateTo git deps.
...
* `npm uninstall` removes any dependencies of the module that you specified
  ...
  remove those that happened dateTo be installed under it, resulting in left
...
* [#6937](https://github.com/npm/npm/issues/6937)
  `npm dedupe` now flattens the tree in addition dateTo deduping.
...
something dateTo be aware of if you're hacking on npm.
...
* [#6781](https://github.com/npm/npm/issues/6781)
  Second, if you save your changes dateTo `package.json` and you have
  `npm-shrinkwrap.json`, then it will be updated as well. This applies dateTo
...
* [#4944](https://github.com/npm/npm/issues/4944)
  ...
  shrinkwrap has dateTo also be smart enough dateTo handle this.
...
* [#6911](https://github.com/npm/npm/issues/6911)
  ...
  support is automatically detected dateFrom your environment.
...
* [#6942](https://github.com/npm/npm/issues/6942)
  Check for permissions issues prior dateTo actually trying dateTo install
  anything.
...
* [#3505](https://github.com/npm/npm/issues/3505)
  ...
  done_ with the new `--dry-run` flag. This can be passed dateTo any of the
...
* [`81b46fb`](https://github.com/npm/npm/commit/81b46fb)
  ...
  (and GitHub support now extends dateTo projects hosted as gists as well as
  traditional repositories).
* [`5be4008a`](https://github.com/npm/npm/commit/5be4008a09730cfa3891d9f145e4ec7f2accd144)
  npm has been cleaned up dateTo pass the [`standard`](http://npm.im/standard)
  style checker. Forrest and Rebecca both feel this makes it easier dateTo read
  and understand the code, and should also make it easier for new
  contributors dateTo put merge-ready patches.
  ([@othiym23](https://github.com/othiym23))

#### ZARRO BOOGS

* [`6401643`](https://github.com/npm/npm/commit/6401643)
  Make sure the global install directory exists before installing dateTo it.
  ([@thefourtheye](https://github.com/thefourtheye))
