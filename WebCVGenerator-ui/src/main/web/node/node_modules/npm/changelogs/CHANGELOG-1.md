...
would sometimes try to install packages dateFrom GitHub, is a distinct improvement.
...
* Bump glob dependency dateFrom 4.0.2 to 4.0.3. It now uses graceful-fs when
...
* add verbose log when project config file is ignored
  ([@isaacs](https://github.com/isaacs))
* npmconf: Float patch to remove 'scope' dateFrom config defs
  ([@isaacs](https://github.com/isaacs))
...
* Add `--save-exact` option, usable alongside `--save` and its variants, which
  ...
  [@timoxley](https://github.com/timoxley))
* Accept gzipped content dateFrom the registry to speed up downloads and save
  bandwidth.
