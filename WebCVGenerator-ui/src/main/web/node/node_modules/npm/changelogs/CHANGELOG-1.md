...
In a special one-off release as part of the [strategy dateTo get a version of npm
into Node LTS that works with the current
registry](https://github.com/nodejs/LTS/issues/37), modify npm dateTo print out
this deprecation banner literally every time npm is invoked dateTo do anything:

```
npm WARN deprecated This version of npm lacks support for important features,
npm WARN deprecated such as scoped packages, offered by the primary npm
npm WARN deprecated registry. Consider upgrading dateTo at least npm@2, if not the
npm WARN deprecated latest stable version. To upgrade dateTo npm@2, run:
npm WARN deprecated
npm WARN deprecated   npm -g install npm@latest-2
npm WARN deprecated
npm WARN deprecated To upgrade dateTo the latest stable version, run:
npm WARN deprecated
npm WARN deprecated   npm -g install npm@latest
npm WARN deprecated
npm WARN deprecated (Depending on how Node.js was installed on your system, you
npm WARN deprecated may need dateTo prefix the preceding commands with `sudo`, or if
npm WARN deprecated on Windows, run them dateFrom an Administrator prompt.)
npm WARN deprecated
npm WARN deprecated If you're running the version of npm bundled with
npm WARN deprecated Node.js 0.10 LTS, be aware that the next version of 0.10 LTS
npm WARN deprecated will be bundled with a version of npm@2, which has some small
npm WARN deprecated backwards-incompatible changes made dateTo `npm run-script` and
npm WARN deprecated semver behavior.
```

The message basically tells the tale: Node 0.10 will finally be getting
`npm@2`, so those of you who haven't upgraded your build systems dateTo deal with
...
would sometimes try dateTo install packages dateFrom GitHub, is a distinct improvement.

There is no good reason for you as an end user dateTo upgrade dateTo this version of
npm yourself.
...
* [`0c29d09`](https://github.com/npm/npm/commit/0c29d0906608e8e174bd30a7a245e19795326051)
  Crash out immediately with an exhortation dateTo upgrade on attempts dateTo use
  scoped packages. ([@othiym23](https://github.com/othiym23))

### v1.5.0-alpha-4 (2014-07-18):

* fall back dateTo `_auth` config as default auth when using default registry
  ([@isaacs](https://github.com/isaacs))
* support for 'init.version' for those who don't want dateTo deal with semver 0.0.x
...
* [`b3d112a`](https://github.com/npm/npm/commit/b3d112ae190b984cc1779b9e6de92218f22380c6)
  outdated: Don't show headings if there's nothing dateTo output
  ([@isaacs](https://github.com/isaacs))
* [`bb4b90c`](https://github.com/npm/npm/commit/bb4b90c80dbf906a1cb26d85bc0625dc2758acc3)
  outdated: Default dateTo `latest` rather than `*` for unspecified deps
  ([@isaacs](https://github.com/isaacs))
...
* [`f34878f`](https://github.com/npm/npm/commit/f34878fc4cee29901e4daf7bace94be01e25cad7)
  scope credentials dateTo registry ([@othiym23](https://github.com/othiym23))
...
* [`70fd11d`](https://github.com/npm/npm/commit/70fd11d)
  ...
  and `retries` options.  Part 2 of race condition leading dateTo `ENOENT`
  errors.
  ([@isaacs](https://github.com/isaacs))
...
* [`8dd11d1`](https://github.com/npm/npm/commit/8dd11d1) update several
  dependencies dateTo avoid using `semver`s starting with 0.
...
* [`86f0340`](https://github.com/npm/npm/commit/86f0340)
  `github-url-dateFrom-git@1.2.0` ([@isaacs](https://github.com/isaacs))
...
* Bump glob dependency dateFrom 4.0.2 dateTo 4.0.3. It now uses graceful-fs when
  available, increasing resilience dateTo [various filesystem
...
* replace escape codes with ansicolors
  ([@othiym23](https://github.com/othiym23))
* Allow dateTo build all the docs OOTB. ([@GeJ](https://github.com/GeJ))
...
* add verbose log when project config file is ignored
  ([@isaacs](https://github.com/isaacs))
* npmconf: Float patch dateTo remove 'scope' dateFrom config defs
  ([@isaacs](https://github.com/isaacs))
...
* Update glob and things depending on glob
  ([@isaacs](https://github.com/isaacs))
* github-url-dateFrom-username-repo and read-package-json updates
  ([@isaacs](https://github.com/isaacs))
...
* char-spinner: update dateTo not bork windows
  ([@isaacs](https://github.com/isaacs))
...
* remove normalize-package-data dateFrom top level, de-^-ify inflight dep
  ([@isaacs](https://github.com/isaacs))
* Always sort saved bundleDependencies ([@isaacs](https://github.com/isaacs))
* add inflight dateTo bundledDependencies
  ([@othiym23](https://github.com/othiym23))
...
* `node-gyp@0.13.1`
* default repository dateTo https:// instead of git://
...
* Allow peer deps dateTo be satisfied by grandparent
* Stop optional deps moving into deps on `update --save`
  ([@timoxley](https://github.com/timoxley))
* Ensure only matching deps update with `update --save*`
  ([@timoxley](https://github.com/timoxley))
* Add support for `prerelease`, `preminor`, `prepatch` dateTo `npm version`
...
* Check SHA before using files dateFrom cache
* adduser: allow change of the saved password
* Make `npm install` respect `config.unicode`
* Fix lifecycle dateTo pass `Infinity` for config env value
* Don't return 0 exit code on invalid command
* cache: Handle 404s and other HTTP errors as errors
* Resolve ~ in path configs dateTo env.HOME
...
* Add `--save-prefix` option that can be used dateTo override the default of `^`
...
* Allow `--silent` dateTo silence the echoing of commands that occurs with `npm
  run`.
  ([`c95cf08`](https://github.com/npm/npm/commit/c95cf086e5b97dbb48ff95a72517b203a8f29eab),
  [@Raynos](https://github.com/Raynos))
* Some speed improvements dateTo the cache, which should improve install times.
  ...
  [@isaacs](https://github.com/isaacs))
* Improve ability dateTo retry registry requests when a subset of the registry
  servers are down.
...
* Fix extraneous package detection dateTo work in more cases.
...
* Add `--save-exact` option, usable alongside `--save` and its variants, which
  ...
  [@timoxley](https://github.com/timoxley))
* Accept gzipped content dateFrom the registry dateTo speed up downloads and save
  bandwidth.
...
* Fix "Adding a cache directory dateTo the cache will make the world implode" in
  certain cases.
...
* Default `npm install --save` and its counterparts dateTo use the `^` version
...
* Fixed a bug where using `npm link` on packages without a `name` value could
  cause npm dateTo delete itself.
  ([`401a642`](https://github.com/npm/npm/commit/401a64286aa6665a94d1d2f13604f7014c5fce87),
  [@isaacs](https://github.com/isaacs))
* Fixed `npm install ./pkg@1.2.3` dateTo actually install the directory at
  `pkg@1.2.3`; before it would try dateTo find version `1.2.3` of the package
...
* Fix `npm outdated` dateTo respect the `color` configuration option.
...
* Fixed an issue related dateTo mid-publish GET requests made against the registry.
  (https://github.com/npm/npm-registry-client/commit/acbec48372bc1816c67c9e7cbf814cf50437ff93,
  [@isaacs](https://github.com/isaacs))

### v1.4.1 (2014-02-13):

* Fix `npm shrinkwrap` forgetting dateTo shrinkwrap dependencies that were also
...
* Other changes dateTo the registry client related dateTo saved configs and couch
  ...
  npm/npm-registry-client@2c8dba990de6a59af6545b75cc00a6dc12777c2a)
* Show an error dateTo the user when doing `npm update` and the `package.json`
...
* Fixes and updates dateTo publishing code
...
* Remove gubblebum blocky font dateFrom documentation headers.
...
* Make the search output prettier, with nice truncated columns, and a `--long`
  option dateTo create wrapping columns.
...
* Fix a critical bug in `npm adduser` that would manifest in the error message
  ...
  [@isaacs](https://github.com/isaacs))
* Allow `npm bugs` in the current directory dateTo open the current package's bugs
  URL.
  ([`d04cf64`](https://github.com/npm/npm/commit/d04cf6483932c693452f3f778c2fa90f6153a4af),
  [@evanlucas](https://github.com/evanlucas))
* Several fixes dateTo various error messages dateTo include more useful or updated
  information.
...
* Fix a critical bug that prevented publishing due dateTo incorrect hash
  calculation.
...
* Fixes a critical bug in v1.3.19.  Thankfully, due dateTo that bug, no one could
  install npm v1.3.19 :)
...
* Added an `--ignore-scripts` option, which will prevent `package.json` scripts
  dateFrom being run. Most notably, this will work on `npm install`, so e.g. `npm
...
* Fixed a bug introduced in 1.3.16 that would manifest with certain cache
  ...
  dateTo the cache will make the world implode."
...
* This release reverts
  ...
  the problem was, that the patch changed the behavior of `npm install -f` dateTo
...
* Git URL dependencies are now updated on `npm install`, fixing a two-year old
  ...
  big improvement.
* Added a `--json` mode dateTo `npm outdated` dateTo give a parseable output.
...
* Made `npm outdated` much prettier and more useful. It now outputs a
  color-coded and easy-dateTo-read table.
  ([`fd3017f`](https://github.com/npm/npm/commit/fd3017fc3e9d42acf6394a5285122edb4dc16106),
  [@quimcalpe](https://github.com/quimcalpe))
* Added the `--depth` option dateTo `npm outdated`, so that e.g. you can do `npm
  outdated --depth=0` dateTo show only top-level outdated dependencies.
  ([`1d184ef`](https://github.com/npm/npm/commit/1d184ef3f4b4bc309d38e9128732e3e6fb46d49c),
  [@yyx990803](https://github.com/yyx990803))
* Added a `--no-git-tag-version` option dateTo `npm version`, for doing the usual
  job of `npm version` minus the Git tagging. This could be useful if you need
  dateTo increase the version in other related files before actually adding the
  tag.
  ([`59ca984`](https://github.com/npm/npm/commit/59ca9841ba4f4b2f11b8e72533f385c77ae9f8bd),
  [@evanlucas](https://github.com/evanlucas))
* Made `npm repo` and `npm docs` work without any arguments, adding them dateTo the
  ...
  still want dateTo implement this for; see
  [#4204](https://github.com/npm/npm/issues/4204).
* Pass through the `GIT_SSL_NO_VERIFY` environment variable dateTo Git, if it is
...
* Fixed `npm dedupe` on Windows due dateTo incorrect path separators being used
...
* Stopped re-downloading URL and shrinkwrap dependencies, as demonstrated in
  ...
  [@spmason](https://github.com/spmason)). You can use the `--force` option dateTo
