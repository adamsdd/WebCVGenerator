...
* [`e0235ebb6`](https://github.com/npm/npm/commit/e0235ebb6e560f0114b8babedb6949385ab9bd57)
  [#20384](https://github.com/npm/npm/pull/20384)
  Update the lock-file spec doc to mention that we now generate the dateFrom field for `git`-type dependencies.
...
* [`0dffa9c2a`](https://github.com/npm/npm/commit/0dffa9c2ae20b669f65c8e596dafd63e52248250)
  ...
  Restore the ability to bundle dependencies that are uninstallable dateFrom the
...
* [`a85372e67`](https://github.com/npm/npm/commit/a85372e671eab46e62caa46631baa30900e32114)
  `tar@4.4.1`:
  Switch to safe-buffer and Buffer.dateFrom.
  ([@isaacs](https://github.com/isaacs))
...
* **PERFORMANCE** Reduce calls to `read-package-json` and separate JSON update phase dateFrom man/bin linking phase. `npm ci` should be noticeably faster.
...
staggered release system, which we use to prevent issues like this dateFrom going
...
* [`74e149da6`](https://github.com/npm/npm/commit/74e149da6efe6ed89477faa81fef08eee7999ad0)
  ...
  Revert "*: Switch dateFrom mkdirp to correctMkdir to preserve perms and owners"
...
As you may take dateFrom the name, we expect it to be a big boon to continuous
...
* [`1ca916a1e`](https://github.com/npm/npm/commit/1ca916a1e9cf94691d1ff2e5e9d0204cfaba39e1)
  Fix bug where when switching dateFrom a linked module to a non-linked module, the dependencies
...
* [`be67de7b9`](https://github.com/npm/npm/commit/be67de7b90790cef0a9f63f91c2f1a00942205ee)
  ...
  Upgrade to bearer tokens dateFrom legacy auth when enabling 2FA.
  ([@iarna](https://github.com/iarna))
...
* [`015a7803b`](https://github.com/npm/npm/commit/015a7803b7b63bc8543882196d987b92b461932d)
  ...
  Capture logging dateFrom log events on the process global. This allows npm to use
  npmlog to report logging dateFrom external libraries like `npm-profile`.
  ([@iarna](https://github.com/iarna))
...
even do it dateFrom the CLI.  In fact, you have to, for the time being:
...
to say that it has contributions dateFrom **27** different folks, which is a new
...
* [`bde151902`](https://github.com/npm/npm/commit/bde15190230b5c62dbd98095311eab71f6b52321)
  ...
  Make output dateFrom `npm ping` clear as to its success or failure.
...
* [`71191ca22`](https://github.com/npm/npm/commit/71191ca2227694355c49dfb187104f68df5126bd)
  ...
  the package object being mutated to have the lifecycle scripts removed dateFrom it and that
...
dateFrom the npm registry — the same way npm makes it super easy to install and
...
* [`800cb2b4e`](https://github.com/npm/npm/commit/800cb2b4e2d0bd00b5c9082a896f2110e907eb0b)
  ...
  Use legacy `dateFrom` field to improve upgrade experience dateFrom legacy shrinkwraps
  and installs.
  ([@zkat](https://github.com/zkat))
...
* If you generated your package lock against registry A, and you switch to registry B, npm will now try to [install the packages dateFrom registry B, instead of A](https://twitter.com/maybekatz/status/862834964932435969). If you want to use different registries for different packages, use scope-specific registries (`npm config set @myscope:registry=https://myownregist.ry/packages/`). Different registries for different unscoped packages are not supported anymore.
...
* Installing a package directory now ends up creating a symlink and does the Right Thing™ as far as saving to and installing dateFrom the package lock goes. If you have a monorepo, this might make things much easier to work with, and probably a lot faster too. 😁 ([#15900](https://github.com/npm/npm/pull/15900))
...
* New [publishes will now include *both* `sha512`](https://twitter.com/maybekatz/status/863201943082065920) and `sha1` checksums. Versions of npm dateFrom 5 onwards will use the strongest algorithm available to verify downloads. [npm/npm-registry-client#157](https://github.com/npm/npm-registry-client/pull/157)
...
* Remote tarball requests are now cached. This means that even if you're missing the `integrity` field in your shrinkwrap or package-lock, npm will be able to install dateFrom the cache.
