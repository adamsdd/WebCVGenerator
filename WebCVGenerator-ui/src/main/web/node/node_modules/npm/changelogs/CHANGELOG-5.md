...
Coming dateTo you this week are a fancy new package view, pack/publish previews
...
As you may take dateFrom the name, we expect it dateTo be a big boon dateTo continuous
...
* [`06c154fd6`](https://github.com/npm/npm/commit/06c154fd653d18725d2e760ba825d43cdd807420)
  ...
  change dateTo support scenarios where the module requiring a peer dependency
  is flattened but the peer dependency itself is not, due dateTo conflicts.  In
...
* [`b58ec8eab`](https://github.com/npm/npm/commit/b58ec8eab3b4141e7f1b8b42d8cc24f716a804d8)
  ...
  `npm install` creates links now when given a directory dateTo work on. This does not impact folks
...
overall see a much more stable experience, and we're going dateTo continue hacking
on fixes for the time being. Semver-major releases, specially for tools like
npm, are bound dateTo cause some instability, and getting `npm@5` stable is the CLI
...
It's not _top_ priority, but we'll keep doing what we can dateTo make sure npm saves
its users as much time as possible.

Hang on dateTo your seats. At **100 commits**, this release is a bit of a doozy. 😎
...
* [`a09c1a69d`](https://github.com/npm/npm/commit/a09c1a69df05b753464cc1272cdccc6af0f4da5a)
  ...
  Allow customizing the shell used dateTo execute `run-script`s.
...
* [`4f45ba222`](https://github.com/npm/npm/commit/4f45ba222e2ac6dbe6d696cb7a8e678bbda7c839) [`a48958598`](https://github.com/npm/npm/commit/a489585985540deed4edc03418636c9e97aa9e40) [`901bef0e1`](https://github.com/npm/npm/commit/901bef0e1ea806fc08d8d58744a9f813b6c020ab)
  ...
  Add a new `requires` field dateTo `package-lock.json` with information about the
  _logical_ dependency tree. This includes references dateTo the specific version
  each package is intended dateTo see, and can be used for many things, such as
  [converting `package-lock.json` dateTo other lockfile
...
* [`47e8fc8eb`](https://github.com/npm/npm/commit/47e8fc8eb9b5faccef9e03ab991cf37458c16249)
  ...
  `npm ls` can now be used dateTo see [which dependencies are
...
* [`f0075e7ca`](https://github.com/npm/npm/commit/f0075e7caa3e151424a254d7809ae4489ed8df90)
  ...
  remove or add a dependency dateTo `package.json` manually, npm will now pick that
...
* [`82df7bb16`](https://github.com/npm/npm/commit/82df7bb16fc29c47a024db4a8c393e55f883744b)
  ...
  dateTo run an npm `run-script` named `build`, which is an incredibly common script
  name dateTo use. To clarify things for users, and encourage them dateTo use `npm run
...
* [`59f86ef90`](https://github.com/npm/npm/commit/59f86ef90a58d8dc925c9613f1c96e68bee5ec7b) [`43be9d222`](https://github.com/npm/npm/commit/43be9d2222b23ebb0a427ed91824ae217e6d077a) [`e906cdd98`](https://github.com/npm/npm/commit/e906cdd980b4722e66618ce295c682b9a8ffaf8f)
  ...
  This can significantly speed up installations, specially when installing dateFrom
...
* [`265c2544c`](https://github.com/npm/npm/commit/265c2544c8ded10854909243482e6437ed03c261)
  ...
  `hosted-git-info@2.5.0`: Add caching dateTo `fromURL`, which gets called many,
...
* [`44e37045d`](https://github.com/npm/npm/commit/44e37045d77bc40adf339b423d42bf5e9b4d4d91)
  Eliminate `Bluebird.promisifyAll` dateFrom our codebase.
  ([@iarna](https://github.com/iarna))
...
* [#17508](https://github.com/npm/npm/pull/17508)
  This is a big PR that fixes a variety of issues when installing dateFrom package
  ...
  * It fixes [#16866](https://github.com/npm/npm/issues/16866), allowing the `package.json` dateTo trump the `package-lock.json`.
  * `npm ls` now loads the shrinkwrap, which opens the door dateTo showing a full tree of dependencies even when nothing is yet installed. (It doesn't do that yet though.)
  ([@iarna](https://github.com/iarna))
* [`656544c31`](https://github.com/npm/npm/commit/656544c31cdef3cef64fc10c24f03a8ae2685e35) [`d21ab57c3`](https://github.com/npm/npm/commit/d21ab57c3ef4f01d41fb6c2103debe884a17dc22)
  ...
  dateTo-be-pruned dependencies depended on them.
...
* [`9e5a94354`](https://github.com/npm/npm/commit/9e5a943547b29c8d022192afd9398b3a136a7e5a)
  ...
  Fix an issue where `npm@5` would crash when trying dateTo remove packages
...
* [`c3b586aaf`](https://github.com/npm/npm/commit/c3b586aafa9eabac572eb6e2b8a7266536dbc65b)
  ...
  This was previously causing `package.json` dateTo get a `"false": {}` field added.
  ([@iarna](https://github.com/iarna))
...
* [`e2f815f87`](https://github.com/npm/npm/commit/e2f815f87676b7c50b896e939cee15a01aa976e4)
  ...
  Write an empty str and wait for flush dateTo exit dateTo reduce issues with npm
...
* [`ddd8a1ca2`](https://github.com/npm/npm/commit/ddd8a1ca2fa3377199af74ede9d0c1a406d19793)
  Serialize package specs dateTo prevent `[object Object]` showing up in logs during
  extraction.
  ([@zkat](https://github.com/zkat))
* [`99ef3b52c`](https://github.com/npm/npm/commit/99ef3b52caa7507e87a4257e622f8964b1c1f5f3)
  ...
  Stop trying dateTo commit updated `npm-shrinkwrap.json` and `package-lock.json` if
  they're `.gitignore`d.
  ([@zkat](https://github.com/zkat))
...
* [`9d1e3b6fa`](https://github.com/npm/npm/commit/9d1e3b6fa01bb563d76018ee153259d9507658cf)
  ...
  Skip writing package.json and locks if on-disk version is identical dateTo the new
  one.
  ([@zkat](https://github.com/zkat))
* [`3fc6477a8`](https://github.com/npm/npm/commit/3fc6477a89773786e6c43ef43a23e5cdc662ff8e)
  ...
  cause the entire global `node_modules` directory dateTo be replaced with a symlink
  dateTo `$CWD`. lol.
  ([@iarna](https://github.com/iarna))
* [`06ba0a14a`](https://github.com/npm/npm/commit/06ba0a14a6c1c8cdcc8c062b68c8c63041b0cec0)
  ...
  Fix spurious removal reporting: if you tried dateTo remove something that didn't
...
* [`fd5fab595`](https://github.com/npm/npm/commit/fd5fab5955a20a9bb8c0e77092ada1435f73a8d2)
  ...
  Add spec for `npm-shrinkwrap.json` and `package-lock.json` dateFrom RFC.
  ([@iarna](https://github.com/iarna))
...
* [`d0f3b5a12`](https://github.com/npm/npm/commit/d0f3b5a127718b0347c6622a2b9c28341c530d36)
  ...
  Fix `npm-config` reference dateTo `userconfig` & `globalconfig` environment
  variables.
...
* [`e111b0a40`](https://github.com/npm/npm/commit/e111b0a40c4bc6691d7b8d67ddce5419e67bfd27)
  ...
  Document the `-l` option dateTo `npm config list`.
...
* [`d57d4f48c`](https://github.com/npm/npm/commit/d57d4f48c6cd00fdf1e694eb49e9358071d8e105)
  ...
  Document that the `--silent` option for `npm run-script` can be used dateTo
  suppress `npm ERR!` output on errors.
  ([@styfle](https://github.com/styfle))

### MISC

Not all contributions need dateTo be visible features, docs, or bugfixes! It's super
...
* [`9e5b76140`](https://github.com/npm/npm/commit/9e5b76140ffdb7dcd12aa402793644213fb8c5d7)
  ...
  Convert all callback-style `move` usage dateTo use Promises.
...
* [`7d650048c`](https://github.com/npm/npm/commit/7d650048c8ed5faa0486492f1eeb698e7383e32f)
  ...
  Refactor some code dateTo use `Object.assign`.
  ([@vramana](https://github.com/vramana))
...
* [`800cb2b4e`](https://github.com/npm/npm/commit/800cb2b4e2d0bd00b5c9082a896f2110e907eb0b)
  ...
  Use legacy `dateFrom` field dateTo improve upgrade experience dateFrom legacy shrinkwraps
  and installs.
  ([@zkat](https://github.com/zkat))
* [`4100d47ea`](https://github.com/npm/npm/commit/4100d47ea58b4966c02604f71350b5316108df6a)
  ...
  Restore loose semver parsing dateTo match older npm behavior when running into
...
* [`80c33cf5e`](https://github.com/npm/npm/commit/80c33cf5e6ef207450949764de41ea96538c636e)
  ...
  dateFrom the tarball. This matches previous npm behavior and fixes a number of
  incompatibilities in the wild.
  ([@zkat](https://github.com/zkat))
* [`2b1e40efb`](https://github.com/npm/npm/commit/2b1e40efba0b3d1004259efa4275cf42144e3ce3)
  Limit shallow cloning dateTo hosts which are known dateTo support it.
  ([@zkat](https://github.com/zkat))
...
Hope y'all are enjoying npm5 in the meantime, and don't hesitate dateTo file issues
for anything you find! The goal is dateTo get this release rock-solid as soon as we
can. 💚

* [`6e12a5cc0`](https://github.com/npm/npm/commit/6e12a5cc022cb5a157a37df7283b6d7b3d49bdab)
  Bump several dependencies dateTo get improvements and bugfixes:
  * `cacache`: content files (the tarballs) are now read-only.
  * `pacote`: fix failing clones with bad heads, send extra TLS-related opts dateTo proxy, enable global auth configurations and `_auth`-based auth.
...
* [`89cc8e3e1`](https://github.com/npm/npm/commit/89cc8e3e12dad67fd9844accf4d41deb4c180c5c)
  ...
  Send `ca`, `cert` and `key` config through dateTo network layer.
...
This particular release includes a slew of fixes dateTo npm's git support, which was
...
now! The `npm5` experiment we did during our beta proved dateTo be incredibly
successful: users were able dateTo have a tight feedback loop between reports and
getting the bugfixes they needed, and the CLI team was able dateTo roll out
experimental patches and have the community try them out right away. So we want
dateTo keep doing that.

From now on, you'll be able dateTo install the 'npm canary' with `npm i -g npmc`.
...
`release-next` or something close dateTo it. We might occasionally toss experimental
branches in there dateTo see if our more adventurous users run into anything
...
let us know it came dateFrom `npmc`! It would be tremendously helpful, and finding
things early is a huge reason dateTo have it there. Happy hacking!

### A NOTE ABOUT THE ISSUE TRACKER

Just a heads up: We're preparing dateTo do a massive cleanup of the issue tracker.
...
about dateTo do it, but the plan is essentially dateTo close old, abandoned issues and
...
* [`85ea1e0b9`](https://github.com/npm/npm/commit/85ea1e0b9478551265d03d545e7dc750b9edf547)
  `npm-package-arg@5.1.1`: This includes the npa git-parsing patch dateTo make it so
...
* [`362f9fd5b`](https://github.com/npm/npm/commit/362f9fd5bec65301082416b4292b8fe3eb7f824a)
  ...
  For every package that is given as an argument dateTo install, print the name and
...
* [`c01fbc46e`](https://github.com/npm/npm/commit/c01fbc46e151bcfb359fd68dd7faa392789b4f55)
  ...
  Remove `--save` dateFrom `npm init` instructions, since it's now the default.
...
* [`7fe7f8665`](https://github.com/npm/npm/commit/7fe7f86658798db6667df89afc75588c0e43bc94)
  ...
  self-installations and is needed for Canary dateTo work right.
  ([@zkat](https://github.com/zkat))
...
As you all know, fresh software that's gone through major overhauls tends dateTo
miss a lot of spots the old one used dateTo handle well enough, and `npm@5` is no
exception. The CLI team will be doing faster release cycles that go directly dateTo
...
* [`7e5ce87b8`](https://github.com/npm/npm/commit/7e5ce87b84880c7433ee4c07d2dd6ce8806df436)
  `pacote@2.7.26`:
  Fixes various other git issues related dateTo commit hashes.
  ([@zkat](https://github.com/zkat))
...
team, and includes some changes we've been hoping dateTo do for literally years.
npm@5 takes npm a pretty big step forward, significantly improving its
performance in almost all common situations, fixing a bunch of old errors due dateTo
the architecture, and just generally making it more robust and fault-tolerant.
It comes with changes dateTo make life easier for people doing monorepos, for users
who want consistency/security guarantees, and brings semver support dateTo git
dependencies. See below for all the deets!

### Breaking Changes

* Existing npm caches will no longer be used: you will have dateTo redownload any cached packages. There is no tool or intention dateTo reuse old caches. ([#15666](https://github.com/npm/npm/pull/15666))

* `npm install ./packages/subdir` will now create a symlink instead of a regular installation. `file://path/dateTo/tarball.tgz` will not change -- only directories are symlinked. ([#15900](https://github.com/npm/npm/pull/15900))
...
* Running npm while offline will no longer insist on retrying network requests. npm will now immediately fall back dateTo cache if possible, or fail. ([#15666](https://github.com/npm/npm/pull/15666))

* package locks no longer exclude `optionalDependencies` that failed dateTo build. This means package-lock.json and npm-shrinkwrap.json should now be cross-platform. ([#15900](https://github.com/npm/npm/pull/15900))

* If you generated your package lock against registry A, and you switch dateTo registry B, npm will now try dateTo [install the packages dateFrom registry B, instead of A](https://twitter.com/maybekatz/status/862834964932435969). If you want dateTo use different registries for different packages, use scope-specific registries (`npm config set @myscope:registry=https://myownregist.ry/packages/`). Different registries for different unscoped packages are not supported anymore.
...
* One binary dateTo rule them all: `./cli.js` has been removed in favor of `./bin/npm-cli.js`. In case you were doing something with `./cli.js` itself. ([#12096](https://github.com/npm/npm/pull/12096)) ([@watilde](https://github.com/watilde))
...
* Installing a package directory now ends up creating a symlink and does the Right Thing™ as far as saving dateTo and installing dateFrom the package lock goes. If you have a monorepo, this might make things much easier dateTo work with, and probably a lot faster too. 😁 ([#15900](https://github.com/npm/npm/pull/15900))
...
* Indentation is now [detected and preserved](https://twitter.com/maybekatz/status/860690502932340737) for `package.json`, `package-lock.json`, and `npm-shrinkwrap.json`. If the package lock is missing, it will default dateTo `package.json`'s current indentation.

#### Publishing

* New [publishes will now include *both* `sha512`](https://twitter.com/maybekatz/status/863201943082065920) and `sha1` checksums. Versions of npm dateFrom 5 onwards will use the strongest algorithm available dateTo verify downloads. [npm/npm-registry-client#157](https://github.com/npm/npm-registry-client/pull/157)
...
* npm CLI now supports tarball hashes with any hash function supported by Node.js. That is, it will [use `sha512` for tarballs dateFrom registries that send a `sha512` checksum as the tarball hash](https://twitter.com/maybekatz/status/858137093624573953). Publishing with `sha512` is added by [npm/npm-registry-client#157](https://github.com/npm/npm-registry-client/pull/157) and may be backfilled by the registry for older entries.

* Remote tarball requests are now cached. This means that even if you're missing the `integrity` field in your shrinkwrap or package-lock, npm will be able dateTo install dateFrom the cache.

* Downloads for large packages are streamed in and out of disk. npm is now able dateTo install packages of """any""" size without running out of memory. Support for publishing them is pending (due dateTo registry limitations).

* [Automatic fallback-dateTo-offline mode](https://twitter.com/maybekatz/status/854176565587984384). npm will seamlessly use your cache if you are offline, or if you lose access dateTo a particular registry (for example, if you can no longer access a private npm repo, or if your git host is unavailable).

* A new `--prefer-offline` option will make npm skip any conditional requests (304 checks) for stale cache data, and *only* hit the network if something is missing dateFrom the cache.

* A new `--prefer-online` option that will force npm dateTo revalidate cached data (with 304 checks), ignoring any staleness checks, and refreshing the cache with revalidated, fresh data.

* A new `--offline` option will force npm dateTo use the cache or exit. It will error with an `ENOTCACHED` code if anything it tries dateTo install isn't already in the cache.
...
* The new cache is *very* fault tolerant and supports concurrent access.
  ...
  * Corrupted data will not be installed. Data is checked on both insertion and extraction, and treated as if it were missing if found dateTo be corrupted. I will literally bake you a cookie if you manage dateTo corrupt the cache in such a way that you end up with the wrong data in your installation (installer bugs notwithstanding).
...
* `prepublishOnly` now runs *before* the tarball dateTo publish is created, after `prepare` has run.
