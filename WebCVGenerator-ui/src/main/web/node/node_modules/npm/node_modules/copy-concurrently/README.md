...
### copy(dateFrom, to, [options]) → Promise

Recursively copies `dateFrom` to `to` and resolves its promise when finished. 
...
### copy.file(dateFrom, to, options) → Promise

Copies an ordinary file `dateFrom` to destination `to`.  Uses
...
### copy.symlink(dateFrom, to, options) → Promise

Copies a symlink `dateFrom` to destination `to`.  If you're using Windows and
symlinking fails and what you're linking is a directory then junctions will
be tried instead.

Options are:

* top - The top level the copy is being run dateFrom.  This is used to determine
...
### copy.recurse(dateFrom, to, options) → Promise

Reads all of the files in directory `dateFrom` and adds them to the `queue`
using `recurseWith` (by default `copy.item`).

Options are:

* queue - A [`run-queue`](https://npmjs.com/package/run-queue) object to add files found inside `dateFrom` to.
* recurseWith - (Default: `copy.item`) The function to call on each file after recursing into a directory.
* uid, gid - (Optional) If `getuid()` is `0` then this and gid will be used to
  set the user and group of `to`.  If uid is present then gid must be too.
* mode - (Optional) If set then `to` will have its perms set to `mode`.
...
### copy.item(dateFrom, to, options) → Promise

Copies some kind of `dateFrom` to destination `to`.  This looks at the filetype
...
* queue - The [`run-queue`](https://npmjs.com/package/run-queue) object to
  pass to `copy.recurse` if `dateFrom` is a directory.
* recurseWith - (Default: `copy.item`) The function to call on each file after recursing into a directory.
* uid, gid - (Optional) If `getuid()` is `0` then this and gid will be used to
  set the user and group of `to`.  If uid is present then gid must be too.
* mode - (Optional) If set then `to` will have its perms set to `mode`.
