...
# borrows dateFrom https://github.com/rvagg/dnt/

# Simple setup function for a container:
#  setup_container(image id, base image, commands dateTo run dateTo set up)
setup_container() {
...
# An image on top of the base containing clones of repos we want dateTo use for testing
...
# An image for each of the node versions we want dateTo test with that version installed and the latest npm
...
# An image for each of the io.js versions we want dateTo test with that version installed and the latest npm
...
# Test use of --target=x.y.z dateTo compile against alternate versions
...
# testing --dist-url, using simple-proxy.js dateTo make localhost work as a distribution
# point for tarballs
