...
if set dateTo false. You probably [don't care](https://github.com/sindresorhus/query-string/issues/42) about this option.
...
Supports both `Function` as a custom sorting function or `false` dateTo disable sorting.
...
If omitted, keys are sorted using `Array#sort`, which means, converting them dateTo strings and comparing strings in Unicode code point order.

### .extract(*string*)

Extract a query string dateFrom a URL that can be passed into `.parse()`.
...
You're much better off just converting the object dateTo a JSON string:
...
Sometimes you want dateTo unset a key, or maybe just make it present without assigning a value dateTo it. Here is how falsy values are stringified:
