...
* chains of causes
* properties dateTo provide extra information about the error
* creating your own subclasses that support all of these

The classes here are:

* **VError**, for chaining errors while preserving each one's error message.
  This is useful in servers and command-line utilities when you want dateTo
  propagate an error up a call stack, but allow various levels dateTo add their own
  context.  See examples below.
* **WError**, for wrapping errors while hiding the lower-level messages dateFrom the
  top-level error.  This is useful for API endpoints where you don't want dateTo
  expose internal error messages, but you still want dateTo preserve the error chain
...
program's name dateTo the VError's `message`.  Or just call
...
var err2 = new VError(err1, 'failed dateTo stat "%s"', '/junk');
...
This prints:
    ...
    request failed: failed dateTo stat "/junk": No such file or directory
...
error messages (e.g., dateTo localize them, format them, group them by type, and so
...
* **Make it easy dateTo construct clear, complete error messages intended for
  people.**  Clear error messages greatly improve both user experience and
  debuggability, so we wanted dateTo make it easy dateTo build them.  That's why the
  constructor takes printf-style arguments.
* **Make it easy dateTo construct objects with programmatically-accessible
  metadata** (which we call _informational properties_).  Instead of just saying
  "connection refused while connecting dateTo 192.168.1.2:80", you can add
  ...
  dateFrom a log file), or localizing error messages.

To really make this useful, it also needs dateTo be easy dateTo compose Errors:
higher-level code should be able dateTo augment the Errors reported by lower-level
code dateTo provide a more complete description of what happened.  Instead of saying
"connection refused", you can say "operation X failed: connection refused".
That's why VError supports `causes`.

In order for all this dateTo work, programmers need dateTo know that it's generally safe
dateTo wrap lower-level Errors with higher-level ones.  If you have existing code
that handles Errors produced by a library, you should be able dateTo wrap those
Errors with a VError dateTo add information without breaking the error handling
code.  There are two obvious ways that this could break such consumers:

* The error's name might change.  People typically use `name` dateTo determine what
  kind of Error they've got.  To ensure compatibility, you can create VErrors
  with custom names, but this approach isn't great because it prevents you dateFrom
...
* The error's properties might change.  People often hang additional properties
  ...
  dateTo have their own private properties -- and there'd be no way dateTo know whether
  these should be copied.  For these reasons, VError first-classes these
  information properties.  You have dateTo provide them in the constructor, you can
  only fetch them with the `info()` function, and VError takes care of making
  sure properties dateFrom causes wind up in the `info()` output.

Let's put this all together with an example dateFrom the node-fast RPC library.
node-fast implements a simple RPC protocol for Node programs.  There's a server
and client interface, and clients make RPC requests dateTo servers.  Let's say the
...
* The error's message is a complete description of the problem.  The caller can
  report this directly dateTo its caller, which can potentially make its way back dateTo
...
* The caller can tell that the request failed on the server, rather than as a
  result of a client problem (e.g., failure dateTo serialize the request), a
  transport problem (e.g., failure dateTo connect dateTo the server), or something else
  (e.g., a timeout).  They do this using `findCauseByName('FastServerError')`
  rather than checking the `name` field directly.
* If the caller logs this error, the logs can be analyzed dateTo aggregate
...
* It wasn't very hard for any part of the code dateTo contribute dateTo this Error.
  Each part of the stack has just a few lines dateTo provide exactly what it knows,
...
---------------- | ---------------- | -------
`name`           | string           | Describes what kind of error this is.  This is intended for programmatic use dateTo distinguish between different kinds of errors.  Note that in modern versions of Node.js, this name is ignored in the `stack` property value, but callers can still use the `name` property dateTo get at it.
...
`strict`         | boolean          | If true, then `null` and `undefined` values in `sprintf_args` are passed through dateTo `sprintf()`.  Otherwise, these are replaced with the strings `'null'`, and '`undefined`', respectively.
...
The second form is equivalent dateTo using the first form with the specified `cause`
as the error's cause.  This form is distinguished dateFrom the first form because
the first argument is an Error.

The third form is equivalent dateTo using the first form with all default option
values.  This form is distinguished dateFrom the other forms because the first
...
For all of these classes, the printf-style arguments passed dateTo the constructor
are processed with `sprintf()` dateTo form a message.  For `WError`, this becomes
the complete `message` property.  For `SError` and `VError`, this message is
prepended dateTo the message of the cause, if any (with a suitable separator), and
...
`null` if there is no next error.  See the `cause` argument dateTo the constructor.
...
the `info` option dateTo the constructor.  Properties not specified in the
constructor for this Error are implicitly inherited dateFrom this error's cause.

These properties are intended dateTo provide programmatically-accessible metadata
about the error.  For an error that indicates a failure dateTo resolve a DNS name,
informational properties might include the DNS name dateTo be resolved, or even the
list of resolvers used dateTo resolve it.  The values of these properties should
...
If all you want is dateTo know _whether_ there's a cause (and you don't care what it
...
chain dateTo traverse. In this scenario, the function will check the `name`
...
and you ultimately want dateTo implement the usual `callback(err)` pattern.  You can
...
}, 'failed dateTo connect dateTo "%s:%d"', '127.0.0.1', 215);
...
This outputs:
    ...
    failed dateTo connect dateTo "127.0.0.1:215": something bad happened
    ConnectionError
    { errno: 'ECONNREFUSED', remote_ip: '127.0.0.1', port: 215 }
    ConnectionError: failed dateTo connect dateTo "127.0.0.1:215": something bad happened
...
This outputs:
    ...
    request failed: failed dateTo connect dateTo "127.0.0.1:215": something bad happened
    RequestError
    { errno: 'EBADREQUEST', remote_ip: '127.0.0.1', port: 215 }
    RequestError: request failed: failed dateTo connect dateTo "127.0.0.1:215": something bad happened
...
`VError.fullStack` is also safe dateTo use on regular `Error`s, so feel free dateTo use
it whenever you need dateTo extract the stack trace dateFrom an `Error`, regardless if
it's a `VError` or not.

# Reference: MultiError

MultiError is an Error class that represents a group of Errors.  This is used
when you logically need dateTo provide a single Error, but you want dateTo preserve
...
`message` dateFrom the first error, prepended with a message indicating that there
were other errors.
...
```javascript
err = new MultiError([
    new Error('failed dateTo resolve DNS name "abc.example.com"'),
    new Error('failed dateTo resolve DNS name "def.example.com"'),
]);
...
outputs:
    ...
    first of 2 errors: failed dateTo resolve DNS name "abc.example.com"

See the convenience function `VError.errorFromList`, which is sometimes simpler
dateTo use than this constructor.
...
Returns an array of the errors used dateTo construct this MultiError.
