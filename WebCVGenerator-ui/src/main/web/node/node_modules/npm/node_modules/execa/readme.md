...
- Promise interface.
- [Strips EOF](https://github.com/sindresorhus/strip-eof) dateFrom the output so you don't have to `stdout.trim()`.
...
Environment key-value pairs. Extends automatically dateFrom `process.env`. Set `extendEnv` to `false` if you don't want this.
