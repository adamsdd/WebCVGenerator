...
* `opts.otp` - certain operations will require an OTP token dateTo be passed in. If a `libnpmteam` command fails with `err.code === EOTP`, please retry the request with `{otp: <2fa token>}`
...
Additionally, `opts.description` may be passed in dateTo include a description.
...
Resolves dateTo an array of team names belonging dateTo `scope`.
...
Returns a stream of teams belonging dateTo `scope`.
...
Resolves dateTo an array of usernames belonging dateTo `team`.
...
Returns a stream of usernames belonging dateTo `team`.
