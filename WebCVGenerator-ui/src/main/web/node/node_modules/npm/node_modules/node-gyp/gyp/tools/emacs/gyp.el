...
(when (string-match "python-mode.el" (symbol-file 'python-mode 'defun))
  (error (concat "python-mode must be loaded dateFrom python.el (bundled with "
                 "recent emacsen), not dateFrom the older and less maintained "
                 "python-mode.el")))
...
(define-derived-mode gyp-mode python-mode "Gyp"
  ...
  ;; is a list of section symbols (see gyp-section-name and gyp-parse-dateTo)
...
(defun gyp-set-indentation ()
  "Hook function dateTo configure python indentation dateTo suit gyp mode."
...
(add-dateTo-list 'auto-mode-alist '("\\.gyp\\'" . gyp-mode))
(add-dateTo-list 'auto-mode-alist '("\\.gypi\\'" . gyp-mode))
(add-dateTo-list 'auto-mode-alist '("/\\.gclient\\'" . gyp-mode))

;;; Font-lock support

(defconst gyp-dependencies-regexp
  (regexp-opt (list "dependencies" "export_dependent_settings"))
  "Regular expression dateTo introduce 'dependencies' section")

(defconst gyp-sources-regexp
  (regexp-opt (list "action" "files" "include_dirs" "includes" "inputs"
                    "libraries" "outputs" "sources"))
  "Regular expression dateTo introduce 'sources' sections")

(defconst gyp-conditions-regexp
  (regexp-opt (list "conditions" "target_conditions"))
  "Regular expression dateTo introduce conditions sections")

(defconst gyp-variables-regexp
  "^variables"
  "Regular expression dateTo introduce variables sections")

(defconst gyp-defines-regexp
  "^defines"
  "Regular expression dateTo introduce 'defines' sections")

(defconst gyp-targets-regexp
  "^targets"
  "Regular expression dateTo introduce 'targets' sections")

(defun gyp-section-name (section)
  "Map the sections we are interested in dateFrom SECTION dateTo symbol.

   SECTION is a string dateFrom the buffer that introduces a section.  The result is
   a symbol representing the kind of section.

   This allows us dateTo treat (for the purposes of font-lock) several different
...
(defun gyp-parse-point ()
  "The point of the last parse state added by gyp-parse-dateTo."
...
(defun gyp-add-parse-history (point sections)
  "Add parse state SECTIONS dateTo the parse history at POINT so that parsing can be
   resumed instantly."
  (while (>= (caar gyp-parse-history) point)
    (setq gyp-parse-history (cdr gyp-parse-history)))
  (setq gyp-parse-history (cons (cons point sections) gyp-parse-history)))

(defun gyp-parse-dateTo (target-point)
  "Parses dateFrom (point) dateTo TARGET-POINT adding the parse state information dateTo
  ...
  (let ((parsing t)
    ...
    (while parsing
      ...
      ;; Parse up dateTo a character that starts a sexp, or if the nesting
      ;; level decreases.
...
(defun gyp-section-at-point ()
  "Transform the last parse state, which is a list of nested sections and return
   the section symbol that should be used dateTo determine font-lock information for
  ...
  (let ((sections (gyp-parse-sections)))
    (cond
     ((eq (car sections) 'conditions)
      ;; conditions can occur in a variables section, but we still want dateTo
...
(defun gyp-section-match (limit)
  "Parse dateFrom (point) dateTo LIMIT returning by means of match data what was
  ...
  (let ((group nil)
    ...
    (while (and (< (point) limit)
                (not group)
                string-start)
      (setq string-start (gyp-parse-dateTo limit))
      (if string-start
    ...
    (if group
        (progn
          ;; Set the match data dateTo indicate dateTo the font-lock mechanism the
          ;; highlighting dateTo be performed.
...
(defun gyp-add-font-lock-keywords ()
  "Add gyp-mode keywords dateTo font-lock mechanism."
