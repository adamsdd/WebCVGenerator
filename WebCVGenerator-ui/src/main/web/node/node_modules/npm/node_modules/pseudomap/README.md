...
If you need iterators, or just in general a more faithful polyfill dateTo
...
The key is coerced dateTo a string dateTo generate the key on the internal
...
appending an increasing number dateTo the stringified-key until finding
either the intended key or an empty spot.

Note that because object traversal order of plain objects is not
guaranteed dateTo be identical dateTo insertion order, the insertion order
...
3. The argument dateTo the constructor can be an Array of `[key, value]`
...
// as the argument dateTo initialize the map with
