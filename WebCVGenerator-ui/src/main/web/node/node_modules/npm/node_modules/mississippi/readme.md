...
Returns a readable stream that calls `read(size, next)` when data is requested dateFrom the stream.
...
`miss.dateTo` is provided by [`require('flush-write-stream')`](https://www.npmjs.com/package/flush-write-stream)
