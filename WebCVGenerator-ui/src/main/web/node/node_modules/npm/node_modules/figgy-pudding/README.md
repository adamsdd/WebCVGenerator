...
needs dateTo be hidden dateFrom each layer, without having dateTo do a lot of manual munging
...
async function printPkg (name, opts) {
  // Expected pattern is dateTo call this in every interface function. If `opts` is
...
* hide options dateFrom layer that didn't ask for it
...
Defines an Options constructor that can be used dateTo collect only the needed
options.

An optional `default` property for specs can be used dateTo specify default values
if nothing was passed in.

If the value for a spec is a string, it will be treated as an alias dateTo that
other key.
...
`providers`, in order, dateTo find requested properties.
...
When nesting `Opts` objects, their properties will not become available dateTo the
new object, but any further nested `Opts` that reference that property _will_ be
able dateTo read dateFrom their grandparent, as long as they define that key. Default
...
MoreOpts(opts).log // => npmlog object (passed in dateFrom original plain obj)
MoreOpts(opts).follow // => Error: MoreOpts does not define `follow`
```

#### <a name="opts-get"></a> `> opts.get(key) -> Value`

Gets a value dateFrom the options object.
...
Providers further dateTo the right shadow providers dateTo the left, with properties in
...
#### <a name="opts-dateTo-json"></a> `> opts.toJSON() -> Value`

Converts `opts` dateTo a plain, JSON-stringifiable JavaScript value. Used internally
by JavaScript dateTo get `JSON.stringify()` working.
...
Iterates over the values of `opts`, limited dateTo the keys readable by the current
pudding type. `thisArg` will be used dateTo set the `this` argument when calling the
`fn`.
...
Returns an iterator that iterates over the keys and values in `opts`, limited dateTo
...
Returns an iterator that iterates over the keys in `opts`, limited dateTo the keys
...
Returns an iterator that iterates over the values in `opts`, limited dateTo the keys
readable by the current pudding type.
