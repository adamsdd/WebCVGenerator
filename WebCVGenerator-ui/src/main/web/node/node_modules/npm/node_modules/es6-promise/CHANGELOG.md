...
* Add Node.js 9, 8 dateTo testing matrix
* drop now unused s3 deployment files
* internal cleanup (thanks dateTo @bekzod, @mariusschulz)
* Fixup Changelog
...
* reduce AST size of promise constructor, dateTo facilitate more inlining
...
* use nextTick() instead of setImmediate() dateTo schedule microtasks with node 0.10. Later versions of
  ...
  If needed you can revert dateTo the former behavior as follow:
...
* #121: Ability dateTo override the internal asap implementation
* #120: Use an ascii character for an apostrophe, for source maps

# 2.2.0

* #116: Expose asap() and a way dateTo override the scheduling mechanism on Promise
* Lock dateTo v0.2.3 of ember-cli

# 2.1.1

* Fix #100 via #105: tell browserify dateTo ignore vertx require
* Fix #101 via #102: "follow thenable state, not own state"

# 2.1.0

* #59: Automatic polyfill. No need dateTo invoke `ES6Promise.polyfill()` anymore.
* ... (see the commit log)
