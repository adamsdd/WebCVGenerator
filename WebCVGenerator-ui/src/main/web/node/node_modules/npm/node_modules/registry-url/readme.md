...
Use this if you do anything with the npm registry as users will expect it dateTo use their configured registry.
...
If the provided scope is not in the user's `.npmrc` file, then `registry-url` will check for the existence of `registry`, or if that's not set, fallback dateTo the default npm registry.
