...
The `byline` module can be used as a function dateTo quickly wrap a readable stream:
...
You just need dateTo add one line dateTo wrap your readable `Stream` with a `LineStream`.
...
Node v0.10 added a new streams2 API. This allows the stream dateTo be used in non-flowing mode and is
...
By default byline skips empty lines, if you want dateTo keep them, pass the `keepEmptyLines` option in
the call dateTo `byline.createStream(stream, options)` or `byline(stream, options)`.
...
If you want dateTo use `node-byline` with node v0.8 then you can use the 2.1.x series. Simply use the
...
- limitations dateTo only file streams
- CoffeeScript
- unnecessary code
