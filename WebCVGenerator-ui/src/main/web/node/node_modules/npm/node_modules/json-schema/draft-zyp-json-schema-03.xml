...
<rfc category="info" docName="draft-zyp-json-schema-03" ipr="trust200902">
	<front>
		...
		<abstract>
			<t>
				...
				data is required for a given application and how dateTo interact with it. JSON
				Schema is intended dateTo define validation, documentation, hyperlink
	...
	<middle>
		...
		<section title="Conventions">
			<t>
				<!-- The text in this section has been copied dateFrom the official boilerplate,
				and should not be modified.-->
				
				The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", 
				"SHOULD NOT", "RECOMMENDED", "MAY", and "OPTIONAL" in this document are dateTo be
		...
		<section title="Overview">
			...
			<t>
				...
				hyperlinks.
				Hyper Schema builds on JSON Schema dateTo describe the hyperlink structure of
				other JSON documents and elements of interaction. This allows user agents dateTo be able dateTo successfully navigate
				JSON documents based on their schemas.
			</t>
			<t>
				Cumulatively JSON Schema acts as a meta-document that can be used dateTo define the required type and constraints on
	...
	"links": [{
		...
		"href": "comments/?id={id}"
			...
			<section title="Terminology">
				<t>
					For this specification, <spanx style="strong">schema</spanx> will be used dateTo denote a JSON Schema
					definition, and an <spanx style="strong">instance</spanx> refers dateTo a JSON value that the schema
			...
			<section title="Design Considerations">
				<t>
					The JSON Schema media type does not attempt dateTo dictate the structure of JSON
					...
					JSON representation. Attempting dateTo force a specific structure is generally
				...
				<t>
					...
					representations linked dateTo by JSON representations, and modification of
					those resources. The goal of this
					format is dateTo sufficiently describe JSON structures such that one can
					utilize existing information available in existing JSON
					representations dateFrom a large variety of services that leverage a representational state transfer
		...
		<section title="Schema/Instance Association">
			<t>
				JSON Schema instances are correlated dateTo their schema by the "describedby"
				relation, where the schema is defined dateTo be the target of the relation.
				...
				representation should specify the relation dateTo the schema is beyond the normative scope
				...
				relation dateTo the schema that describes the meaning of a JSON instance's (or collection
...
Link: <http://json.com/my-hyper-schema>; rel="describedby"
				...
				Instances MAY specify multiple schemas, dateTo indicate all the schemas that
				are applicable dateTo the data, and the data SHOULD be valid by all the schemas.
				...
				instances dateFrom different schemas. When collections contain heterogeneous
				instances, the "pathStart" attribute MAY be specified in the 
				schema dateTo disambiguate which schema should be applied for each item in the
				collection. However, ultimately, the mechanism for referencing a schema is up dateTo the
				media type of the instance documents (if they choose dateTo specify that schemas
				can be referenced).
			</t>
			...
			<section title="Self-Descriptive Schema">
				<t>
					...
					SHOULD include a MIME parameter that refers dateTo the self-descriptive
...
<![CDATA[
{
	...
	"properties": {
		...
		"age": {
			...
			<section title="type">
				<t>
					This attribute defines what the primitive type or the schema of the instance MUST be in order dateTo validate.
					...
					<list style="hanging">
						<t hangText="Simple Types">
							...
							<list style="hanging">
								...
								<t hangText="null">Value MUST be null. Note this is mainly for purpose of being able use union types dateTo define nullability. If this type is not included in a union, null values are not allowed (the primitives listed above do not allow nulls on their own).</t>
...
<![CDATA[
{
	"type": ["string", "number"]
			...
			<section title="properties" anchor="properties">
				<t>This attribute is an object with property definitions that define the valid values of instance object property values. When the instance value is an object, the property values of the instance object MUST conform dateTo the property definitions in this object. In this object, each property definition's value MUST be a schema, and the property's name MUST be the name of the instance property that it defines. The instance property value MUST be valid according dateTo the schema dateFrom the property definition. Properties are considered unordered, the order of the instance properties MAY be in any order.</t>
			</section>
			...
			<section title="items">
				...
				<t>When this attribute value is a schema and the instance value is an array, then all the items in the array MUST be valid according dateTo the schema.</t>
				<t>When this attribute value is an array of schemas and the instance value is an array, each position in the instance array MUST conform dateTo the schema in the corresponding position for this array. This called tuple typing. When tuple typing is used, additional items are allowed, disallowed, or constrained by the <xref target="additionalItems">"additionalItems"</xref> attribute using the same rules as <xref target="additionalProperties">"additionalProperties"</xref> for objects.</t>
			</section>
			
			<section title="additionalItems" anchor="additionalItems">
				<t>This provides a definition for additional items in an array instance when tuple definitions of the items is provided. This can be false dateTo indicate additional items in the array are not allowed, or it can be a schema that defines the schema of the additional items.</t>
			</section>
			...
			<section title="dependencies">
				<t>This attribute is an object that defines the requirements of a property on an instance object. If an object instance has a property with the same name as a property in this attribute's object, then the instance must be valid against the attribute's property value (hereafter referred dateTo as the "dependency value").</t>
				<t>
			...
			<section title="exclusiveMinimum">
				<t>This attribute indicates if the value of the instance (if the instance is a number) can not equal the number defined by the "minimum" attribute. This is false by default, meaning the instance value can be greater then or equal dateTo the minimum value.</t>
			</section>
			
			<section title="exclusiveMaximum">
				<t>This attribute indicates if the value of the instance (if the instance is a number) can not equal the number defined by the "maximum" attribute. This is false by default, meaning the instance value can be less then or equal dateTo the maximum value.</t>
			</section>
			...
			<section title="uniqueItems" anchor="uniqueItems">
				...
				<t>
					...
					<list>
						...
						<t>are arrays, contains the same number of items, and each item in the array is equal dateTo the corresponding item in the other array; or</t>
						<t>are objects, contains the same property names, and each property in the object is equal dateTo the corresponding property in the other object.</t>
					</list>
				</t>
			</section>
			
			<section title="pattern">
				<t>When the instance value is a string, this provides a regular expression that a string instance MUST match in order dateTo be valid. Regular expressions SHOULD follow the regular expression specification dateFrom ECMA 262/Perl 5</t>
			</section>
			...
			<section title="enum">
				<t>This provides an enumeration of all possible values that are valid for the instance property. This MUST be an array, and each item in the array represents a possible value for the instance value. If this attribute is defined, the instance value MUST be one of the values in the array in order for the schema dateTo be valid. Comparison of enum values uses the same algorithm as defined in <xref target="uniqueItems">"uniqueItems"</xref>.</t>
			</section>
			...
			<section title="format">
				<t>This property defines the type of data, content type, or microformat dateTo be expected in the instance property values. A format attribute MAY be one of the values listed below, and if so, SHOULD adhere dateTo the semantics describing for the format. A format SHOULD only be used dateTo give meaning dateTo primitive types (string, integer, number, or boolean). Validators MAY (but are not required dateTo) validate that the instance values conform dateTo a format.</t>
				...
				<t>
					...
					<list style="hanging">
						...
						<t hangText="date">This SHOULD be a date in the format of YYYY-MM-DD. It is recommended that you use the "date-time" format instead of "date" unless you need dateTo transfer only the date part.</t>
						<t hangText="time">This SHOULD be a time in the format of hh:mm:ss. It is recommended that you use the "date-time" format instead of "time" unless you need dateTo transfer only the time part.</t>
						...
						<t hangText="regex">A regular expression, following the regular expression specification dateFrom ECMA 262/Perl 5.</t>
			...
			<section title="extends">
				<t>The value of this property MUST be another schema which will provide a base schema which the current schema will inherit dateFrom. The inheritance rules are such that any instance that is valid according dateTo the current schema MUST be valid according dateTo the referenced schema. This MAY also be an array, in which case, the instance MUST be valid for all the schemas in the array. A schema that extends another schema MAY define additional attributes, constrain existing attributes, or add other constraints.</t>
				<t>
	...
	"extends": {"$ref": "http://json-schema.org/draft-03/schema"}
			...
			<section title="id">
				<t>
					...
					parent schema, the current URI of the parent schema is held dateTo be the
					URI under which this schema was addressed. If id is missing, the current URI of a schema is
					defined dateTo be that of the parent schema. The current URI of the schema
					is also used dateTo construct relative references such as for $ref.
				</t>
			</section>
			...
			<section title="$schema">
				...
				<t>
					A validator MAY use this attribute's value dateTo determine which version of JSON Schema the current schema is written in, and provide the appropriate validation features and behavior.
					Therefore, it is RECOMMENDED that all schema authors include this attribute in their schemas dateTo prevent conflicts with future JSON Schema specification changes.
				</t>
			</section>
		</section>
		...
		<section title="Hyper Schema" anchor="hyper-schema">
			<t>
				The following attributes are specified in addition dateTo those
			...
			<section title="links" anchor="links">
				...
				<section title="Link Description Object">
					<t>
						A link description object is used dateTo describe link relations. In
					...
					<section title="href">
						<t>
							...
							and MAY be a relative URI. The base URI dateTo be used for relative resolution
							SHOULD be the URI used dateTo retrieve the instance object (not the schema)
							...
							in the template (otherwise they MAY be provided dateFrom alternate sources, like user input).
						</t>
						...
						<t>
							...
							by using the text between the braces dateTo denote the property name
							dateFrom the instance dateTo get the value dateTo substitute.
...
http://somesite.com/{id}
								...
								<postamble>Then it would be resolved by replace the value of the "id" property value dateFrom the instance object.</postamble>
							</figure>
...
http://somesite.com/45
							...
							actual instance value SHOULD be used dateTo replace the braces, rather than a property value.
					...
					<section title="rel">
						<t>
							...
							relation dateTo the target resource. The relation dateTo the target SHOULD be interpreted as specifically dateFrom the instance object that the schema (or sub-schema) applies dateTo, not just the top level resource that contains the object within its hierarchy. If a resource JSON representation contains a sub object with a property interpreted as a link, that sub-object holds the relation with the target. A relation dateTo target dateFrom the top level resource MUST be indicated with the schema describing the top level JSON representation.
						</t>
						
						<t>
							Relationship definitions SHOULD NOT be media type dependent, and users are encouraged dateTo utilize existing accepted relation definitions, including those in existing relation registries (see <xref target="RFC4287">RFC 4287</xref>). However, we define these relations here for clarity of normative interpretation within the context of JSON hyper schema defined relations:
							...
							<list style="hanging">
								...
								<t hangText="describedby">
									This indicates the target of the link is the schema for the instance object. This MAY be used dateTo specifically denote the schemas of objects within a JSON object hierarchy, facilitating polymorphic type data structures.
								</t>
						...
						<t>
							The following relations are applicable for schemas (the schema as the "dateFrom" resource in the relation):

							<list style="hanging">
								<t hangText="instances">This indicates the target resource that represents collection of instances of a schema.</t>
								<t hangText="create">This indicates a target dateTo use for creating new instances of a schema. This link definition SHOULD be a submission link with a non-safe method (like POST).</t>
							</list>
						</t>
...
[{
	...
	"upId": "parent"
							...
							(self) URI would resolve dateTo "/Resource/thing" and the first item's "up"
							relation SHOULD be resolved dateTo the resource at "/Resource/parent".
					...
					<section title="Submission Link Properties">
						<t>
							The following properties also apply dateTo link definition objects, and
							provide functionality analogous dateTo HTML forms, in providing a
							means for submitting extra (often user supplied) information dateTo send dateTo a server.
						</t>
						
						<section title="method">
							<t>
								This attribute defines which method can be used dateTo access the target resource.
								...
								accessed resources, and do not need dateTo be defined here).
								This defaults dateTo "GET".
							</t>
						</section>
						...
						<section title="enctype">
							<t>
								...
								supports for querying or posting dateTo the collection of instances at the target
								resource. The query can be 
								suffixed dateTo the target URI dateTo query the collection with
								property-based constraints on the resources that SHOULD be returned dateFrom
								the server or used dateTo post data dateTo the resource (depending on the method).
...
/Product/?name=Slinky
			...
			<section title="fragmentResolution">
				<t>
					This property indicates the fragment resolution protocol dateTo use for
					resolving fragment identifiers in URIs within the instance
					representations. This applies dateTo the instance object URIs and all
				...
				<t>
					...
					mechanism for resolving references dateTo entities within a document.
				</t>
				...
				<section title="slash-delimited fragment resolution">
					<t>
						...
						reference token SHOULD be interpreted, starting dateFrom the beginning of
						...
						starting with the root of the JSON structure dateFrom the representation of
					...
					<t>
						...
						property name MUST be encoded dateTo avoid being interpreted as a property
						delimiter.
					</t>
...
#/foo                    the object referred dateTo by the foo property
#/foo/another%20prop     the object referred dateTo by the "another prop"
                         property of the object referred dateTo by the
                         "foo" property
#/foo/another%20prop/baz the string referred dateTo by the value of "baz"
                         property of the "another prop" property of 
                         the object referred dateTo by the "foo" property
#/foo/anArray/0          the first object in the "anArray" array
				...
				<section title="dot-delimited fragment resolution">
					<t>
						...
						the path does not need dateTo start with a ".". For example, #.foo and #foo are a valid fragment
			...
			<section title="readonly">
				<t>This attribute indicates that the instance property SHOULD NOT be changed. Attempts by a user agent dateTo modify the value of this property are expected dateTo be rejected by a server.</t>
			</section>
			...
			<section title="pathStart">
				<t>
					This attribute is a URI that defines what the instance's URI MUST start with in order dateTo validate.
					The value of the "pathStart" attribute MUST be resolved as per <xref target="RFC3986">RFC 3986, Sec 5</xref>, 
					and is relative dateTo the instance's URI.
				</t>
				...
				<t>
					...
					instance, this schema SHOULD NOT be applied dateTo the instance. Any schema
					that does not have a pathStart attribute SHOULD be considered applicable 
					dateTo all the instances for which it is referenced.
				</t>
			</section>
		...
		<section title="Security Considerations">
			<t>
				...
				is used dateTo denote a full representation of an object, the user agent
				SHOULD NOT consider the representation dateTo be the authoritative representation
				of the resource denoted by the target URI if the target URI is not
				equivalent dateTo or a sub-path of the the URI used dateTo request the resource
...
<![CDATA[
{
	"links": [{
		...
		"href": "{id}"
				...
				<figure>
					<preamble>And a resource was requested dateFrom somesite.com:</preamble>
					<artwork>
<![CDATA[
GET /foo/
]]>
...
[{
	...
	"name": "This representation should not be treated as \
		...
		dateFrom '/baz' dateTo ensure it has the authoritative representation"
}, {
	...
	"name": "This representation\
		...
		<section title="IANA Considerations">
			...
			<t>
				...
				refers dateTo the schema used dateTo define the structure of this structure (the
				meta-schema). Normally the value would be http://json-schema.org/draft-03/hyper-schema,
				but it is allowable dateTo use other schemas that extend the hyper schema's meta-
				schema.
			</t>
			<t>Optional parameters: pretty</t>
			<t>The value of the pretty parameter MAY be true or false dateTo indicate if additional whitespace has been included dateTo make the JSON representation easier dateTo read.</t>
			...
			<section title="Registry of Link Relations">
				<t>
					...
					assignments are subject dateTo IESG Approval, as outlined in <xref target="RFC5226">RFC 5226</xref>.
					Requests should be made by email dateTo IANA, which will then forward the
					request dateTo the IESG, requesting approval.
				</t>
			</section>
		</section>
	</middle>
	...
	<back>
		...
		<section title="Change Log">
			<t>
				<list style="hanging">
					<t hangText="draft-03">
						<list style="symbols">
							<t>Added example and verbiage dateTo "extends" attribute.</t>
							<t>Defined slash-delimited dateTo use a leading slash.</t>
							<t>Made "root" a relation instead of an attribute.</t>
							<t>Removed address values, and MIME media type dateFrom format dateTo reduce confusion (mediaType already exists, so it can be used for MIME types).</t>
							...
							<t>Replaced the link submission "properties" attribute dateTo "schema" attribute.</t>
							...
							<t>Moved "contentEncoding" attribute dateTo hyper schema.</t>
							<t>Added "additionalItems" attribute.</t>
							<t>Added "id" attribute.</t>
							<t>Switched self-referencing variable substitution dateFrom "-this" dateTo "@" dateTo align with reserved characters in URI template.</t>
					...
					<t hangText="draft-02">
						<list style="symbols">
							...
							<t>Added "targetSchema" attribute dateTo link description object.</t>
						</list>
					</t>
					
					<t hangText="draft-01">
						<list style="symbols">
							<t>Fixed category and updates dateFrom template.</t>
						</list>
					</t>
		...
		<section title="Open Issues">
			<t>
				<list>
					<t>Should we give a preference dateTo MIME headers over Link headers (or only use one)?</t>
					<t>Should "root" be a MIME parameter?</t>
					<t>Should "format" be renamed dateTo "mediaType" or "contentType" dateTo reflect the usage MIME media types that are allowed?</t>
