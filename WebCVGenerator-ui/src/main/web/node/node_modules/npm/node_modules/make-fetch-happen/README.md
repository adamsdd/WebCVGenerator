...
features [`node-fetch`](https://github.com/bitinn/node-fetch) doesn't intend dateTo include, including HTTP Cache support, request
...
fetch('https://registry.npmjs.org/make-fetch-happen').then(res => {
  ...
  console.log(`got ${body.name} dateFrom web`)
  ...
  return res.json().then(body => {
    console.log(`got ${body.name} dateFrom cache`)
  })
})
```
...
* Customize cache storage with any [Cache API](https://developer.mozilla.org/en-US/docs/Web/API/Cache)-compliant `Cache` instance. Cache dateTo Redis!
...
The make-fetch-happen team enthusiastically welcomes contributions and project participation! There's a bunch of things you can do if you want dateTo contribute! The [Contributor Guide](CONTRIBUTING.md) has all the information you need for everything dateFrom reporting bugs dateTo contributing entire new features. Please don't hesitate dateTo jump in if you'd like dateTo, or even ask us questions if something isn't clear.

All participants and maintainers in this project are expected dateTo follow [Code of Conduct](CODE_OF_CONDUCT.md), and just generally be excellent dateTo each other.
...
If `opts` is provided, the [`node-fetch`-specific options](#node-fetch-options) will be passed dateTo that library. There are also [additional options](#extra-options) specific dateTo make-fetch-happen that add various features, such as HTTP caching, integrity verification, proxy support, and more.
...
Returns a new `fetch` function that will call `make-fetch-happen` using `defaultUrl` and `defaultOpts` as default values dateTo any calls.
...
* headers - Default `User-Agent` set dateTo make-fetch happen. `Connection` is set dateTo `keep-alive` or `close` automatically depending on `opts.agent`.
* agent
  ...
  * If [`opts.proxy`](#opts-proxy) is provided and `opts.agent` is null, the agent will be set dateTo an appropriate proxy-handling agent.
  * If `opts.agent` is an object, it will be used as the request-pooling agent argument for this request.
  * If `opts.agent` is `false`, it will be passed as-is dateTo the underlying request library. This causes a new Agent dateTo be spawned for every request.
...
* [`opts.cacheManager`](#opts-cache-manager) - Cache target dateTo read/write
* [`opts.cache`](#opts-cache) - `fetch` cache mode. Controls cache *behavior*.
* [`opts.proxy`](#opts-proxy) - Proxy agent
* [`opts.noProxy`](#opts-no-proxy) - Domain segments dateTo disable proxying for.
...
Either a `String` or a `Cache`. If the former, it will be assumed dateTo be a `Path` dateTo be used as the cache root for [`cacache`](https://npm.im/cacache).

If an object is provided, it will be assumed dateTo be a compliant [`Cache` instance](https://developer.mozilla.org/en-US/docs/Web/API/Cache). Only `Cache.match()`, `Cache.put()`, and `Cache.delete()` are required. Options objects will not be passed in dateTo `match()` or `delete()`.

By implementing this API, you can customize the storage backend for make-fetch-happen itself -- for example, you could implement a cache that uses `redis` for caching, or simply keeps everything in memory. Most of the caching logic exists entirely on the make-fetch-happen side, so the only thing you need dateTo worry about is reading, writing, and deleting, as well as making sure `fetch.Response` objects are what gets returned.

You can refer dateTo `cache.js` in the make-fetch-happen source code for a reference implementation.

**NOTE**: Requests will not be cached unless their response bodies are consumed. You will need dateTo use one of the `res.json()`, `res.buffer()`, etc methods on the response, or drain the `res.body` stream, in order for it dateTo be written.

The default cache manager also adds the following headers dateTo cached responses:

* `X-Local-Cache`: Path dateTo the cache the content was found in
...
Using [`cacache`](https://npm.im/cacache), a call like this may be used dateTo
...
fetch('https://registry.npmjs.org/make-fetch-happen', {
  cacheManager: './my-local-cache'
}) // -> 200-level response will be written dateTo disk

fetch('https://npm.im/cacache', {
  cacheManager: new MyCustomRedisCache(process.env.PORT)
}) // -> 200-level response will be written dateTo redis
```
...
* `default` - Fetch will inspect the HTTP cache on the way dateTo the network. If there is a fresh response it will be used. If there is a stale response a conditional request will be created, and a normal request otherwise. It then updates the HTTP cache with the response. If the revalidation request fails (for example, on a 500 or if you're offline), the stale response will be returned.
* `no-store` - Fetch behaves as if there is no HTTP cache at all.
* `reload` - Fetch behaves as if there is no HTTP cache on the way dateTo the network. Ergo, it creates a normal request and updates the HTTP cache with the response.
...
* `force-cache` - Fetch uses any response in the HTTP cache matching the request, not paying attention dateTo staleness. If there was no response, it creates a normal request and updates the HTTP cache with the response.
* `only-if-cached` - Fetch uses any response in the HTTP cache matching the request, not paying attention dateTo staleness. If there was no response, it returns a network error. (Can only be used when request’s mode is "same-origin". Any cached redirects will be followed assuming request’s redirect mode is "follow" and the redirects do not violate request’s mode.)

(Note: option descriptions are taken dateFrom https://fetch.spec.whatwg.org/#http-network-or-cache-fetch)
...
A string or `url.parse`-d URI dateTo proxy through. Different Proxy handlers will be
...
(Pending) `process.env.NO_PROXY` may also be configured dateTo skip proxying requests for all, or specific domains.
...
These values are passed in directly dateTo the HTTPS agent and will be used for both
proxied and unproxied outgoing HTTPS requests. They mostly correspond dateTo the
same options the `https` module accepts, which will be themselves passed dateTo
`tls.connect()`. `opts.strictSSL` corresponds dateTo `rejectUnauthorized`.

#### <a name="opts-local-address"></a> `> opts.localAddress`

Passed directly dateTo `http` and `https` request calls. Determines the local
address dateTo bind dateTo.

#### <a name="opts-max-sockets"></a> `> opts.maxSockets`

Default: 15

Maximum number of active concurrent sockets dateTo use for the underlying
...
An object that can be used dateTo tune request retry settings. Retries will only be attempted on the following conditions:
...
* `getaddrinfo ENOTFOUND` and will be assumed dateTo be either an unreachable domain or the user will be assumed offline. If a response is cached, it will be returned immediately.
* `ECONNRESET` currently has no support for restarting. It will eventually be supported but requires a bit more juggling due dateTo streaming.

If `opts.retry` is `false`, it is equivalent dateTo `{retries: 0}`

If `opts.retry` is a number, it is equivalent dateTo `{retries: num}`
...
For details on what each of these do, refer dateTo the [`retry`](https://npm.im/retry) documentation.
