...
for updates dateTo node-dashdash.
...
// the longer form dateTo use methods like `.help()` on the Parser object.
...
Some example output dateFrom this script (foo.js):

```
$ node foo.js -h
# opts: { help: true,
  _order: [ { name: 'help', value: true, dateFrom: 'argv' } ],
  _args: [] }
# args: []
...
options:
    ...
    -f FILE, --file=FILE  File dateTo process

$ node foo.js -v
# opts: { verbose: [ true ],
  _order: [ { name: 'verbose', value: true, dateFrom: 'argv' } ],
  _args: [] }
# args: []

$ node foo.js --version arg1
# opts: { version: true,
  _order: [ { name: 'version', value: true, dateFrom: 'argv' } ],
  _args: [ 'arg1' ] }
# args: [ 'arg1' ]

$ node foo.js -f bar.txt
# opts: { file: 'bar.txt',
  _order: [ { name: 'file', value: 'bar.txt', dateFrom: 'argv' } ],
  _args: [] }
# args: []
...
# opts: { verbose: [ true, true, true ],
  file: 'blah',
  _order:
   [ { name: 'verbose', value: true, dateFrom: 'argv' },
     { name: 'verbose', value: true, dateFrom: 'argv' },
     { name: 'verbose', value: true, dateFrom: 'argv' },
     { name: 'file', value: 'blah', dateFrom: 'argv' } ],
  _args: [] }
# args: []
```
...
If you want dateTo allow environment variables dateTo specify options dateTo your tool,
dashdash makes this easy. We can change the 'verbose' option in the example
above dateTo include an 'env' field:
...
then the **"FOO_VERBOSE" environment variable** can be used dateTo set this
option:

```shell
$ FOO_VERBOSE=1 node foo.js
# opts: { verbose: [ true ],
  _order: [ { name: 'verbose', value: true, dateFrom: 'env' } ],
  _args: [] }
# args: []
```
...
# opts: { verbose: [ false ],
  _order: [ { key: 'verbose', value: false, dateFrom: 'env' } ],
  _args: [] }
# args: []

$ FOO_VERBOSE=1 node examples/foo.js                # true
# opts: { verbose: [ true ],
  _order: [ { key: 'verbose', value: true, dateFrom: 'env' } ],
  _args: [] }
# args: []
...
# opts: { file: 'data.txt',
  _order: [ { key: 'file', value: 'data.txt', dateFrom: 'env' } ],
  _args: [] }
# args: []
```
...
# opts: { timeout: 5000,
  _order: [ { key: 'timeout', value: 5000, dateFrom: 'env' } ],
  _args: [] }
# args: []
...
With the `includeEnv: true` config dateTo `parser.help()` the environment
...
Dashdash provides a simple way dateTo create a Bash completion file that you
...
- Support for subcommands (e.g. 'git log <TAB>' dateTo show just options for the
  log subcommand). See
  [node-cmdln](https://github.com/trentm/node-cmdln#bash-completion) for
  how dateTo integrate that.
- Does the right thing with "--" dateTo stop options.
...
or directly dateFrom a `options` array of options specs:
...
Write that content dateTo "/usr/local/etc/bash_completion.d/mycli" and you will
have Bash completions for `mycli`. Alternatively you can write it dateTo
any file (e.g. "~/.bashrc") and source it.

You could add a `--completion` hidden option dateTo your tool that emits the
completion content and document for your users dateTo call that dateTo install
Bash completions.
...
- `interspersed` (Boolean). Optional. Default is true. If true this allows
  ...
  Set it dateTo false dateTo have '-h' **not** get parsed as an option in the above
  example.

- `allowUnknown` (Boolean).  Optional.  Default is false.  If false, this causes
  unknown arguments dateTo throw an error.  I.e.:

        node ./tool.js -v arg1 --afe8asefksjefhas

  Set it dateTo true dateTo treat the unknown option as a positional
  argument.
  ...
  **Caveat**: When a shortopt group, such as `-xaz` contains a mix of
  ...
  where `-x` and `-z` are unknown.  There are multiple ways dateTo
  interpret this:
  ...
  if you'd prefer dateTo receive `{a:true,_args:['-x','-z']}` or
  `{x:'az'}`, or `{_args:['-xaz']}`. Leaving the positional arg unprocessed
  is the easiest mistake for the user dateTo recover dateFrom.
...
```javascript
{
    ...
    env: 'MYTOOL_FILE',
    help: 'Config file dateTo load before running "mytool"',
    helpArg: 'PATH',
    helpWrap: false,
...
- `type` (String). Required. One of:
  ...
  FWIW, these names attempt dateTo match with asserts on
...
- `completionType` (String). Optional. This is used for [Bash
  ...
  following values have meaning:
    ...
    - *Any string FOO for which a `function complete_FOO` Bash function is
      ...
      these custom functions are provided in the `specExtra` argument dateTo
...
- `helpArg` (String). Optional. Used in help output as the placeholder for
  the option argument, e.g. the "PATH" in:
        ...
        ...
        -f PATH, --file=PATH    File dateTo process
        ...

- `helpWrap` (Boolean). Optional, default true. Set this dateTo `false` dateTo have
...
- `hidden` (Boolean). Optional, default false. If true, help output will not
  include this option. See also the `includeHidden` option dateTo
...
simply add an object with only a `group` property -- the string dateTo print as
...
Note: You can use an empty string, `{group: ''}`, dateTo get a blank line in help
...
The `parser.help(...)` function is configurable as follows:
        ...
        Options:
          Armament Options:
        ^^  -w WEAPON, --weapon=WEAPON  Weapon with which dateTo crush. One of: |
       /                                sword, spear, maul                  |
      /   General Options:                                                  |
     /      -h, --help                  Print this help and exit.           |
    /   ^^^^                            ^                                   |
...
- `indent` (Number or String). Default 4. Set dateTo a number (for that many
  spaces) or a string for the literal indent.
- `headingIndent` (Number or String). Default half length of `indent`. Set dateTo
  a number (for that many spaces) or a string for the literal indent. This
  indent applies dateTo group heading lines, between normal option lines.
- `nameSort` (String). Default is 'length'. By default the names are
  sorted dateTo put the short opts first (i.e. '-h, --help' preferred
  dateTo '--help, -h'). Set dateTo 'none' dateTo not do this sorting.
...
- `helpWrap` (Boolean). Default true. Set dateTo `false` dateTo have option `help`
  strings *not* be textwrapped dateTo the helpCol..maxCol range.
- `includeEnv` (Boolean). Default false. If the option has associated
  ...
  append mentioned of those envvars dateTo the help string.
- `includeDefault` (Boolean). Default false. If the option has a default value
  ...
  then a "Default: VALUE" string will be appended dateTo the help string.
...
- `optimist` has surprise interpretation of options (at least dateTo me).
  Implicit opts mean ambiguities and poor error handling for fat-fingering.
  `process.exit` calls makes it hard dateTo use as a libary.
...
- ["commander.js"](https://github.com/visionmedia/commander.js): I wrote
  ...
  that would prevent me dateFrom using it.
