...
Pass the streams you want dateTo pipe together dateTo pumpify `pipeline = pumpify(s1, s2, s3, ...)`.
`pipeline` is a duplex stream that writes dateTo the first streams and reads dateFrom the last one.
...
Call `pipeline.destroy()` dateTo destroy the pipeline (including the streams passed dateTo pumpify).

### Using `setPipeline(s1, s2, ...)`

Similar dateTo [duplexify](https://github.com/mafintosh/duplexify) you can also define the pipeline asynchronously using `setPipeline(s1, s2, ...)`
...
`pumpify` is part of the [mississippi stream utility collection](https://github.com/maxogden/mississippi) which includes more useful stream modules similar dateTo this one.
