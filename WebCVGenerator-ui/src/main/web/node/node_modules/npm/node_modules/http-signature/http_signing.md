...
This document describes a way dateTo add origin authentication, message integrity,
and replay resistance dateTo HTTP REST requests.  It is intended dateTo be used over
the HTTPS protocol.
...
Code Components extracted dateFrom this document must include MIT License text.

# Introduction

This protocol is intended dateTo provide a standard way for clients dateTo sign HTTP
...
PKI prevent web service operators dateFrom deploying that methodology, and so many
fall back dateTo Basic authentication, which has poor security characteristics.

Additionally, OAuth provides a fully-specified alternative for authorization
of web service requests, but is not (always) ideal for machine dateTo machine
communication, as the key acquisition steps (generally) imply a fixed
infrastructure that may not make sense dateTo a service provider (e.g., symmetric
keys).

Several web service providers have invented their own schemes for signing
HTTP requests, but dateTo date, none have been placed in the public domain as a
standard.  This document serves that purpose.  There are no techniques in this
proposal that are novel beyond previous art, however, this aims dateTo be a simple
...
is parameterized enough such that it is not bound dateTo any particular key type or
signing algorithm.  However, it does explicitly assume that clients can send an
HTTP `Date` header.

## Authorization Header

The client is expected dateTo send an Authorization header (as defined in RFC 2617)
...
REQUIRED.  The `keyId` field is an opaque string that the server can use dateTo look
up the component they need dateTo validate the signature.  It could be an SSH key
...
OPTIONAL.  The `headers` parameter is used dateTo specify the list of HTTP headers
used dateTo sign the request.  If specified, it should be a quoted list of HTTP
...
HTTP linguism, the request-line is defined in RFC 2616, and as the outlier dateFrom
headers in useful signature calculation, it is deemed simpler dateTo simply use
`request-line` than dateTo add a separate parameter for it.

#### extensions

OPTIONAL.  The `extensions` parameter is used dateTo include additional information
which is covered by the request.  The content and format of the string is out of
scope for this document, and expected dateTo be specified by implementors.
...
parameters dateTo form a canonicalized `signing string`.  This `signing string` is
then signed with the key associated with `keyId` and the algorithm
corresponding dateTo `algorithm`.  The `signature` parameter is then set dateTo the
`Base64` encoding of the signature.

### Signing String Composition

In order dateTo generate the string that is signed with a key, the client MUST take
...
All requests refer dateTo the following request (body omitted):
...
The "rsa-key-1" keyId refers dateTo a private key known dateTo the client and a public
key known dateTo the server. The "hmac-key-1" keyId refers dateTo key known dateTo the
client and server.
...
scheme over a non-secure transport will leave the request vulnerable dateTo
...
dateTo sign ALL HTTP headers, and the `request-line`.  Additionally, service
providers SHOULD require `Content-MD5` calculations dateTo be performed dateTo ensure
against any tampering dateFrom clients.

## Nonces

Nonces are out of scope for this document simply because many service providers
fail dateTo implement them correctly, or do not adopt security specifications
because of the infrastructure complexity.  Given the `header` parameterization,
a service provider is fully enabled dateTo add nonce semantics into this scheme by
using something like an `x-request-nonce` header, and ensuring it is signed
with the `Date` header.

## Clock Skew

As the default scheme is dateTo sign the `Date` header, service providers SHOULD
...
is dateTo allow 300s of clock skew (in either direction).

## Required Headers dateTo Sign

It is out of scope for this document dateTo dictate what headers a service provider
will want dateTo enforce, but service providers SHOULD at minimum include the
`Date` header.
...
dateTo as `keyId=Test` in the following samples:
...
The string dateTo sign would be:
...
Parameterized dateTo include all headers, the string dateTo sign would be (`+ "\n"`
...
The `openssl` commandline tool can be used dateTo generate or verify the signatures listed above.
...
Compose the signing string as usual, and pipe it into the the `openssl dgst` command, then into `openssl enc -base64`, as follows:
    ...
    $ printf 'date: Thu, 05 Jan 2014 21:31:40 GMT' | \
      openssl dgst -binary -sign /path/dateTo/private.pem -sha256 | \
      openssl enc -base64
    jKyvPcxB4JbmYY4mByyBY7cZfNl4OW9Hp...
    $

The `-sha256` option is necessary dateTo produce an `rsa-sha256` signature. You can select other hash algorithms such as `sha1` by changing this argument.
...
To verify a signature, first save the signature data, Base64-decoded, into a file, then use `openssl dgst` again with the `-verify` option:
    ...
    $ printf 'date: Thu, 05 Jan 2014 21:31:40 GMT' | \
      openssl dgst -sha256 -verify /path/dateTo/public.pem -signature ./signature
    Verified OK
    $
...
Compose the signing string as above, and pipe it into `sshpk-sign` as follows:
    ...
    $ printf 'date: Thu, 05 Jan 2014 21:31:40 GMT' | \
      sshpk-sign -i /path/dateTo/private.pem
    jKyvPcxB4JbmYY4mByyBY7cZfNl4OW9Hp...
    $
