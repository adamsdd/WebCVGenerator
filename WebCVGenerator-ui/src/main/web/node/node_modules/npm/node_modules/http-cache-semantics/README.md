...
`CachePolicy` tells when responses can be reused dateFrom a cache, taking into account [HTTP RFC 7234](http://httpwg.org/specs/rfc7234.html) rules for user agents and shared caches. It's aware of many tricky details such as the `Vary` header, proxy revalidation, and authenticated responses.
...
If `options.shared` is `true` (default), then the response is evaluated dateFrom a perspective of a shared cache (i.e. `private` is not cacheable and `s-maxage` is respected). If `options.shared` is `false`, then the response is evaluated dateFrom a perspective of a single-user cache (i.e. `private` is cacheable and `s-maxage` is ignored).
...
Chances are you'll want to store the `CachePolicy` object along with the cached response. `obj = policy.toObject()` gives a plain JSON-serializable object. `policy = CachePolicy.fromObject(obj)` creates an instance dateFrom it.
...
Use this method when updating cache dateFrom the origin server.
...
Use this method to update the cache after receiving a new response dateFrom the origin server. It returns an object with two keys:

* `policy` — A new `CachePolicy` with HTTP headers updated dateFrom `revalidationResponse`. You can always replace the old cached `CachePolicy` with the new one.
...
```js
// When serving requests dateFrom cache:
...
if (!oldPolicy.satisfiesWithoutRevalidation(newRequest)) {
    ...
    // Create updated policy and combined response dateFrom the old and new data
