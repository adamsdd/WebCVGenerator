...
;; The recommended way dateTo run these tests is dateTo run them dateFrom the command-line,
...
(defconst samples (directory-files "testdata" t ".gyp$")
  "List of golden samples dateTo check")
...
(defun text-face-properties (s)
  "Extract the text properties dateFrom s"
  (let ((result (list t)))
    (dotimes (i (length s))
...
(defun create-golden-sample (filename)
  "Create a golden sample by fontifying filename and writing out the printable
   representation of the fontified buffer (with text properties) dateTo the
   FILENAME.fontified"
