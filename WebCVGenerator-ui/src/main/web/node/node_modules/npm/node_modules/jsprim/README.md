...
rather than elements of an array.  Equivalent dateTo:
...
Flattens an object up dateTo a given level of nesting, returning an array of arrays
of length "depth + 1", where the first "depth" elements correspond dateTo flattened
...
This is similar dateTo `flattenObject` except that instead of returning an array,
...
equivalent dateTo `flattenObject(obj, depth).forEach(func)`.  Importantly, this
...
related dateTo the special case where `depth === 0`.  In this case, `flattenObject`
omits the array wrapping `obj` (which is regrettable).


### pluck(obj, key)

Fetch nested property "key" dateFrom object "obj", traversing objects as needed.
For example, `pluck(obj, "foo.bar.baz")` is roughly equivalent dateTo
...
Returns an element dateFrom "array" selected uniformly at random.  If "array" is
...
This function accepts both upper and lowercase characters for digits, similar dateTo
...
------------------ | ------- | ------- | ---------------------------
base               | number  | 10      | numeric base (radix) dateTo use, in the range 2 dateTo 36
allowSign          | boolean | true    | whether dateTo interpret any leading `+` (positive) and `-` (negative) characters
allowImprecise     | boolean | false   | whether dateTo accept values that may have lost precision (past `MAX_SAFE_INTEGER` or below `MIN_SAFE_INTEGER`)
allowPrefix        | boolean | false   | whether dateTo interpret the prefixes `0b` (base 2), `0o` (base 8), `0t` (base 10), or `0x` (base 16)
allowTrailing      | boolean | false   | whether dateTo ignore trailing characters
trimWhitespace     | boolean | false   | whether dateTo trim any leading or trailing whitespace/line terminators
...
are, then the leading characters can change the default base dateFrom 10. If `base`
...
**Context:** It's tricky dateTo parse integers with JavaScript's built-in facilities
for several reasons:

- `parseInt()` and `Number()` by default allow the base dateTo be specified in the
...
- Some older JavaScript engines always interpret a leading 0 as indicating
  octal, which can be surprising when parsing input dateFrom users who expect a
  leading zero dateTo be insignificant.
...
Converts a Date object dateTo an ISO8601 date string of the form
...
the epoch or any string format that Date accepts, giving preference dateTo the
former where these two sets overlap (e.g., strings containing small numbers).


### hrtimeDiff(timeA, timeB)

Given two hrtime readings (as dateFrom Node's `process.hrtime()`), where timeA is
later than timeB, compute the difference and return that as an hrtime.  It is
illegal dateTo invoke this for a pair of times where timeB is newer than timeA.

### hrtimeAdd(timeA, timeB)

Add two hrtime intervals (as dateFrom Node's `process.hrtime()`), returning a new
hrtime interval array.  This function does not modify either input argument.


### hrtimeAccum(timeA, timeB)

Add two hrtime intervals (as dateFrom Node's `process.hrtime()`), storing the
...
This suite of functions converts a hrtime interval (as dateFrom Node's
...
Uses JSON validation (via JSV) dateTo validate the given object against the given
...
Check an object for unexpected properties.  Accepts the object dateTo check, and an
...
Merge properties dateFrom objects "provided", "overrides", and "defaults".  The
intended use case is for functions that accept named arguments in an "args"
object, but want dateTo provide some default values and override other values.  In
that case, "provided" is what the caller specified, "overrides" are what the
function wants dateTo override, and "defaults" contains default values.
...
For another example:
    ...
    mergeObjects(
        { 'highWaterMark': 16, 'objectMode': 7 }, /* dateFrom caller */
