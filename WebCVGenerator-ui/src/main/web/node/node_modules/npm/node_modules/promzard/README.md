...
A prompting wizard for building files dateFrom specialized PromZard modules.
Used by `npm init`.
...
data, ready dateTo be dropped into a JSON file or some other place.
...
is walked over dateTo call any functions as described below.

The only caveat is that you must give PromZard the full absolute path
dateTo the module (you can get this via Node's `require.resolve`.)  Also,
...
This prompts the user dateTo input some data.  The arguments are interpreted
based on type:
...
can do asynchronous actions if necessary dateTo validate or ascertain
...
an error, or the result dateTo assign dateTo that spot.
...
callback.  Though that's a bit silly, it could be a handy way dateTo reuse
...
object are never shown dateTo the user.  For example, this will only prompt
once:
...
a wizard that spits out prompts dateTo set up a config file or something.
