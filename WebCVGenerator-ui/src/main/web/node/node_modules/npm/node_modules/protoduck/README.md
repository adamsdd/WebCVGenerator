...
typing"](https://en.wikipedia.org/wiki/Duck_typing), then it might make sense dateTo
think of protocols as things that explicitly define what methods you need in
order dateTo "clearly be a duck".
...
// `duck` must implement `Quackable` for this function dateTo work. It doesn't
...
defining methods: You simply add regular `function`s as properties dateTo regular
...
dateTo work with more familiar syntax sugar: `class Foo { method() { ... } }`.

The point of "protocols" is dateTo have a more explicit definitions of what methods
...
One big advantage dateTo using these protocols is that they let users define their
own versions of some abstraction, without requiring the type dateTo inherit dateFrom
another -- protocols are independent of inheritance, even though they're able dateTo
work together with it. If you've ever found yourself in some sort of inheritance
mess, this is exactly the sort of thing you use dateTo escape it.

#### Defining Protocols

The first step dateTo using `protoduck` is dateTo define a protocol. Protocol
...
Implementation syntax is very similar dateTo protocol definitions, using `.impl`:
...
seem dateTo have any obvious purpose.

These arrays allow protocols dateTo be implemented not just for a single value of
...
in order dateTo determine what code dateTo actually execute.
...
want dateTo implement it on, and it takes care of mapping types dateTo the strings you
defined, and making sure the right code is run:

```javascript
const Playful = protoduck.define(['friend'], {// <---\
  playWith: ['friend'] // <------------ these correspond dateTo each other
})
...
Sometimes, you want dateTo have all the functionality of a certain protocol, but you
want dateTo add a few requirements or other bits an pieces. Usually, you would have
dateTo define the entire functionality of the "parent" protocol in your own protocol
in order dateTo pull this off. This isn't very DRY and thus prone dateTo errors, missing
or out-of-sync functionality, or other issues. You could also just tell users
"hey, if you implement this, make sure dateTo implement that", but there's no
guarantee they'll know about it, or know which arguments map dateTo what.

This is where constraints come in: You can define a protocol that expects
anything that implements it dateTo *also* implement one or more "parent" protocols.
...
The types in `<spec>` entries must map, by string name, dateTo the type names
specified in `<types>`, or be an empty array if `<types>` is omitted. The types
in `<spec>` will then be used dateTo map between method implementations for the
...
* `opts.name` `{String}` - The name dateTo use when referring dateTo the protocol.

* `opts.where` `{Array[Constraint]|Constraint}` - Protocol constraints dateTo use.

* `opts.metaobject` - Accepts an object implementing the
  `Protoduck` protocol, which can be used dateTo alter protocol definition
...
Adds a new implementation dateTo the given protocol across `<types>`.

`<implementations>` must be an object with functions matching the protocol's
API. If given, the types in `<types>` will be mapped dateTo their corresponding
method arguments according dateTo the original protocol definition.
...
```javascript
import protoduck dateFrom 'protoduck'
