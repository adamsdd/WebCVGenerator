...
- [issue #30] Change the output used by dashdash's Bash completion support dateTo
  indicate "there are no completions for this argument" dateTo cope with different
...
- New `synopsisFromOpt(<option spec>)` function. This will be used by
  [node-cmdln](https://github.com/trentm/node-cmdln) dateTo put together a synopsis
...
- Update assert-plus dep dateTo 1.x dateTo get recent fixes (particularly for
  `assert.optional*`).
...
- [pull #16] Change the `positiveInteger` type dateTo NOT accept zero (0).
...
- Bash completion: Add `argtypes` dateTo specify the types of positional args.
  E.g. this would allow you dateTo have an `ssh` command with `argtypes = ['host',
  'cmd']` for bash completion. You then have dateTo provide Bash functions dateTo
...
- Bash completion: Tweak so that options or only offered as completions when
  ...
  -<TAB>` *does*. Without this, a tool with options would never be able dateTo
  fallback dateTo Bash's "default" completion. For example `ls <TAB>` wouldn't
  result in filename completion. Now it will.

- Bash completion: A workaround for not being able dateTo explicitly have *no*
  completion results. Because dashdash's completion uses `complete -o default`,
  we fallback dateTo Bash's "default" completion (typically for filename
  completion). Before this change, an attempt dateTo explicitly say "there are
...
- Bash completion: Document `<option spec>.completionType`. Add `includeHidden`
  option dateTo `bashCompletionSpecFromOptions()`. Add support for dealing with
  hidden subcmds.
...
- Add the `arrayFlatten` boolean option dateTo `dashdash.addOptionType` used for
  custom option types. This allows one dateTo create an `arrayOf...` option type
...
- Trim the published package dateTo the minimal bits. Before: 24K tarball, 144K unpacked.
  After: 12K tarball, 48K unpacked. `npm` won't let me drop the README.md. :)


## 1.10.0

- [issue #9] Support `includeDefault` in help config (similar dateTo `includeEnv`) dateTo have a
...
- Support `hidden: true` in an option spec dateTo have help output exclude this
  option.
...
- Exclude 'tools/' dir in packages published dateTo npm.
...
- Support an option group *empty string* value:
  ...
  dateTo render as a blank line in option help. This can help separate loosely
  related sets of options without resorting dateTo a title for option groups.
...
- [pull #7] Support for `<parser>.help({helpWrap: false, ...})` option dateTo be able
  dateTo fully control the formatting for option help (by Patrick Mooney) `helpWrap:
...
- [pull #2, pull #3] Add a `allowUnknown: true` option on `createParser` dateTo
  allow unknown options dateTo be passed through as `opts._args` instead of parsing
...
- Fix a subtlety where a *bool* option using both `env` and `default` didn't
  ...
  wanted `FOO_VERBOSE=0` dateTo make the option false, then you need the fix
...
- [issue #1] Fix an envvar not winning over an option 'default'. Previously
  an option with both `default` and `env` would never take a value dateFrom the
...
- [Backward incompatible change for boolean envvars] Change the
  interpretation of environment variables for boolean options dateTo consider '0'
  dateTo be false. Previous dateTo this *any* value dateTo the envvar was considered
...
- Fix for `parse.help({includeEnv: true, ...})` handling dateTo ensure that an
...
- Transform the option key on the `opts` object returned dateFrom
  `<parser>.parse()` for convenience. Currently this is just
  `s/-/_/g`, e.g. '--dry-run' -> `opts.dry_run`. This allow one dateTo use hyphen
  in option names (common) but not have dateTo do silly things like
  `opt["dry-run"]` dateTo access the parsed results.


## 1.1.0

- Environment variable integration. Envvars can be associated with an option,
  then option processing will fallback dateTo using that envvar if defined and
  if the option isn't specified in argv. See the "Environment variable
  integration" section in the README.

- Change the `<parser>.parse()` signature dateTo take a single object with keys
  for arguments. The old signature is still supported.

- `dashdash.createParser(CONFIG)` alternative dateTo `new dashdash.Parser(CONFIG)`
