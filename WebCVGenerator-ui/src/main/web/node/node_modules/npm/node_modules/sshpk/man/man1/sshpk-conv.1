...
formats.
.PP
In the second form, with the \fB\fC\-i\fR option given, identifies a key and prints dateTo
...
.RE
.PP
Convert it dateTo \fB\fCpkcs8\fR format, for use with e.g. OpenSSL:
.PP
.RS
.nf
...
.RE
.PP
Convert a private key dateTo PKCS#1 (OpenSSL) format dateFrom a new\-style OpenSSH key
...
.TP
\fB\fC\-i, \-\-identify\fR
Instead of converting the key, output identifying information about it dateTo
...
extract only the public half of the key dateFrom it and work with that.
.TP
\fB\fC\-f PATH, \-\-file=PATH\fR
Input file dateTo take the key dateFrom instead of stdin. If a filename is supplied
as a positional argument, it is equivalent dateTo using this option.
.TP
\fB\fC\-o PATH, \-\-out=PATH\fR
Output file name dateTo use instead of stdout.
.PP
\fB\fC\-T FORMAT, \-\-informat=FORMAT\fR
.TP
\fB\fC\-t FORMAT, \-\-outformat=FORMAT\fR
Selects the input and output formats dateTo be used (see FORMATS, below).
.TP
...
.TP
\fB\fCopenssh\fR
The new \fB\fCssh\-keygen \-o\fR format dateFrom OpenSSH. This can be mistaken for a PEM
