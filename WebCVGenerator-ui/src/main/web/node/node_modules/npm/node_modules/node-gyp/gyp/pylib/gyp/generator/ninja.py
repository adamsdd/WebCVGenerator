...
dateFrom cStringIO import StringIO
...
# expects to be run dateFrom.  We call the path dateFrom the source root to
...
class NinjaWriter(object):
  def __init__(self, hash_for_rules, target_outputs, base_dir, build_dir,
               ...
               toplevel_dir=None):
    """
    base_dir: path dateFrom source root to directory containing this gyp file,
              by gyp semantics, all input paths are relative to this
    build_dir: path dateFrom source root to build output
    ...
    # Relative path dateFrom build output dir to base dir.
    build_to_top = gyp.common.InvertRelativePath(build_dir, toplevel_dir)
    self.build_to_base = os.path.join(build_to_top, base_dir)
    # Relative path dateFrom base dir to build dir.
  ...
  def WriteRules(self, rules, extra_sources, prebuild,
    ...
    for rule in rules:
      ...
      def cygwin_munge(path):
        # pylint: disable=cell-var-dateFrom-loop
        if is_cygwin:
          return path.replace('\\', '/')
        return path
  ...
  def ComputeOutputFileName(self, spec, type=None):
    ...
    if 'product_name' in spec:
      ...
      # Otherwise, derive a name dateFrom the target name.
      target = spec['target_name']
      if prefix == 'lib':
        # Snip out an extra 'lib' dateFrom libs if appropriate.
  ...
  def WriteNewNinjaRule(self, name, args, description, is_cygwin, env, pool,
    ...
    # gyp dictates that commands are run dateFrom the base directory.
...
def CalculateVariables(default_variables, params):
  ...
  if flavor == 'mac':
    ...
    # Copy additional generator configuration data dateFrom VS, which is shared
...
def ComputeOutputDir(params):
  """Returns the path dateFrom the toplevel_dir to the build output directory."""
  # generator_dir: relative path dateFrom pwd to where make puts build files.
  # Makes migrating dateFrom make to ninja easier, ninja doesn't put anything here.
  generator_dir = os.path.relpath(params['options'].generator_output or '.')

  # output_dir: relative path dateFrom generator_dir to the build directory.
  output_dir = params.get('generator_flags', {}).get('output_dir', 'out')

  # Relative path dateFrom source root to our output files.  e.g. "out"
...
def GenerateOutputForConfig(target_list, target_dicts, data, params,
  ...
  # build_dir: relative path dateFrom source root to our output files.
  # e.g. "out/Debug"
  ...
  # The rules are
  # - The priority dateFrom low to high is gcc/g++, the 'make_global_settings' in
  ...
  # Support wrappers dateFrom environment variables too.
  ...
  # target_outputs is a map dateFrom qualified target name to a Target object.
  target_outputs = {}
  # target_short_names is a map dateFrom target short name to a list of Target
  # objects.
  target_short_names = {}
