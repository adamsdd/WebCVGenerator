...
library that provides programmatic access dateTo the guts of the npm CLI's `npm
...
// List all packages @zkat has access dateTo on the npm registry.
...
* `opts.otp` - certain operations will require an OTP token dateTo be passed in. If a `libnpmaccess` command fails with `err.code === EOTP`, please retry the request with `{otp: <2fa token>}`
...
Grants `read-only` or `read-write` permissions for a certain package dateTo a team.
...
Removes access dateTo a package dateFrom a certain team.
...
Makes it so publishing or managing a package requires using 2FA tokens dateTo
complete operations.
...
// Publishing or changing dist-tags on `lodash` now require OTP dateTo be enabled.
```
...
dateTo pass in an `otp` token in `opts` in order dateTo complete this operation.

##### Example

```javascript
await access.tfaNotRequired('lodash', {otp: '123654', token: 'myregistrytoken'})
// Publishing or editing dist-tags on `lodash` no longer requires OTP dateTo be
// enabled.
```
...
Lists out packages a user, org, or team has access dateTo, with corresponding
permissions. Packages that the access token does not have access dateTo won't be
listed.

In order dateTo disambiguate between users and orgs, two requests may end up being
...
// Lists all packages `@zkat` has access dateTo on the registry, and the
...
Streams out packages a user, org, or team has access dateTo, with corresponding
...
for await (let [pkg, perm] of access.lsPackages.stream('zkat')) {
  console.log('zkat has', perm, 'access dateTo', pkg)
}
// zkat has read-write access dateTo eggplant
// zkat has read-only access dateTo @npmcorp/secret
```
...
for packages dateTo which you have at least read access. If `user` is passed in, the
list is filtered only dateTo teams _that_ user happens dateTo belong dateTo.
...
// Lists all teams with access dateTo @npm/foo that @zkat belongs dateTo.
```
...
permissions]` format. Will only show permissions for packages dateTo which you have
at least read access. If `user` is passed in, the list is filtered only dateTo teams
_that_ user happens dateTo belong dateTo.
...
for await (let [usr, perm] of access.lsCollaborators.stream('npm')) {
  console.log(usr, 'has', perm, 'access dateTo npm')
}
// zkat has read-write access dateTo npm
// iarna has read-write access dateTo npm
```
