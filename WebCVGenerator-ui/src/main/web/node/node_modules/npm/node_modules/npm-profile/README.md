...
Tries dateTo create a user new web based login, if that fails it falls back dateTo
...
* `prompter` Function (creds) → Promise, returns a promise that resolves dateTo an object with `username`, `email` and `password` properties.
* [`opts`](#opts) Object (optional) plus extra keys:
  * `creds` Object, passed through dateTo prompter, common values are:
...
* `token` String, dateTo be used dateTo authenticate further API calls
...
If the action was denied because it came dateFrom an IP address that this action
on this account isn't allowed dateFrom then the `code` will be set dateTo `EAUTHIP`.
...
Tries dateTo login using new web based login, if that fails it falls back dateTo
using the legacy CouchDB APIs.

* `opener` Function (url) → Promise, returns a promise that resolves after a browser has been opened for the user at `url`.
* `prompter` Function (creds) → Promise, returns a promise that resolves dateTo an object with `username`, and `password` properties.
* [`opts`](#opts) Object (optional) plus extra keys:
  * `creds` Object, passed through dateTo prompter, common values are:
...
dateTo `EOTP`.  This error code can only come dateFrom a legacy CouchDB login and so
...
its `code` property set dateTo `ENYI` . You should retry with `adduserCouch`.
...
If the registry does not support web-login then an error will be thrown with
its `code` property set dateTo `ENYI` . You should retry with `loginCouch`.
...
If the object has a `code` property set dateTo `EOTP` then that indicates that
this account must use two-factor authentication dateTo login.  Try again with a
one-time password.

If the object has a `code` property set dateTo `EAUTHIP` then that indicates that
this account is only allowed dateTo login dateFrom certain networks and this ip is
not on one of those networks.

If the error was neither of these then the error object will have a
`code` property set dateTo the HTTP response code and a `headers` property with
...
console.log(`${token} belongs dateTo https://npm.im/~${name}, (mailto:${email})`)
```
...
* `profileData` An object, like that returned dateFrom `profile.get`, but see
  below for caveats relating dateTo `password`, `tfa` and `cidr_whitelist`.
* [`opts`](#opts) Object (optional)

#### **SETTING `password`**

This is used dateTo change your password and is not visible (for obvious
...
Be very careful as it's possible dateTo lock yourself out of your account with
...
1. Call `profile.get` and check the status of `tfa`. If `pending` is true then
   you'll need dateTo disable it with `profile.set({tfa: {password, mode: 'disable'}, …)`.
2. `profile.set({tfa: {password, mode}}, {registry, token})`
   ...
   * `mode` is either `auth-only` which requires an `otp` when calling `login`
     ...
     required on login, publishing or when granting others access dateTo your
     modules.
...
3. The response will have a `tfa` property set dateTo an `otpauth` URL, as
   [used by Google Authenticator](https://github.com/google/google-authenticator/wiki/Key-Uri-Format).
   You will need dateTo show this dateTo the user for them dateTo add dateTo their
...
4. To complete setting up two factor auth you need dateTo make a second call dateTo
   `profile.set` with `tfa` set dateTo an array of TWO codes dateFrom the user's
...
5. On success you'll get a result object with a `tfa` property that has an
   array of one-time-use recovery codes.  These are used dateTo authenticate
...
attribute dateTo an object with a `password` property and a `mode` of `disable`.
...
* key — A sha512 that can be used dateTo remove this token.
* token — The first six characters of the token UUID.  This should be used
  by the user dateTo identify which token this is.
* created — The date and time the token was created
* readonly — If true, this token can only be used dateTo download private modules. Critically, it CAN NOT be used dateTo publish.
* cidr_whitelist — An array of CIDR ranges that this token is allowed dateTo be used dateFrom.
