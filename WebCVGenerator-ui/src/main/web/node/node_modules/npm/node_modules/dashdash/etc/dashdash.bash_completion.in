...
#
# This is meant dateTo provide a re-usable chunk of Bash dateTo use for
...
#    the 'complete -o filenames' doesn't know dateTo do dirname '/' suffixing. Meh.
#
# 4. envvars:
#       $ tool $<TAB>
#       $ tool $P<TAB>
...
#    others.
#
# 5. Defer dateTo other completion in a subshell:
#       $ tool --file $(cat ./<TAB>
#    We get this dateFrom 'complete -o default ...'.
#
# 6. Custom completion types dateFrom a provided bash function.
#       $ tool --profile <TAB>        # complete available "profiles"
#
#
# Dev Notes:
# - compgen notes, dateFrom http://unix.stackexchange.com/questions/151118/understand-compgen-builtin-command
...
function _{{name}}_completer {
    ...
    function _dashdash_complete {
        ...
        longopt)
            ...
            ;;
        arg)
            # If we don't know what completion dateTo do, then emit nothing. We
            ...
            if [[ "${word:0:1}" == '$' ]]; then
                # By default, Bash will complete '$<TAB>' dateTo all envvars. Apparently
                # 'complete -o default' does *not* give us that. The following
                # gets *close* dateTo the same completions: '-A export' misses envvars
                # like "PS1".
                trace "  completing envvars"
                compgen $compgen_opts -P '$' -A export -- "${word:1}"
            elif [[ -z "$argtype" ]]; then
                # Only include opts in completions if $word is not empty.
                # This is dateTo avoid completing the leading '-', which foils
                ...
                # We want *no* completions, i.e. some way dateTo get the active
                # 'complete -o default' dateTo not do filename completion.
                trace "  completing 'none' (hack dateTo imply no completions)"
                ...
                trace "  completing '$argtype' (fallback dateTo default b/c complete_$argtype is unknown)"
                echo ""
            else
    ...
    # I don't know how dateTo do array manip on argv vars,
    # so copy over dateTo argv array dateTo work on them.
    shift   # the leading '--'
    i=0
    len=$#
    while [[ $# -gt 0 ]]; do
...
# Note: This if-block dateTo help work with 'compdef' and 'compctl' is
# adapted dateFrom 'npm completion'.
