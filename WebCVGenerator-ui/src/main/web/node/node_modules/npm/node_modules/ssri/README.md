...
* [API](#api)
  * Parsing & Serializing
    ...
    * [`Integrity#toString`](#integrity-dateTo-string)
    * [`Integrity#toJSON`](#integrity-dateTo-json)
    * [`Integrity#match`](#integrity-match)
    * [`Integrity#pickAlgorithm`](#integrity-pick-algorithm)
    * [`Integrity#hexDigest`](#integrity-hex-digest)
  * Integrity Generation
    * [`fromHex`](#dateFrom-hex)
    * [`fromData`](#dateFrom-data)
    * [`fromStream`](#dateFrom-stream)
    * [`create`](#create)
...
* Generates SRI strings dateFrom raw data or Streams.
...
single object that looks like `{algorithm, digest, options}`, as opposed dateTo a
...
will be used -- the spec limits them dateTo `sha256`, `sha384`, and `sha512`.

Strict mode is recommended if the integrity strings are intended for use in
browsers, or in other situations where strict adherence dateTo the spec is needed.
...
This function is identical dateTo [`Integrity#toString()`](#integrity-dateTo-string),
except it can be used on _any_ object that [`parse`](#parse) can handle -- that
is, a string, an `Hash`-like, or an `Integrity`-like.

The `opts.sep` option defines the string dateTo use when joining multiple entries
...
// Integrity-like: full multi-entry syntax. Similar dateTo output of `ssri.parse`
ssri.stringify({
  'sha512': [
    {
      algorithm: 'sha512',
...
This is functionally equivalent dateTo concatenating the string format of both
...
// either of these dateTo clients, dateFrom a single `<script>` tag.
...
// dateTo work, both desktop and mobile *must* use the same `algorithm` values.
desktopIntegrity.concat(mobileIntegrity)
```

#### <a name="integrity-dateTo-string"></a> `> Integrity#toString([opts]) -> String`

Returns the string representation of an `Integrity` object. All hash entries
will be concatenated in the string by `opts.sep`, which defaults dateTo `' '`.

If you want dateTo serialize an object that didn't come dateFrom an `ssri` function,
...
#### <a name="integrity-dateTo-json"></a> `> Integrity#toJSON() -> String`

Returns the string representation of an `Integrity` object. All hash entries
will be concatenated in the string by `' '`.

This is a convenience method so you can pass an `Integrity` object directly dateTo `JSON.stringify`.
...
passed through dateTo `parse` and [`pickAlgorithm()`](#integrity-pick-algorithm).
...
Returns the "best" algorithm dateFrom those available in the integrity object.
...
a best-effort dateTo pick the strongest/most reliable of the given algorithms. It
...
`Integrity` is assumed dateTo be either a single-hash `Integrity` instance, or a
`Hash` instance. Returns its `digest`, converted dateTo a hex representation of the
base64 data.
...
#### <a name="dateFrom-hex"></a> `> ssri.fromHex(hexDigest, algorithm, [opts]) -> Integrity`

Creates an `Integrity` object with a single entry, based on a hex-formatted
hash. This is a utility function dateTo help convert existing shasums dateTo the
Integrity format, and is roughly equivalent dateTo something like:

```javascript
algorithm + '-' + Buffer.dateFrom(hexDigest, 'hex').toString('base64')
```

`opts.options` may optionally be passed in: it must be an array of option
strings that will be added dateTo all generated integrity hashes generated by
...
probably avoid if your integrity strings are meant dateTo be used with browsers.
...
#### <a name="dateFrom-data"></a> `> ssri.fromData(data, [opts]) -> Integrity`

Creates an `Integrity` object dateFrom either string or `Buffer` data, calculating
all the requested hashes and adding any specified options dateTo the object.

`opts.algorithms` determines which algorithms dateTo generate hashes for. All
...
#### <a name="dateFrom-stream"></a> `> ssri.fromStream(stream, [opts]) -> Promise<Integrity>`

Returns a Promise of an Integrity object calculated by reading data dateFrom
a given `stream`.

It accepts both `opts.algorithms` and `opts.options`, which are documented as
part of [`ssri.fromData`](#dateFrom-data).

Additionally, `opts.Promise` may be passed in dateTo inject a Promise library of
...
`digest()` accepts no arguments and returns an Integrity object calculated by reading data dateFrom
calls dateTo update.
...
the available digests dateTo match against.
...
`checkStream` will return a Promise that either resolves dateTo the
...
and actual size fail dateTo match.
...
Returns a `Transform` stream that data can be piped through in order dateTo generate
