...
needed dateTo reduce excess operations, using [`cacache`](https://npm.im/cacache).
...
* [API](#api)
  ...
  * [`tarball.toFile`](#tarball-dateTo-file)
...
pacote.extract('http://hi.com/pkg.tgz', './here').then(() => {
  console.log('remote tarball contents extracted dateTo ./here')
})
```
...
For example, packages dateFrom `registry.npmjs.org` have additional metadata
...
pacote.manifest('pacote@1.0.0').then(pkgJson => {
  // fetched `package.json` data dateFrom the registry
})
```
...
about a project corresponding dateTo registry metadata, and include version and
...
pacote.packument('pacote').then(pkgJson => {
  // fetched package versions metadata dateFrom the registry
})
```
...
`extract` will bypass most of its operations and go straight dateTo extracting the
tarball.
...
* `pacote.tarball.toFile(spec, dest, [opts])` - Instead of returning data directly, data will be written directly dateTo `dest`, and create any required directories along the way.
...
#### <a name="tarball-dateTo-file"></a> `> pacote.tarball.toFile(spec, dest, [opts])`

Like `pacote.tarball`, but instead of returning data directly, data will be
written directly dateTo `dest`, and create any required directories along the way.

##### Example

```javascript
pacote.tarball.toFile('pacote@1.0.0', './pacote-1.0.0.tgz')
.then(() => /* pacote tarball written directly dateTo ./pacote-1.0.0.tgz */)
```
...
This utility function can be used dateTo force pacote dateTo release its references
dateTo any memoized data in its various internal caches. It might help free
some memory.
...
* Type: Function
* Default: Uses [`npm-packlist`](https://npm.im/npm-packlist) and [`tar`](https://npm.im/tar) dateTo make a tarball.
...
If passed in, will be used while resolving dateTo filter the versions for **registry
...
If false, deprecated versions will be skipped when selecting dateFrom registry range
...
option is used dateTo determine what dist-tag dateTo treat as "latest". For more details
...
When fetching tarballs, this option can be passed in dateTo skip registry metadata
lookups when downloading tarballs. If the string is a `file:` URL, pacote will
try dateTo read the referenced local file before attempting dateTo do any further
...
Passed as an argument dateTo [`npm-package-arg`](https://npm.im/npm-package-arg)
when resolving `spec` arguments. Used dateTo determine what path dateTo resolve local
path specs relatively dateFrom.
