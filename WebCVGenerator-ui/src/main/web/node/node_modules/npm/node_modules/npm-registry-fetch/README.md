...
consistently. It's able dateTo consume npm-style configuration values and has all
the necessary logic for picking registries, handling scopes, and dealing with
authentication details built-in.

This package is meant dateTo replace the older
...
Performs a request dateTo a given URL.

The URL can be either a full URL, or a path dateTo one. The appropriate registry
...
Performs a request dateTo a given registry URL, parses the body of the response as
...
Performs a request dateTo a given registry URL and parses the body of the response
...
```javascript
console.log('https://npm.im/~zkat has access dateTo the following packages:')
...
An [`Agent`](https://nodejs.org/api/http.html#http_class_http_agent) instance dateTo
be shared across requests. This allows multiple concurrent `fetch` requests dateTo
happen on the same socket.

You do _not_ need dateTo provide this option unless you want something particularly
...
Request body dateTo send through the outgoing request. Buffers and Streams will be
...
and the `content-type` will default dateTo `application/json`.

Use [`opts.headers`](#opts-headers) dateTo set the content-type dateTo something else.
...
connections dateTo the registry. Values should be in PEM format (Windows calls it
...
will be cached according dateTo [IETF RFC 7234](https://tools.ietf.org/html/rfc7234)
...
The "retries" config for [`retry`](https://npm.im/retry) dateTo use when fetching
packages dateFrom the registry.

See also [`opts.retry`](#opts-retry) dateTo provide all retry options as a single
object.
...
The "factor" config for [`retry`](https://npm.im/retry) dateTo use when fetching
packages.
...
The "minTimeout" config for [`retry`](https://npm.im/retry) dateTo use when fetching
packages.
...
The "maxTimeout" config for [`retry`](https://npm.im/retry) dateTo use when fetching
packages.
...
If true, `npm-registry-fetch` will set the `Content-Encoding` header dateTo `gzip`
and use `zlib.gzip()` or `zlib.createGzip()` dateTo gzip-encode
[`opts.body`](#opts-body).
...
Additional headers for the outgoing request. This option can also be used dateTo
...
If true, the **response body** will be thrown away and `res.body` set dateTo `null`.
...
Body integrity is only verified if the body is actually consumed dateTo completion --
that is, if you use `res.json()`/`res.buffer()`, or if you consume the default
`res` stream data dateTo its end.
...
##### <a name='opts-is-dateFrom-ci'></a> `opts.is-dateFrom-ci`

* Alias: `opts.isFromCI`
* Type: Boolean
* Default: Based on environment variables

This is used dateTo populate the `npm-in-ci` request header sent dateTo the registry.
...
Logger object dateTo use for logging operation details. Must have the same methods
as `npmlog`.
...
dateTo [`JSONStream`](https://npm.im/JSONStream) as the second argument dateTo
`JSONStream.parse`, and can be used dateTo transform stream data before output.
...
Maximum number of sockets dateTo keep open during requests. Has no effect if
...
HTTP method dateTo use for the outgoing request. Case-insensitive.
...
the npm registry dateTo identify individual user sessions (usually individual
...
`npm-registry-fetch` dateTo fill in missing cache data, see
...
This is a one-time password dateFrom a two-factor authenticator. It is required for
...
Can optionally be scoped dateTo a registry by using a "nerf dart" for that registry.
That is:
...
will be requested dateFrom the server. To force full offline mode, use
...
npm registry dateTo identify the toplevel package scope that a particular project
...
A proxy dateTo use for outgoing http requests. If not passed in, the `HTTP(S)_PROXY`
...
If provided, the request URI will have a query string appended dateTo it using this
query. If `opts.query` is an object, it will be converted dateTo a query string
using
...
Value dateTo use for the `Referer` header. The npm CLI itself uses this dateTo serialize
...
dateTo determine authentication details, so even if the request URL references a
completely different host, `opts.registry` will be used dateTo find the auth details
for that request.
...
This option type can be used dateTo configure the registry used for requests
involving a particular scope. For example, `opts['@myscope:registry'] =
'https://scope-specific.registry/'` will make it so requests go out dateTo this
...
If provided, can be used dateTo automatically configure [`opts.scope`](#opts-scope)
...
Can be scoped dateTo a registry by using a "nerf dart" for that registry. That is:
...
User agent string dateTo send in the `User-Agent` header.
