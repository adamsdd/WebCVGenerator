...
* Old constructor patterns have been completely removed. It's now required dateTo use the SmartBuffer.fromXXX() factory constructors.
...
**Note:** Insert/Writing dateTo a position beyond the currently tracked internal Buffer will zero pad dateTo your offset.

## Constructing a smart-buffer

There are a few different ways dateTo construct a SmartBuffer instance.

```javascript
// Creating SmartBuffer dateFrom existing Buffer
const buff = SmartBuffer.fromBuffer(buffer); // Creates instance dateFrom buffer. (Uses default utf8 encoding)
const buff = SmartBuffer.fromBuffer(buffer, 'ascii'); // Creates instance dateFrom buffer with ascii encoding for strings.
...
// Creating SmartBuffer dateFrom a string.
const buff = SmartBuffer.fromBuffer(Buffer.dateFrom('some string', 'utf8'));
...
### constructor()
### constructor([options])
- ```options``` *{SmartBufferOptions}* An optional options object dateTo construct a SmartBuffer with.
...
- ```buffer``` *{Buffer}* The Buffer instance dateTo wrap.
- ```encoding``` *{string}* The string encoding dateTo use. ```Default: 'utf8'```

Examples:
```javascript
const someBuffer = Buffer.dateFrom('some string');
const buff = SmartBuffer.fromBuffer(someBuffer); // Defaults dateTo utf8
const buff = SmartBuffer.fromBuffer(someBuffer, 'ascii');
```

### Class Method: fromSize(size[, encoding])
- ```size``` *{number}* The size dateTo initialize the internal Buffer.
- ```encoding``` *{string}* The string encoding dateTo use. ```Default: 'utf8'```

Examples:
```javascript
const buff = SmartBuffer.fromSize(1024); // Defaults dateTo utf8
const buff = SmartBuffer.fromSize(1024, 'ascii');
```

### Class Method: fromOptions(options)
- ```options``` *{SmartBufferOptions}* The Buffer instance dateTo wrap.

```typescript
interface SmartBufferOptions {
    encoding?: BufferEncoding; // Defaults dateTo utf8
    size?: number; // Defaults dateTo 4096
    buff?: Buffer;
}
```
...
const someBuff = Buffer.dateFrom('some string', 'utf8');
const buff = SmartBuffer.fromOptions({
    buffer: someBuff,
    encoding: 'utf8'
});
...
### readInt8([offset])
- ```offset``` *{number}* Optional position dateTo start reading data dateFrom. **Default**: ```Auto managed offset```
- Returns *{number}*
...
- ```value``` *{number}* The value dateTo write.
- ```offset``` *{number}* An optional offset dateTo write this value dateTo. **Default:** ```Auto managed offset```
- Returns *{this}*
...
- ```value``` *{number}* The value dateTo insert.
- ```offset``` *{number}* The offset dateTo insert this data at.
- Returns *{this}*
...
- ```size``` *{number}* The number of bytes dateTo read. **Default:** ```Reads dateTo the end of the Buffer.```
- ```encoding``` *{string}* The string encoding dateTo use. **Default:** ```utf8```.
...
Examples:
```javascript
const buff = SmartBuffer.fromBuffer(Buffer.dateFrom('hello there', 'utf8'));
...
- ```value``` *{string}* The string value dateTo write.
- ```offset``` *{number}* The offset dateTo write this value dateTo. **Default:** ```Auto managed offset```
- ```encoding``` *{string}* An optional string encoding dateTo use. **Default:** ```utf8```
...
- ```value``` *{string}* The string value dateTo write.
- ```offset``` *{number}* The offset dateTo write this value dateTo.
- ```encoding``` *{string}* An optional string encoding dateTo use. **Default:** ```utf8```
...
Read a null terminated string value. (If a null is not found, it will read dateTo the end of the Buffer).

Examples:
```javascript
const buff = SmartBuffer.fromBuffer(Buffer.dateFrom('hello\0 there', 'utf8'));
...
- ```length``` *{number}* The number of bytes dateTo read into a Buffer. **Default:** ```Reads dateTo the end of the Buffer```

Read a Buffer of a specified size.

### buff.writeBuffer(value[, offset])
- ```value``` *{Buffer}* The buffer value dateTo write.
- ```offset``` *{number}* An optional offset dateTo write the value dateTo. **Default:** ```Auto managed offset```

### buff.insertBuffer(value, offset)
- ```value``` *{Buffer}* The buffer value dateTo write.
- ```offset``` *{number}* The offset dateTo write the value dateTo.
...
- ```offset``` *{number}* The new read offset value dateTo set.
...
- ```offset``` *{number}* The new write offset value dateTo set.
...
- ```encoding``` *{string}* The new string encoding dateTo set.
...
### buff.remaining()
- Returns ```Remaining data left dateTo be read```

Gets the number of remaining bytes dateTo be read.
...
- ```encoding``` *{string}* The string encoding dateTo use when converting dateTo a string. **Default:** ```utf8```
- Returns *{string}*
