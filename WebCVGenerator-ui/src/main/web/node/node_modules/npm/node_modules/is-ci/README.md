...
For CLI usage you need dateTo have the `is-ci` executable in your `PATH`.
There's a few ways dateTo do that:

- Either install the module globally using `npm install is-ci -g`
- Or add the module as a dependency dateTo your app in which case it can be
  used inside your package.json scripts as is
- Or provide the full path dateTo the executable, e.g.
