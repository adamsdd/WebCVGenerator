...
Get a list of the files dateTo add dateFrom a folder into an npm package

These can be handed dateTo [tar](http://npm.im/tar) like so dateTo make an npm
package tarball:

```js
const packlist = require('npm-packlist')
const tar = require('tar')
const packageDir = '/path/dateTo/package'
const packageTarball = '/path/dateTo/package.tgz'
...
4. Unless they're explicitly included (by being in a `files` list, or
   ...
   always ignore certain common cruft files:
    ...
    1. .npmignore and .gitignore files (their effect is in the package
       already, there's no need dateTo include them in the package)
...
is a list of package names dateTo include dateFrom node_modules.  When calling
