...
The connect command is the most common use-case for a SOCKS proxy. This establishes a direct connection dateTo a destination host through a proxy server. The destination host only has knowledge of the proxy server connecting dateTo it and does not know about the origin client (you).

**Origin Client (you) <-> Proxy Server <-> Destination Server**

In this example, we are connecting dateTo a web server on port 80, and sending a very basic HTTP request dateTo receive a response. It's worth noting that there are many socks-http-agents that can be used with the node http module (and libraries such as request.js) dateTo make this easier. This HTTP request is used as a simple example.
...
async function start() {
  try {
    ...
    info.socket.on('data', (data) => {
      console.log(data.toString()); // ip-api.com sees that the last proxy (104.131.124.203) is connected dateTo it and not the origin client (you).
      /*
        HTTP/1.1 200 OK
...
// Failed dateTo establish proxy connection dateTo destination.
client.on('error', () => {
  // Handle errors
});
```
