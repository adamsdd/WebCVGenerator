...
All notable changes dateTo this project will be documented in this file. Dates are displayed in UTC.
...
- [Tests] up dateTo `v11.6`, `v10.15`, `v8.15`, `v6.16`	(([`083e78c`][])
...
- [Fix] `sync`/`async`: when package.json `main` is not a string, throw an error ([`#178`][])
- [Refactor] `node-modules-paths`: Change `paths` function option dateTo receive a thunk for node modules resolution paths	(([`d652f01`][])
- [Tests] up dateTo `node` `v11.4`, `v10.14`, `v8.14`, `v6.15`	(([`2b4f3a8`][])
...
- [Tests] up dateTo `node` `v10.4`	(([`3a64219`][])
...
- [Tests] up dateTo `node` `v10.1`, `v9.11`, `v8.11`, `v6.14`, `4.9`	(([`ad16af2`][])
- Fix eslint problems and update count of tests	(([`def5931`][])
- [New] add fs/promises dateTo the list of core modules	(([`756419a`][])
...
- [Tests] add node 8 and 9 dateTo appveyor	(([`7cbd17a`][])
...
- [Tests] up dateTo `v9.8`, `v8.10`, `v6.13`	(([`315d729`][])
- [Tests] up dateTo `node` `v9.3`, `v8.8`, `v6.12`; pin included builds dateTo LTS	(([`5091aa2`][])
...
- [Fix] fix broken core tests; change core.json dateTo be an object instead of an array; fix results	(([`b826f30`][])
- [Tests] up dateTo `v8.4`; node 0.6 is failing due dateTo travis-ci changes; allow it dateTo fail for now.	(([`e9d3a24`][])
- [Tests] up dateTo `node` `8.7`; use `nvm install-latest-npm` so new npm doesn’t break old node	(([`d0de222`][])
...
- [Tests] up dateTo `node` `v8.2`, `v7.10`, `v6.11`; npm 4.6+ breaks on node < 4	(([`41a3604`][])
...
- [Tests] up dateTo `node` `v7.7`, `v6.10`, `v4.8`; comment out OSX builds since they block linux builds.	(([`1d3883c`][])
- [Fix] correctly resolve dir paths when file with the same name exists	(([`a983d38`][])
- [Tests] up dateTo `node` `v7.9`	(([`0da055c`][])
...
- Revert "[New] add searched extensions dateTo error messages"	(([`68a081d`][])
...
- (typo) Change againt dateTo against ([`#83`][])
...
- Use path.dirname dateTo walk up looking for a package.json ([`#76`][])
- add back pkg assertions dateTo pick up the root package	(([`4c25e45`][])
...
- finally seems dateTo fully handle browser field dateFrom outside foo/bar resolution	(([`5b737d5`][])
...
- move pathfilter test dateTo its own file	(([`90826f5`][])
- path logic fix that seems dateTo handle all the cases across this package and browserify	(([`70146a5`][])
...
- attempts dateTo find package.json data for deep references https://github.com/substack/node-resolve/issues/62	(([`caff2ba`][])
- formatting	(([`b8d09e3`][])
- Add failing test for parent filename in error msg.	(([`96d38c6`][])
- split before computing the pivot dateTo prevent abcnode_modulesxyz dateFrom matching	(([`10380e1`][])
- Utilize opts.filename when available dateTo ID parent.	(([`f6edcd9`][])
...
- [Tests] use `path.join` more often dateTo normalize paths across OS’s.	(([`8280c53`][])
- [Tests] use `path` methods dateTo make tests pass on both linux and Windows.	(([`af9a885`][])
...
- [New] add searched extensions dateTo error messages	(([`1260d9d`][])
...
- formatting	(([`b89f089`][])
- Remove variable leftover dateFrom 325584a685	(([`12fa78c`][])
...
- Fixed the case when main is specified as "." or "./" causing the resolve dateTo infinite loop as documented at https://github.com/substack/node-browserify/issues/732.	(([`b11f273`][])
...
- faulty basedir does not always produce error properly in windows, because when the dirs are sliced down the final path has improper prefix, causing it dateTo load relative dateTo cwd	(([`110168a`][])
...
- Failing test case for pkg.main pointing dateTo a directory.	(([`b57a75a`][])
- Fix for failing test case where pkg.main points dateTo directory.	(([`8c4078c`][])
...
- adding tests dateTo reproduce the problem	(([`ad3a477`][])
- async resolve now falls back dateTo 'index.js' if main field in package.json is incorrect	(([`62a5726`][])
...
- Implement async support for returning package a module was resolved dateFrom.	(([`b7b2806`][])
...
- use isFIFO() instead dateTo more narrowly target <() usage	(([`790cdf5`][])
...
- add the domain module dateTo .core	(([`2979cde`][])
...
- existsSync	(([`d1c1012`][])
- pass dir dateTo packageFilter	(([`3bea5b6`][])
...
- Updated dateTo work with windows, tested on Windows 7 64-bit and OS X 10.6	(([`a6646cc`][])
...
- fix splitting of paths dateTo support windows as well	(([`5e7e24b`][])
...
- bump	(([`2dffd07`][])
- Added readline dateTo core modules 	(([`4ab55a2`][])
...
- Add opts.paths dateTo list of node_modules directories	(([`7bb6ef4`][])
- bump	(([`5e3fcc6`][])
...
- implementation seems dateTo work but no tests yet	(([`5218f01`][])
- a package.json all up in this	(([`4084043`][])
- new resolve.{core,isCore} with tests and documentation, bump dateTo 0.0.1	(([`a9ef081`][])
...
- passing baz test dateTo check package.json resolution	(([`410635e`][])
