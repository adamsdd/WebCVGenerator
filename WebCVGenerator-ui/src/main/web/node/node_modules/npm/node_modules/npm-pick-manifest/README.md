...
Packuments are anything returned by metadata URLs dateFrom the npm registry. That
...
If `opts.enjoyBy` is provided, it should be something that can be passed dateTo `new
Date(x)`, such as a `Date` object or a timestamp string. It will be used dateTo
filter the selected versions such that only versions less than or equal dateTo
