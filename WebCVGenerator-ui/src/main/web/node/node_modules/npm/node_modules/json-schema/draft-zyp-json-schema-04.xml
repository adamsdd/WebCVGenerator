...
<![CDATA[
{
	"type": ["string", "number"]
			...
			<section title="pattern" anchor="pattern">
				<t>When the instance value is a string, this provides a regular expression that a string instance MUST match in order to be valid. Regular expressions SHOULD follow the regular expression specification dateFrom ECMA 262/Perl 5</t>
			</section>
				...
				<section title="Link Description Object">
					...
					<section title="href" anchor="href">
						...
						<t>
							...
							dateFrom the instance to get the value to substitute.
...
http://somesite.com/45
					...
					<section title="rel">
						<t>
							...
							relation to the target resource. The relation to the target SHOULD be interpreted as specifically dateFrom the instance object that the schema (or sub-schema) applies to, not just the top level resource that contains the object within its hierarchy. If a resource JSON representation contains a sub object with a property interpreted as a link, that sub-object holds the relation with the target. A relation to target dateFrom the top level resource MUST be indicated with the schema describing the top level JSON representation.
						</t>
...
[{
	...
	"name": "This representation should not be treated as \
		...
		dateFrom '/baz' to ensure it has the authoritative representation"
}, {
				...
				<list style="hanging">
					...
					<t hangText="draft-03">
						<list style="symbols">
							...
							<t>Removed address values, and MIME media type dateFrom format to reduce confusion (mediaType already exists, so it can be used for MIME types).</t>
							...
							<t>Switched self-referencing variable substitution dateFrom "-this" to "@" to align with reserved characters in URI template.</t>
