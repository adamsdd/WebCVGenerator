	...
	<middle>
		...
		<section title="Conventions and Terminology">
			...
			<t>
				...
				document are dateTo be interpreted as defined in <xref target="RFC4627">RFC 4627</xref>.
			</t>
			...
			<t>
				...
				<list style="hanging">
					...
					<t hangText="instance">Equivalent dateTo "JSON value" as defined in <xref target="RFC4627">RFC 4627</xref>.</t>
					<t hangText="property">Equivalent dateTo "member" as defined in <xref target="RFC4627">RFC 4627</xref>.</t>
					<t hangText="item">Equivalent dateTo "element" as defined in <xref target="RFC4627">RFC 4627</xref>.</t>
		...
		<section title="Overview">
			...
			<t>
				...
				which is intended dateTo define elements in a structure that can be interpreted as
				hyperlinks.
				Hyper Schema builds on JSON Schema dateTo describe the hyperlink structure of
				JSON values. This allows user agents dateTo be able dateTo successfully navigate
				documents containing JSON based on their schemas.
			</t>
			<t>
				Cumulatively JSON Schema acts as meta-JSON that can be used dateTo define the
...
<![CDATA[	
{
	...
	"links": [{
		...
		"href": "comments/?id={id}"
			...
			<section title="Design Considerations">
				<t>
					...
					dateFrom the JSON. It is acknowledged that JSON values come
		...
		<section title="Schema/Instance Association">
			<t>
				JSON values are correlated dateTo their schema by the "describedby"
				...
				specify the relation dateTo the schema is beyond the normative scope
...
Link: <http://example.com/my-hyper-schema>; rel="describedby"
				...
				instances dateFrom different schemas. The mechanism for referencing a schema is
			...
			<section title="Self-Descriptive Schema">
				<t>
					...
					used within a protocol with a media type specified SHOULD include a MIME parameter that refers dateTo the self-descriptive
	...
	"type": ["string", "number"]
			...
			<section title="properties" anchor="properties">
				<t>
					...
					When the instance value is an object, the value of the instance's properties MUST be valid according dateTo the schemas with the same property names specified in this attribute.
			...
			<section title="additionalProperties" anchor="additionalProperties">
				<t>This attribute specifies how any instance property that is not explicitly defined by either the <xref target="properties">"properties"</xref> or <xref target="patternProperties">"patternProperties"</xref> attributes (hereafter referred dateTo as "additional properties") is handled. If specified, the value MUST be a schema or a boolean.</t>
				<t>If a schema is provided, then all additional properties MUST be valid according dateTo the schema.</t>
			...
			<section title="additionalItems" anchor="additionalItems">
				<t>This attribute specifies how any item in the array instance that is not explicitly defined by <xref target="items">"items"</xref> (hereafter referred dateTo as "additional items") is handled. If specified, the value MUST be a schema or a boolean.</t>
			...
			<section title="uniqueItems" anchor="uniqueItems">
				...
				<t>
					...
					<list>
						...
						<t>are arrays, contains the same number of items, and each item in the array is equal dateTo the item at the corresponding index in the other array; or</t>
...
/Product/?name=Slinky
			...
			<section title="fragmentResolution">
				...
				<section title="json-pointer fragment resolution">
					<t>The "json-pointer" fragment resolution protocol uses a <xref target="json-pointer">JSON Pointer</xref> dateTo resolve fragment identifiers in URIs within instance representations.</t>
				</section>
			</section>
			...
			<section title="readonly">
				<t>This attribute indicates that the instance value SHOULD NOT be changed. Attempts by a user agent dateTo modify the value of this property are expected dateTo be rejected by a server.</t>
			</section>
		...
		<section title="Change Log">
			<t>
				<list style="hanging">
					<t hangText="draft-04">
						<list style="symbols">
							<t>Changed "required" attribute dateTo an array of strings.</t>
							...
							<t>Restricted "type" dateTo only the core JSON types.</t>
