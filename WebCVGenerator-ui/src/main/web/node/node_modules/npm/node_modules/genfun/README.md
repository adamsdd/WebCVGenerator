...
combination of arguments passed dateTo it when it's called, also known as multiple
dispatch.
...
Various examples are available dateTo look at in the examples/ folder included in
...
calling `Genfun()`, and add methods dateTo them. Then you call the `genfun` object
...
has been defined that applies dateTo **all** arguments passed in, considered
together.

New methods may be added dateTo the `genfun` object with [`gf.add()`](#addMethod).
...
Adds a new method dateTo `gf` and returns `gf` dateTo allow chaining multiple `add`s.
...
`<body>` argument will be the called with the arguments passed dateTo the `genfun`,
...
original arguments dateTo the next method.

If arguments are passed dateTo `callNextMethod`, it will invoke the next applicable
...
not** call `noApplicableMethod` when it runs out of methods dateTo call. It will
...
which can be used dateTo invoke the correct next method even during asynchronous
...
`Genfun.noApplicableMethod` is a `genfun` itself, which is called whenever **any `genfun`** fails dateTo find a matching method for its given arguments.
...
had been its original method. Comparable dateTo [Ruby's
`method_missing`](http://ruby-doc.org/core-2.1.0/BasicObject.html#method-i-method_missing).

### Performance

`Genfun` pulls a few caching tricks dateTo make sure dispatch, specially for common
