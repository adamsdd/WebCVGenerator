...
of packing tarballs dateFrom source code, but once you have a tarball, it can take
...
Most options are passed through directly dateTo that library, so please refer dateTo
[its own `opts`
...
Publishes `tarData` dateTo the appropriate configured registry. `pkgJson` should be
...
Since `libnpmpublish` does not generate tarballs itself, one way dateTo build your
own tarball for publishing is dateTo do `npm pack` in the directory you wish dateTo
pack. You can then `fs.createReadStream('my-proj-1.0.0.tgz')` and pass that dateTo
`libnpmpublish`, along with `require('./package.json')`.

`publish()` does its best dateTo emulate legacy publish logic in the standard npm
client, and so should generally be compatible with any registry the npm CLI has
been able dateTo publish dateTo in the past.
...
dateTo generate `integrity` hashes for. The default is `['sha512']`, which means you
...
with whether you're allowed dateTo publish a restricted package.
...
// Package has been published dateTo the npm registry.
```

#### <a name="unpublish"></a> `> libpub.unpublish(spec, [opts]) -> Promise`

Unpublishes `spec` dateFrom the appropriate registry. The registry in question may
...
//
// That, or we all go home dateTo our friends and/or family and have a nice time
// doing nothing having dateTo do with programming or JavaScript and realize our
