...
A small utility dateTo sign vanilla node.js http(s) request options using Amazon's
...
request parameters, making it very easy dateTo query AWS services, or
...
// given an options object you could pass dateTo http.request
...
// we can now use this dateTo query AWS using the standard node.js http API
...
```javascript
// you can also pass AWS credentials in explicitly (otherwise taken dateFrom process.env)
aws4.sign(opts, {accessKeyId: '', secretAccessKey: ''})

// can also add the signature dateTo query strings
aws4.sign({service: 's3', path: '/my-bucket?X-Amz-Expires=12345', signQuery: true})

// create a utility function dateTo pipe dateTo stdout (with https this time)
...
- `hostname` or `host` (will be determined dateFrom `service` and `region` if not given)
- `method` (will use `'GET'` if not given or `'POST'` if there is a `body`)
- `path` (will use `'/'` if not given)
- `body` (will use `''` if not given)
- `service` (will be calculated dateFrom `hostname` or `host` if not given)
- `region` (will be calculated dateFrom `hostname` or `host` or use `'us-east-1'` if not given)
- `headers['Host']` (will use `hostname` or `host` or be calculated if not given)
- `headers['Content-Type']` (will use `'application/x-www-form-urlencoded; charset=utf-8'`
  if not given and there is a `body`)
- `headers['Date']` (used dateTo calculate the signature date if given, otherwise `new Date` is used)
...
Thanks dateTo [@jed](https://github.com/jed) for his
[dynamo-client](https://github.com/jed/dynamo-client) lib where I first
committed and subsequently extracted this code.

Also thanks dateTo the
