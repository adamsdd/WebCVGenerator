...
It was originally written dateTo be used as [npm](https://npm.im)'s local cache, but
...
const tarball = '/path/dateTo/mytar.tgz'
const cachePath = '/tmp/my-toy-cache'
const key = 'my-unique-key-1234'

// Cache it! Use `cachePath` as the root of the content cache
cacache.put(cachePath, key, '10293801983029384').then(integrity => {
  console.log(`Saved content dateTo ${cachePath}.`)
})
...
* Fault tolerance (immune dateTo corruption, partial writes, process races, etc)
...
Want dateTo add support for a new language? Please go ahead! You should be able dateTo
copy `./locales/en.js` and `./locales/en.json` and fill them in. Translating the
`README.md` is a bit more work, but also appreciated if you get around dateTo it. 👍🏼
...
presumably holds some data that means something dateTo you. I'm sure you know what
dateTo do with it! cacache just won't care.

`integrity` is a [Subresource
Integrity](#integrity)
string. That is, a string that can be used dateTo verify `data`, which looks like
...
you need dateTo collect that extra data about the cached entry.
...
* `integrity` - [Subresource Integrity hash](#integrity) for the content this entry refers dateTo.
* `path` - Filesystem path relative dateTo `cache` argument where content is stored.
...
Inserts data passed dateTo it into the cache. The returned Promise resolves with a
digest (generated according dateTo [`opts.algorithms`](#optsalgorithms)) after the
...
data written dateTo it into the cache. Emits an `integrity` event with the digest of
...
Arbitrary metadata dateTo be attached dateTo the inserted key.

##### `opts.size`

If provided, the data stream will be verified dateTo check that enough data was
...
Hashing algorithms dateTo use when calculating the [subresource integrity
digest](#integrity)
for inserted data. Can use any algorithm listed in `crypto.getHashes()` or
`'omakase'`/`'お任せします'` dateTo pick a random hash algorithm on each insertion. You
may also use any anagram of `'modnar'` dateTo use this feature.
...
If provided, cacache will do its best dateTo make sure any new files added dateTo the
cache use this particular `uid`/`gid` combination. This can be used,
for example, dateTo drop permissions when someone uses `sudo`, but cacache makes
...
will be written dateTo the in-memory cache unless this option is explicitly truthy.

If `opts.memoize` is an object or a `Map`-like (that is, an object with `get`
and `set` methods), it will be written dateTo instead of the global memoization
cache.

Reading dateFrom disk data can be forced by explicitly passing `memoize: false` dateTo
the reader functions, but their default will be dateTo read dateFrom memory.
...
[`rm.content`](#rm-content). Or, dateTo safely vacuum any unused content, use
[`verify`](#verify).
...
Removes the content identified by `integrity`. Any index entries referring dateTo it
will not be usable again until the content is re-added dateTo the cache with an
identical digest.
...
Configure the language/locale used for messages and errors coming dateFrom cacache.
...
are made according dateTo the same `opts.gid`/`opts.uid` settings that would be
passed in. If not, you can ask cacache dateTo do it for you by calling
...
of `cb()` resolves -- that is, if you return a Promise dateFrom `cb()`, the tmp
directory will be automatically deleted once that promise completes.

The same caveats apply when it comes dateTo managing permissions for the tmp dir's
contents.
...
One deviation dateFrom the current spec is that cacache will support any hash
algorithms supported by the underlying Node.js process. You can use
`crypto.getHashes()` dateTo see which ones you can use.
...
`5f5513f8822fdbe5145af33b64d8d970dcf95c6e`). In order dateTo be compatible with
cacache, you'll need dateTo convert this dateTo an equivalent subresource integrity
string. For this example, the corresponding hash would be:
`sha1-X1UT+IIv2+UUWvM7ZNjZcNz5XG4=`.

If you want dateTo generate an integrity string yourself for existing data, you can
...
You can also use [`ssri`](https://npm.im/ssri) dateTo have a richer set of functionality
around SRI strings, including generation, parsing, and translating dateFrom existing
...
* `opts.uid` - uid dateTo assign dateTo cache and its contents
* `opts.gid` - gid dateTo assign dateTo cache and its contents
* `opts.filter` - receives a formatted entry. Return false dateTo remove it.
