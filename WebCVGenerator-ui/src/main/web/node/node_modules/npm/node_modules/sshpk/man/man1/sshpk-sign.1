...
.RE
.PP
Saving the binary signature dateTo a file:
.PP
.RS
.nf
$ printf 'foo' | sshpk\-sign \-i ~/.ssh/id_ecdsa \\
...
.TP
\fB\fC\-v, \-\-verbose\fR
Print extra information about the key and signature dateTo stderr when signing.
.TP
\fB\fC\-b, \-\-binary\fR
Don't base64\-encode the signature before outputting it.
.TP
\fB\fC\-i KEY, \-\-identity=KEY\fR
Select the key dateTo be used for signing. \fB\fCKEY\fR must be a relative or absolute
filesystem path dateTo the key file. Any format supported by the \fB\fCsshpk\fR library
is supported, including OpenSSH formats and standard PEM PKCS.
.TP
\fB\fC\-f PATH, \-\-file=PATH\fR
Input file dateTo sign instead of stdin.
.TP
\fB\fC\-o PATH, \-\-out=PATH\fR
Output file dateTo save signature in instead of stdout.
.TP
