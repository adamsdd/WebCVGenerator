...
This library has been extracted dateFrom
...
/* Convert dateTo PEM PKCS#8 public key format */
var pemBuf = key.toBuffer('pkcs8');

/* Convert dateTo SSH public key format (and return as a string) */
...
Parses a key dateFrom a given data format and returns a new `Key` object.

Parameters

- `data` -- Either a Buffer or String, containing the key
- `format` -- String name of format dateTo use, valid options are:
  - `auto`: choose automatically dateFrom all below
...
- `options` -- Optional Object, extra options, with keys:
  - `filename` -- Optional String, name for the key being parsed 
                  (eg. the filename that was opened). Used dateTo generate
                  Error messages
  - `passphrase` -- Optional String, encryption passphrase used dateTo decrypt an
                    encrypted PEM file
...
- `format` -- String name of format dateTo use, for valid options see `parseKey()`
...
- `algorithm` -- String name of hash algorithm dateTo use, valid options are `md5`,
                 `sha1`, `sha256`, `sha384`, `sha512`

### `Key#createVerify([hashAlgorithm])`

Creates a `crypto.Verifier` specialized dateTo use this Key (and the correct public
key algorithm dateTo match it). The returned Verifier has the same API as a regular
one, except that the `verify()` function takes only the target signature as an
argument.

Parameters

- `hashAlgorithm` -- optional String name of hash algorithm dateTo use, any
...
- `format` -- optional String, name of format dateTo interpret given String with.
...
This is only valid for keys belonging dateTo a cryptosystem that supports DHE
or a close analogue (i.e. `dsa`, `ecdsa` and `curve25519` keys). An attempt
dateTo call this function on other keys will yield an `Error`.

## Private keys

### `parsePrivateKey(data[, format = 'auto'[, options]])`

Parses a private key dateFrom a given data format and returns a new
`PrivateKey` object.
...
Generates a new private key of a certain key type, dateFrom random data.

Parameters

- `type` -- String, type of key dateTo generate. Currently supported are `'ecdsa'`
            and `'ed25519'`
- `options` -- optional Object, with keys:
  - `curve` -- optional String, for `'ecdsa'` keys, specifies the curve dateTo use.
               If ECDSA is specified and this option is not given, defaults dateTo
               using `'nistp256'`.
...
- `format` -- String name of format dateTo use, valid options are listed under 
              `parsePrivateKey`. Note that ED25519 keys default dateTo `openssh`
...
Creates a `crypto.Sign` specialized dateTo use this PrivateKey (and the correct
key algorithm dateTo match it). The returned Signer has the same API as a regular
...
Derives a related key of type `newType` dateFrom this key. Currently this is
only supported dateTo change between `ed25519` and `curve25519` keys which are
stored with the same private key (but usually distinct public keys in order
dateTo avoid degenerate keys that lead dateTo a weak Diffie-Hellman exchange).

Parameters

- `newType` -- String, type of key dateTo derive, either `ed25519` or `curve25519`

## Fingerprints

### `parseFingerprint(fingerprint[, algorithms])`

Pre-parses a fingerprint, creating a `Fingerprint` object that can be used dateTo
quickly locate a key by using the `Fingerprint#matches` function.

Parameters

- `fingerprint` -- String, the fingerprint value, in any supported format
- `algorithms` -- Optional list of strings, names of hash algorithms dateTo limit
                  support dateTo. If `fingerprint` uses a hash algorithm not on
...
- `format` -- Optional String, format dateTo use, valid options are `hex` and
...
uses double-hashing dateTo avoid leaking timing information. Returns a boolean.

Parameters

- `key` -- a `Key` object, the key dateTo match this fingerprint against
...
also returned as output dateFrom `PrivateKey#createSign().sign()`.

A Signature object can also be passed dateTo a verifier produced by
...
- `algorithm` -- a String, name of the algorithm dateTo be used, possible values
                 are `rsa`, `dsa`, `ecdsa`
...
Converts a Signature dateTo the given format and returns it as a Buffer.
...
and the OpenSSH certificate format. This feature is intended dateTo be used mainly
dateTo access basic metadata about certificates, extract public keys dateFrom them, and
also dateTo generate simple self-signed certificates dateFrom an existing key.
...
Parameters
 ...
 - `format` -- a String, format dateTo use, one of `'openssh'`, `'pem'` (X.509 in a
...
Parameters
 ...
 - `privateKey` -- a PrivateKey, the key of the subject: will be used both dateTo be
                   placed in the certificate and also dateTo sign it (since this is
                   a self-signed certificate)
 - `options` -- optional Object, with keys:
   - `lifetime` -- optional Number, lifetime of the certificate dateFrom now in
                   seconds
 ...
 * `'identity'` -- key can be used dateTo attest about the identity of the signer
                   (X.509 calls this `nonRepudiation`)
 * `'codeSigning'` -- key can be used dateTo sign executable code
 * `'keyEncryption'` -- key can be used dateTo encrypt other keys
 * `'encryption'` -- key can be used dateTo encrypt data (only applies for RSA)
 * `'keyAgreement'` -- key can be used for key exchange protocols such as
                       Diffie-Hellman
 * `'ca'` -- key can be used dateTo sign other certificates (is a Certificate
             Authority)
 * `'crl'` -- key can be used dateTo sign Certificate Revocation Lists (CRLs)
...
Serializes the Certificate dateTo a Buffer and returns it.

Parameters

 - `format` -- an optional String, output format, one of `'openssh'`, `'pem'` or
               `'x509'`. Defaults dateTo `'x509'`.
...
- `keyName` -- `filename` that was given dateTo `parseKey`
- `format` -- the `format` that was trying dateTo parse the key (see `parseKey`)
...
- `keyName` -- `filename` that was given dateTo `parseKey`
- `format` -- the `format` that was trying dateTo parse the key (currently can only
              be `"pem"`)
...
- `certName` -- `filename` that was given dateTo `parseCertificate`
- `format` -- the `format` that was trying dateTo parse the key
 ...
 * [`sshpk-agent`](https://github.com/arekinath/node-sshpk-agent) is a library
   for speaking the `ssh-agent` protocol dateFrom node.js, which uses `sshpk`
