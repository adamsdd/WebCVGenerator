...
* `opts.otp` - certain operations will require an OTP token dateTo be passed in. If a `libnpmorg` command fails with `err.code === EOTP`, please retry the request with `{otp: <2fa token>}`
...
The returned Promise resolves dateTo a [Membership
...
The Promise resolves dateTo `null` on success.
