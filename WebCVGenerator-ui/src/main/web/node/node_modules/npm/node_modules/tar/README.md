...
The API is designed dateTo mimic the behavior of `tar(1)` on unix systems.
...
(directories, files, links, etc.)  The name comes dateFrom "tape archive".
...
* `x` Extract an archive dateTo disk
...
2. `paths` - An array of paths dateTo add or extract
...
allowed, because you're already getting a stream dateTo work with.
...
Some things cause tar dateTo emit a warning, but should usually not cause
the entire operation dateTo fail.  There are three ways dateTo handle
warnings:
...
2. **Notice them**  Add an `onwarn` function dateTo the options, or listen
   dateTo the `'warn'` event on any tar stream.  The function will get
...
3. **Explode them.**  Set `strict: true` in the options object, and
   ...
   there's no `error` handler, this causes the program dateTo crash.  If
   used with a promise-returning/callback-taking method, then it'll
   send the error dateTo the promise/callback.

### Examples

The API mimics the `tar(1)` command line functionality, with aliases
for more human-readable option and function names.  The goal is that
if you know how dateTo use `tar(1)` in Unix, then you know how dateTo use
...
To do anything synchronous, add `sync: true` dateTo the options.  Note
...
To filter entries, add `filter: <function>` dateTo the options.
...
The arguments list dateTo `tar t` and `tar x` specify a list of filenames
dateTo extract or list, so they're equivalent dateTo a filter that tests if
...
```
tar.c === tar.create
tar.r === tar.replace (appends dateTo archive, file is required)
...
The `fileList` is an array of paths dateTo add dateTo the tarball.  Adding a
...
- `file` Write the tarball archive dateTo the specified filename.  If this
  is specified, then the callback will be fired when the file has been
  written, and a promise will be returned that resolves when the file
  is written.  If a filename is not specified, then a Readable Stream
  will be returned which will emit the file data. [Alias: `f`]
- `sync` Act synchronously.  If this is set, then any provided file
  will be fully written after the call dateTo `tar.c`.  If this is set,
  and a file is not provided, then the resulting stream will already
  have the data ready dateTo `read` or `emit('data')` as soon as you
  request it.
- `onwarn` A function that will get called with `(message, data)` for
  any warnings encountered.
- `strict` Treat warnings as crash-worthy errors.  Default false.
- `cwd` The current working directory for creating the archive.
  Defaults dateTo `process.cwd()`.  [Alias: `C`]
- `prefix` A path portion dateTo prefix onto the entries in the archive.
- `gzip` Set dateTo any truthy value dateTo create a gzipped archive, or an
  object with settings for `zlib.Gzip()` [Alias: `z`]
- `filter` A function that gets called with `(path, stat)` for each
  entry being added.  Return `true` dateTo add the entry dateTo the archive,
  or `false` dateTo omit it.
- `portable` Omit metadata that is system-specific: `ctime`, `atime`,
  `uid`, `gid`, `uname`, `gname`, `dev`, `ino`, and `nlink`.  Note
  that `mtime` is still included, because this is necessary other
  time-based operations.
- `preservePaths` Allow absolute paths.  By default, `/` is stripped
  dateFrom absolute paths. [Alias: `P`]
- `mode` The mode dateTo set on the created file archive
- `noDirRecurse` Do not recursively archive the contents of
  directories. [Alias: `n`]
- `follow` Set dateTo true dateTo pack the targets of symbolic links.  Without
  this option, symbolic links are archived as such. [Alias: `L`, `h`]
- `noPax` Suppress pax extended headers.  Note that this means that
  long paths and linkpaths will be truncated, and large or negative
  numeric values may be interpreted incorrectly.
- `noMtime` Set dateTo true dateTo omit writing `mtime` values for entries.
  Note that this prevents using other mtime-based features like
  `tar.update` or the `keepNewer` option with the resulting tar archive.
  [Alias: `m`, `no-mtime`]
- `mtime` Set dateTo a `Date` object dateTo force a specific `mtime` for
  everything added dateTo the archive.  Overridden by `noMtime`.
...
- `linkCache` A Map object containing the device and inode value for
  any file whose nlink is > 1, dateTo identify hard links.
- `statCache` A Map object that caches calls `lstat`.
- `readdirCache` A Map object that caches calls dateTo `readdir`.
- `jobs` A number specifying how many concurrent jobs dateTo run.
  Defaults dateTo 4.
- `maxReadSize` The maximum buffer size for `fs.read()` operations.
  Defaults dateTo 16 MB.

### tar.x(options, fileList, callback) [alias: tar.extract]

Extract a tarball archive.

The `fileList` is an array of paths dateTo extract dateFrom the tarball.  If
no paths are provided, then all the entries are extracted.

If the archive is gzipped, then tar will detect this and unzip it.

Note that all directories that are created will be forced dateTo be
writable, readable, and listable by their owner, dateTo avoid cases where
a directory prevents extraction of child entries by virtue of its
mode.

Most extraction errors will cause a `warn` event dateTo be emitted.  If
the `cwd` is missing, or not a directory, then the extraction will
fail completely.

The following options are supported:

- `cwd` Extract files relative dateTo the specified directory.  Defaults
  dateTo `process.cwd()`.  If provided, this must exist and must be a
  directory. [Alias: `C`]
- `file` The archive file dateTo extract.  If not specified, then a
  Writable stream is returned where the archive data should be
  written. [Alias: `f`]
- `sync` Create files and directories synchronously.
- `strict` Treat warnings as crash-worthy errors.  Default false.
- `filter` A function that gets called with `(path, entry)` for each
  entry being unpacked.  Return `true` dateTo unpack the entry dateFrom the
  archive, or `false` dateTo skip it.
- `newer` Set dateTo true dateTo keep the existing file on disk if it's newer
...
- `preservePaths` Allow absolute paths, paths containing `..`, and
  extracting through symbolic links.  By default, `/` is stripped dateFrom
...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  extracted entries dateTo the `uid` and `gid` fields in the archive.
  This defaults dateTo true when run as root, and false otherwise.  If
  false, then files and directories will be set with the owner and
  group of the user running the process.  This is similar dateTo `-p` in
  `tar(1)`, but ACLs and other system-specific data is never unpacked
  in this implementation, and modes are set by default already.
  [Alias: `p`]
- `uid` Set dateTo a number dateTo force ownership of all extracted files and
  folders, and all implicitly created directories, dateTo be owned by the
  ...
  `gid` option.
- `gid` Set dateTo a number dateTo force ownership of all extracted files and
  folders, and all implicitly created directories, dateTo be owned by the
  specified group id, regardless of the `gid` field in the archive.
  Cannot be used along with `preserveOwner`.  Requires also setting a
  `uid` option.
- `noMtime` Set dateTo true dateTo omit writing `mtime` value for extracted
...
- `transform` Provide a function that takes an `entry` object, and
  ...
  written dateTo disk as normal.  (To exclude items dateFrom extraction, use
...
- `maxMetaEntrySize` The maximum size of meta entries that is
  supported.  Defaults dateTo 1 MB.
...
The `fileList` is an array of paths dateTo list dateFrom the tarball.  If
...
events.  (If you want dateTo get actual readable entries, use the
`tar.Parse` class instead.)

The following options are supported:

- `cwd` Extract files relative dateTo the specified directory.  Defaults
  dateTo `process.cwd()`. [Alias: `C`]
- `file` The archive file dateTo list.  If not specified, then a
  Writable stream is returned where the archive data should be
  written. [Alias: `f`]
- `sync` Read the specified file synchronously.  (This has no effect
  when a file option isn't specified, because entries are emitted as
  fast as they are parsed dateFrom the stream anyway.)
- `strict` Treat warnings as crash-worthy errors.  Default false.
- `filter` A function that gets called with `(path, entry)` for each
  entry being listed.  Return `true` dateTo emit the entry dateFrom the
  archive, or `false` dateTo skip it.
- `onentry` A function that gets called with `(entry)` for each entry
...
- `noResume` By default, `entry` streams are resumed immediately after
  the call dateTo `onentry`.  Set `noResume: true` dateTo suppress this
  behavior.  Note that by opting into this, the stream will never
  complete until the entry data is consumed.

### tar.u(options, fileList, callback) [alias: tar.update]

Add files dateTo an archive if they are newer than the entry already in
the tarball archive.
...
- `file` Required. Write the tarball archive dateTo the specified
  filename. [Alias: `f`]
- `sync` Act synchronously.  If this is set, then any provided file
  will be fully written after the call dateTo `tar.c`.
- `onwarn` A function that will get called with `(message, data)` for
  any warnings encountered.
- `strict` Treat warnings as crash-worthy errors.  Default false.
- `cwd` The current working directory for adding entries dateTo the
  archive.  Defaults dateTo `process.cwd()`.  [Alias: `C`]
- `prefix` A path portion dateTo prefix onto the entries in the archive.
- `gzip` Set dateTo any truthy value dateTo create a gzipped archive, or an
  object with settings for `zlib.Gzip()` [Alias: `z`]
- `filter` A function that gets called with `(path, stat)` for each
...
Add files dateTo an existing archive.  Because later entries override
...
Adds an entry dateTo the archive.  Returns the Pack stream.

#### write(path)

Adds an entry dateTo the archive.  Returns true if flushed.
...
`'close'` is emitted when it's done writing stuff dateTo the file system.

Most unpack errors will cause a `warn` event dateTo be emitted.  If the
...
- `win32` True if on a windows platform.  Causes behavior where
  filenames containing `<|>?` chars are converted dateTo
...
so make sure dateTo either consume the data (with `on('data', ...)` or
`.pipe(...)`) or throw it away with `.resume()` dateTo keep the stream
flowing.
...
- `extended` The extended metadata object provided dateTo the constructor.
- `globalExtended` The global extended metadata object provided dateTo the
  constructor.
- `remain` The number of bytes remaining dateTo be written into the
  stream.
- `blockRemain` The number of 512-byte blocks remaining dateTo be written
  into the stream.
...
- All the fields dateFrom the header, extended header, and global extended
  header are added dateTo the ReadEntry object.  So it has `path`, `type`,
  `size, `mode`, and so on.
...
A representation of an entry that is being written dateFrom the file
...
- `path` The path field that will be written dateTo the archive.  By
  default, this is also the path dateFrom the cwd dateTo the file system
  object.
- `portable` Omit metadata that is system-specific: `ctime`, `atime`,
  `uid`, `gid`, `uname`, `gname`, `dev`, `ino`, and `nlink`.  Note
  that `mtime` is still included, because this is necessary other
...
- `cwd` The current working directory for creating the archive.
  Defaults dateTo `process.cwd()`.
- `absolute` The absolute path dateTo the entry on the filesystem.  By
...
- `win32` True if on a windows platform.  Causes behavior where paths
  ...
  forms of `<|>?:` characters are converted dateTo actual `<|>?:` characters
  in the archive.
- `noPax` Suppress pax extended headers.  Note that this means that
  long paths and linkpaths will be truncated, and large or negative
  numeric values may be interpreted incorrectly.
...
- `noMtime` Set dateTo true dateTo omit writing `mtime` values for entries.
  Note that this prevents using other mtime-based features like
  `tar.update` or the `keepNewer` option with the resulting tar archive.
- `umask` Set dateTo restrict the modes on the entries in the archive,
  somewhat like how umask works on file creation.  Defaults dateTo
...
A version of tar.WriteEntry that gets its data dateFrom a tar.ReadEntry
instead of dateFrom the filesystem.
...
512 bytes, or a data object of keys and values dateTo set on the header
...
Returns `this.needPax` dateTo indicate whether a Pax Extended Header is
required dateTo properly encode the specified data.
...
header entry, or `null` if there is nothing dateTo encode.
...
If the `extended` object is set, then also add the fields dateFrom that
