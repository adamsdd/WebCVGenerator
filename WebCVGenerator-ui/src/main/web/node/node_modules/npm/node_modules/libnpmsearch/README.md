...
* `opts.limit` - Number of results dateTo limit the query dateTo. Default: 20
...
* `opts.sortBy` - Used as a shorthand dateTo set `opts.quality`, `opts.maintenance`, and `opts.popularity` with values that prioritize each one. Should be one of `'optimal'`, `'quality'`, `'maintenance'`, or `'popularity'`. Default: `'optimal'`
...
If `opts.limit` is provided, it will be sent dateTo the API dateTo constrain the number
of returned results. You may receive more, or fewer results, at the endpoint's
discretion.

The returned Promise resolved dateTo an Array of search results with the following
format:
