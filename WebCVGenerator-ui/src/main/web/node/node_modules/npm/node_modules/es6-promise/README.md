...
For API details and how dateTo use promises, see the <a href="http://www.html5rocks.com/en/tutorials/es6/promises/">JavaScript Promises HTML5Rocks article</a>.
...
around this, you can use a string dateTo access the property as shown in the
following example.
...
Notice that we don't assign the result of `polyfill()` dateTo any variable. The `polyfill()` method will patch the global environment (in this case dateTo the `Promise` name) when called.

## Building & Testing

You will need dateTo have PhantomJS installed globally in order dateTo run the tests.

`npm install -g phantomjs`

* `npm run build` dateTo build
* `npm test` dateTo run tests
* `npm start` dateTo run a build watcher, and webserver dateTo test
